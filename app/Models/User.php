...
{ 
    ...
    public static function GetLowestBidder($id=null){
        ...
        $Symbol = SmGeneralSettings::select('currency_symbol')->find(1);
            ...
            if($LowestBidder !=""){
                $name ='<span class="text-capitalize">'. $LowestBidder->company_name.' (<span class="letterspecing"> '.$Symbol->currency_symbol.' '.User::NumberToBangladeshiTakaFormat($LowestBidder->company_bid_amount).'</span> )</span>';
            }else{ 
    ...
    public static function getFileName($path=null){ //upcoming tender
        if($path == NULL) return '';
