...
class SysCustomerController extends Controller
{
    ...
    public function customer(Request $request)
    {
        ...
        /*try {
            if(session('logged_session_data.company_id') == 1 && Auth::user()->id == 1){
                $dup = DB::table('sys_cust_suppl_merge')
                ->select('from_id', 'to_id')
                ->where('process', 'sys_crm_deals')
                ->get();

                if (count($dup) > 0) {
                    foreach ($dup as $d) {
                        $fromRecord = SysCustSuppl::where('id', $d->from_id)->first();
                        if ($fromRecord) {
                            $toRecord = SysCustSuppl::where('id', $d->to_id)->first();
                            if ($toRecord) {
                                $existingAccess = $toRecord->company_access;
                                $newAccess = $fromRecord->company_access;
                                $mergedAccess = array_unique(array_filter(array_merge(
                                    explode(',', $existingAccess),
                                    explode(',', $newAccess)
                                )));
                                SysCustSuppl::where('id', $d->to_id)->update([
                                    'company_access' => implode(',', $mergedAccess)
                                ]);
                            }
                        }
                    }
        ...
        /*try {
            if(session('logged_session_data.company_id') == 1 && Auth::user()->id == 1){
                $dup = DB::table('sys_chartofaccounts')
                ->select('account_code')
                ->groupBy('account_code')
                ->havingRaw('COUNT(*) >= 2')
                ->pluck('account_code');
                return $dup;
                $g_cust = SysCustSuppl::select('id','code','name','company_id')->wherein('code',$dup)->where('company_id',1)->get();
                if (count($g_cust)>0) {
                    foreach ($g_cust as $dt) {
                        $g_acco = SysChartofAccounts::select('id','account_code','account_name','company_id')->where('account_code',$dt->code)->where('company_id',1)->first();
                        $new_code = SysHelper::get_new_customer_code();
                        if(isset($g_acco)){
                            SysCustSuppl::where('id',$dt->id)->update(['code' => $new_code]);
                            SysChartofAccounts::where('id',$g_acco->id)->update(['account_code' => $new_code, 'account_name' => $dt->name]);
                        }
                    }
                }
            }
        ...
        // try {
        //     if(Auth::user()->id == 1){
        //         $update_deal_id = DB::table('sys_crm_deals')->select('cust_id')->distinct()->get();
        //         foreach ($update_deal_id as $value) {
        //             $address_new = SysCustSupplAddressbook::where('cust_suppl_id',$value->cust_id)->orderby('id','desc')->first();
        //             if(isset($address_new)){
        //                 SysCrmDeals::where('cust_id',$value->cust_id)->update([
        ...
        try{
            //if($_POST){
                $customer_query = SysCustSuppl::select('id','name','code','contcat_person','mobile','email', 'vat_country', 'vat_percentage', 'vat_number', 'credit_limit', 'credit_days', 'payment_terms', 'transaction_type','customer_type','first_name','last_name','contcat_number','customer_salutation','status','is_file','internal')
                ->where('catid',1);
            ...
            if(SysHelper::get_pagination_post($request)){
                ...
                if ($request->sales_person != "") {
                    ...
                    }else{$customer_query->where('id', 0);}
                }
                if ($request->sales != "") {
...
$active_customers = $dealIdsFromInvoice
    ...
    ->merge($leadIds);
            ...
            $customer = $customer_query->orderby('name','asc')->get();
            ...
            return view('backEnd.cust-suppl.customer_list', compact('customer','staff','countries','states','customer_list','duplicate_customer'));
    ...
    public function customer_pending(Request $request)
    {
        $r = SysHelper::get_data_by_role();
        $company_id = $r[0];
        $com_id = session('logged_session_data.company_id');
        ...
        try{
            //if($_POST){
            if(SysHelper::get_pagination_post($request)){
                $customer_query = SysCustSuppl::select('id','name','contcat_person','mobile','email', 'vat_country', 'vat_percentage', 'vat_number', 'credit_limit', 'credit_days', 'payment_terms', 'transaction_type','status')
            ...
            $customer = $customer_query->where('status',3)->orderby('name','asc')->paginate(50);
            
            return view('backEnd.cust-suppl.customer_list_pending', compact('customer','staff','countries','states'));
    ...
    public function addCustomerStore(Request $request)
    {
        ...
        $company_access = "";
        if($request->company_access!="") { $company_access =implode(",",$request->company_access); }
        if(!in_array(1,$request->company_access)){ $company_access='1,'.$company_access; }
    ...
    public function customerUpdate(Request $request)
    {
        ...
        try {
            ...
            if(SysHelper::check_customer_is_added($request->customer_name)==0){
                 $new_customer->name = $request->customer_name;
                 $new_customer->customer_name_display = $request->customer_name_display;
                 SysChartofAccounts::where('account_code',$new_customer->code)->update(['account_name' => $request->customer_name,'company_access' => $company_access,'internal' => $request->internal]);
            }
    ...
    public function customer_import_data(Request $request)
    {
        try{
            ...
            if(count($data)>0){
                ...
                $new_customer->credit_limit = $dt->credit_limit;
...
public function customer_from_list(Request $request)
    {
        $r = SysHelper::get_data_by_role();
        $company_id = $r[0];
        $staff = SysHelper::get_sales_persons2();
        ...
        try{
            //if($_POST){
            if(SysHelper::get_pagination_post($request)){
                $customer_query = SysCustSupplForm::select('id','name','first_name','contcat_number','mobile','vat_number','email')->wherein('company_id',$company_id)->where('status',1)->where('catid',1);
                if ($request->company_name != "") {
                    $customer_query->where('name','like','%'.$request->company_name.'%');
                }
                if ($request->contact_name != "") {
            ...
            return view('backEnd.cust-suppl.customer_list_form', compact('customer','staff','countries','states'));
    ...
    public function customer_form_edit($id)
    {
        try {
            ...
            $a=$editData->name; $b=$editData->email;
    ...
    public function customer_form_merge($cust_id, $sub_id){
        try {
            ...
            $new_customer->credit_limit = $new_customer->credit_limit;
    ...
    public function add_customer_detail_popup(Request $request)//kunal modified
    {
        try {
            ...
            } else { $company_access = session('logged_session_data.company_id').','.$request->company_id; }
    ...
    function customerMerge(Request $request)
    {
        try {
            ...
            DB::commit();Toastr::success('Customer Merged Successfully', 'Success');
            return redirect()->back();
        } catch (\Throwable $th) {
            DB::rollBack();
            return $th;
