...
class SysPaymentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->payment_through == 3 && $request->chequebook && $request->cheque_number) {
                if ($this->isChequeNumberAlreadyUsed($request->chequebook, $request->cheque_number)) {
            ...
            if (count(array_filter($request->account_id)) > 0 && count(array_filter($request->amount)) > 0) {
                ...
                if (intval($request->payment_through) === 3 && intval($request->mode) === 2) {
                ...
                // Persist temporary attachments from add form so they are linked to the created payment
                if ($request->has('temp_attachment_ids')) {
                    $attachmentIds = array_filter((array) $request->temp_attachment_ids);
                    if (count($attachmentIds) > 0) {
                        SysPaymentAttachment::whereIn('id', $attachmentIds)
                            ->where(function ($query) {
                                $query->where('sys_payment_id', 0)
                                      ->orWhereNull('sys_payment_id');
                            })
                            ->where('created_by', Auth::user()->id)
                            ->update(['sys_payment_id' => $py->id]);
                    }
                }
                ...
                // For cheque payments, redirect to payment details page, with flag to trigger cheque download from that page.
                if (intval($request->payment_through) === 3 && intval($request->mode) === 2 && !empty($py->cheque_id)) {
                    
                    return redirect('payment/' . $py->id . '?download_cheque=1');
                }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if (intval($request->payment_through) === 3 && intval($request->mode) === 2) {
                ...
                $isdone = DB::table('sys_payment_cheque')->where('sys_payment_id', $py->id)->get();
            ...
            DB::commit();
            Toastr::success('Operation successful', 'Success');
            if (intval($py->payment_through) === 3 && intval($py->mode) === 2 && !empty($py->cheque_id)) {
                return redirect('payment/' . $id . '?download_cheque=1');
            }
    ...
    public function payment_cheque_list(Request $request)
    {
        try {
            ...
            $selectedChequeId = $request->input('cheque_id');
            $selectedSortId = $request->input('sort_id');
            $from_date = $request->input('from_date');
            $to_date = $request->input('to_date');

            $chequebook = Chequebook::where('company_id', $com_id)->when($selectedBankId, function($q) use ($selectedBankId){
                return $q->where('bank_id', $selectedBankId);
            })->get();
            ...
            if ($selectedChequeId) {
                $query->where('cheque_id', $selectedChequeId);
            }
            ...
            if ($selectedSortId) {
                switch ($selectedSortId) {
                    case 1: // Today
                        $query->whereDate('cheque_date', date('Y-m-d'));
                        break;
                    case 2: // This Week
                        $start_date = date('Y-m-d', strtotime('monday this week'));
                        $end_date = date('Y-m-d', strtotime('sunday this week'));
                        $query->whereDate('cheque_date', '>=', $start_date)->whereDate('cheque_date', '<=', $end_date);
                        break;
                    case 3: // Last Week
                        ...
                        break;
                    case 4: // This Month
                        $query->whereRaw("DATE_FORMAT(cheque_date, '%Y-%m') = '" . date('Y-m') . "'");
                        break;
                    case 5: // Last Month
                    ...
                    case 6: // Last 6 Months
                        ...
                        break;
                    case 7: // This Year
                        $query->whereRaw("DATE_FORMAT(cheque_date, '%Y') = '" . date('Y') . "'");
                        break;
                    case 8: // Last Year
                        $query->whereRaw("DATE_FORMAT(cheque_date, '%Y') = '" . date('Y', strtotime('-1 year')) . "'");
                        break;
                    default:
                        break;
                }
            ...
            if (!$selectedSortId || !in_array($selectedSortId, [9, 10, 11])) {
            ...
            return view('backEnd.payment.payment_cheque_list', compact('data', 'bank', 'supplier', 'chequebook', 'currency1', 'currency2', 'selectedBankId', 'selectedSupplierId', 'selectedChequeId', 'selectedSortId', 'from_date', 'to_date'));
        } catch (\Throwable $th) {
            return $th;
        }
    }

    public function getPaymentAttachments($paymentId)
    {
        try {
            if (empty($paymentId) || $paymentId == 0) {
                $attachments = SysPaymentAttachment::where('sys_payment_id', 0)
                    ->where('created_by', Auth::user()->id)
                    ->orderBy('created_at', 'desc')
                    ->get();
            } else {
                $attachments = SysPaymentAttachment::where('sys_payment_id', $paymentId)
                    ->orderBy('created_at', 'desc')
                    ->get();
            }
    ...
    public function uploadPaymentAttachments(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'sys_payment_id' => 'nullable|integer',
        ...
        $paymentId = intval($request->input('sys_payment_id', 0));
        if ($paymentId < 0) {
            $paymentId = 0;
        }
        ...
        foreach ($request->file('files', []) as $file) {
            ...
            $dest = public_path('uploads/payment_attachments');
            if (!file_exists($dest)) {
                mkdir($dest, 0755, true);
            }
            $file->move($dest, $fileName);
            $storedPath = 'uploads/payment_attachments/' . $fileName;

            $attachment = SysPaymentAttachment::create([
                'sys_payment_id' => $paymentId,
        ...
        if ($paymentId === 0) {
            $allAttachments = SysPaymentAttachment::where('sys_payment_id', 0)
                ->where('created_by', Auth::user()->id)
                ->orderBy('created_at', 'desc')
                ->get();
        } else {
            $allAttachments = SysPaymentAttachment::where('sys_payment_id', $paymentId)
                ->orderBy('created_at', 'desc')
                ->get();
        }
    ...
    public function deletePaymentAttachment($id)
    {
        try {
            $attachment = SysPaymentAttachment::findOrFail($id);
    ...
    public function downloadPaymentAttachment($id)
    {
        try {
            $attachment = SysPaymentAttachment::findOrFail($id);
            $storedPath = str_replace('\\', '/', $attachment->file_path);
    ...
    {
        try {
            $usedPayments = SysPaymentCheque::with(['payment', 'bankname', 'suppliername', 'deal_code', 'createdby'])
                ->where('cheque_id', $chequebookId)
                ->where('company_id', session('logged_session_data.company_id'))
                ->where('status', 1)
                ->get();

            $used = $usedPayments->map(function ($item) {
                return [
                    'id' => $item->id,
                    ...
                    'cheque_attachment' => $item->attachment,
    ...
    public function getNextAvailableCheque($bankId)
    {
        try {
            ...
            if ($chequebooks->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'No chequebooks found',
                ]);
            }

            foreach ($chequebooks as $book) {
                // Build a hash-map of used cheque numbers for O(1) lookup
                $used = SysPayment::where('chequebook_id', $book->id)
            ...
            return response()->json([
                'success' => false,
                'message' => 'All cheque numbers are exhausted for this bank',
            ]);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    ...
    public function getChequebookIdByChequeNumber($bankId, $chequeNumber)
    {
        try {
            $chequeNumber = trim($chequeNumber);
            if ($chequeNumber === '') {
                return response()->json(['success' => false, 'message' => 'Cheque number is required'], 422);
            }

            $chequebook = Chequebook::where('bank_id', $bankId)
                ->whereNull('deleted_at')
                ->whereRaw('? BETWEEN CAST(start_no AS SIGNED) AND CAST(end_no AS SIGNED)', [$chequeNumber])
                ->orderBy('id', 'asc')
                ->first();

            if (!$chequebook) {
                return response()->json(['success' => false, 'message' => 'Cheque number does not belong to any active chequebook for this bank'], 404);
            }

            return response()->json([
                'success' => true,
                'chequebook_id' => $chequebook->id,
                'chequebook_doc' => $chequebook->doc_number,
                'chequebook_start_no' => $chequebook->start_no,
                'chequebook_end_no' => $chequebook->end_no,
            ]);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => $e->getMessage()], 500);
        }
    ...
    public function storePaymentDeal(Request $request)
    {
        ...
        try {
            ...
            if (count(array_filter($request->account_id)) > 0 && count(array_filter($request->amount)) > 0) {
                ...
                if (intval($request->payment_through) === 3 && intval($request->mode) === 2 && !empty($py->cheque_id)) {
                    return redirect('crm-deal-track-approval-list/' . $request->dealtrack_id . '?download_cheque=1&cheque_id=' . $py->cheque_id);
                }
