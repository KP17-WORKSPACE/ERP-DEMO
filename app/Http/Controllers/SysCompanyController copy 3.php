...
// =====================================================================
        ...
        try {
            ...
            while ($request->has("warehouses.$warehouseCount.warehouse_name")) {
                ...
                if ($lastDocumentNumber === null) {
                    $lastDocumentNumber = SysHelper::get_new_code_lead(
                        'company_warehouses',
                        'WH',
                        'document_number',
    ...
    public function createAccounts(SysCompany $company)
    {
        ...
        try {
            $cash = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['cash']
            )->first();

            $sales = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['sales']
            )->first();

            $output_vat = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['outputvat']
            )->first();

            $closing_stock = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['closingstock']
            )->first();

            $opening_stock = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['openingstock']
            )->first();

            $purchase_return = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['purchasereturn']
            )->first();

            $sales_return = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['salesreturn']
            )->first();

            $purchase = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['purchase']
            )->first();

            $input_vat = SysAccountGroupSub2::whereRaw(
                "LOWER(REPLACE(title, ' ', '')) = ?",
                ['inputvat']
            )->first();

            if ($cash) {
                $this->storeAccounts('Cash in Hand', $cash->group_id, $cash->sub_id, $cash->id, $company->id, $company->other_code);
            }

            if ($sales) {
                $this->storeAccounts('Sales', $sales->group_id, $sales->sub_id, $sales->id, $company->id, $company->other_code);
            }

             if ($output_vat) {
                $this->storeAccounts('VAT on Sales', $output_vat->group_id, $output_vat->sub_id, $output_vat->id, $company->id, $company->other_code);
            }

             if ($closing_stock) {
                $this->storeAccounts('Closing Stock', $closing_stock->group_id, $closing_stock->sub_id, $closing_stock->id, $company->id, $company->other_code);
            }

             if ($opening_stock) {
                $this->storeAccounts('Opening Stock', $opening_stock->group_id, $opening_stock->sub_id, $opening_stock->id, $company->id, $company->other_code);
            }

            if ($purchase_return) {
                $this->storeAccounts('Purchase Return', $purchase_return->group_id, $purchase_return->sub_id, $purchase_return->id, $company->id, $company->other_code);
            }

             if ($sales_return) {
                $this->storeAccounts('Sales Return', $sales_return->group_id, $sales_return->sub_id, $sales_return->id, $company->id, $company->other_code);
            }

             if ($purchase) {
                $this->storeAccounts('Purchase', $purchase->group_id, $purchase->sub_id, $purchase->id, $company->id, $company->other_code);
            }

            if ($input_vat) {
                $this->storeAccounts('VAT on Purchase', $input_vat->group_id, $input_vat->sub_id, $input_vat->id, $company->id, $company->other_code);
            }
    ...
    public function storeAccounts($account_name, $group_id, $sub_id, $sub2_id, $company_id, $other_code)
    {
        try {
            DB::beginTransaction();
            $amount_dr = 0;
    ...
    public function storeAccountsBank(SysCompany $company, Request $request)
    {
        try {
            DB::beginTransaction();
            $amount_dr = 0;
            ...
            foreach ($banks as $bank) {
              ...
              $accounts = new SysChartofAccounts();
                $accounts->account_code = SysHelper::get_new_account_code_without_company($company->other_code);
                $accounts->account_name = $request->account_name;
                $groups = SysAccountGroupSub2::select('group_id', 'sub_id')->where('title', 'Bank')->first();
