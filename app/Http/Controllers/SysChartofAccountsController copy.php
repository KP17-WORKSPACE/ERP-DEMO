...
class SysChartofAccountsController extends Controller
{
    ...
    public function chartofaccountsAdd(Request $request)
    {
        try {
            ...
            $accounts = SysChartofAccounts::whereRaw("find_in_set($com_id,sys_chartofaccounts.company_access)")->orderBy('group', 'asc')
   ...
   public function edit_accounts(Request $request, $id)
    ...
    {
        $com_id = session('logged_session_data.company_id');
        $accounts2 = SysChartofAccounts::whereRaw("find_in_set($com_id, sys_chartofaccounts.company_access)")
            ->orderBy('group', 'asc')
            ->orderBy('subgroup', 'asc')
            ->orderBy('subgroup2', 'asc')
            ->get();

        $account_tran2 = SysChartofAccountsTransaction::select('account_id', 'transaction_date', 'debit_amount', 'credit_amount')
            ->where('company_id', $com_id)
            ->where('transaction_type', 'openingbalance')
            ->get();
        ...
        return view('backEnd.chart-of-accounts.accounts-table-view', compact('accounts2', 'account_tran2','accountgroupsub2'));
    }
