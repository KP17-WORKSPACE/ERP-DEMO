...
class SysCustomerController extends Controller
{
    ...
    public function customer(Request $request, $id = null)
    {
        ...
        try {
            //if($_POST){
            $customer_query = SysCustSuppl::select('id', 'name', 'code', 'contcat_person', 'mobile', 'email', 'vat_country', 'vat_percentage', 'vat_number', 'credit_limit', 'credit_days', 'payment_terms', 'transaction_type', 'customer_type', 'first_name', 'last_name', 'contcat_number', 'customer_salutation', 'status', 'is_file', 'internal', 'created_at')
                ->where('catid', 1);
            ...
            if (SysHelper::get_pagination_post($request)) {
                ...
                if ($request->assigned_filter != "") {
                    if ($request->assigned_filter == 'none') {
                        // Customers with no assigned users
                        $customer_query->whereDoesntHave('assignedUsers');
                    } else {
                        // Customers assigned to specific user(s)
                        $customer_query->whereHas('assignedUsers', function ($q) use ($request) {
            ...
            return view('backEnd.cust-suppl.customer_list', compact('customer', 'staff', 'countries', 'states', 'customer_list', 'duplicate_customer', 'ctrl_company_name', 'ctrl_contact_name', 'ctrl_email', 'ctrl_vat_country', 'ctrl_vat_state', 'ctrl_sales_person', 'active_id', 'selectedCus', 'addCustomer', 'action', 'editData', 'status_filter', 'information_filter'));
