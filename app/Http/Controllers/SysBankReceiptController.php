...
class SysBankReceiptController extends Controller
{
     ...
     */
    public function bankreceiptList(Request $request)
    {
        $com_ids = SysHelper::get_company_access();
        try{
            $bankreceipt = SysBankReceipt::wherein('company_id',$com_ids)->all();
            //$cashreceipt_list = SysCashReceiptList::all();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse($bankreceipt, null);
            }
            return view('backEnd.bank-receipt.bankreceiptlist', compact('bankreceipt'));
        }catch (\Exception $e) {
    ...
    public function bankreceiptAdd(Request $request)
    {
        try{
        ...
        $cust = SysChartofAccounts::select('id','account_name')->where('status',1)->orderby('account_name','asc')->get(); //1cust, 3supp
            ...
            return view('backEnd.bank-receipt.bankreceiptadd', compact('cust','receiptmode','currency'));
        }catch (\Exception $e) {
           Toastr::error('Operation Failed', 'Failed');
           return redirect()->back();
        }
    ...
    public function store(Request $request)
    {
        try{
            ...
            if(count(array_filter($request->account_id))>0 && count(array_filter($request->amount))>0){
                $mxid = SysBankReceipt::max('id');
                $br = new SysBankReceipt();
                $br->doc_number = 'BR-'.($mxid+1);
                $br->doc_date = date('Y-m-d', strtotime($request->doc_date));
                $br->receipt_mode = $request->receipt_mode;
                $br->cheque_date = date('Y-m-d', strtotime($request->cheque_date));
                $br->cheque_number = $request->cheque_number;
                $br->cheque_bank_name = $request->cheque_bank_name;
                $br->currency = $request->currency;
                $br->narration = $request->narration;
                $br->status = 1;
                $br->created_by = Auth::user()->id;
                $br->created_at = Carbon::now('+04:00');
                $br->company_id = session('logged_session_data.company_id');
                $results = $br->save();
                $br->toArray();
                
                SysHelper::trn_chartof_accounts_transaction($request->receipt_mode,$br->id,$br->doc_number,$br->doc_date,'bankreceipt',array_sum($request->amount),'0.00',$request->narration,1);

                for($i = 0; $i < count($request->account_id); $i++) {
                    if($request->account_id[$i] !="" && $request->amount[$i] !=""){
                        SysHelper::trn_chartof_accounts_transaction($request->account_id[$i],$br->id,$br->doc_number,$br->doc_date,'bankreceipt','0.00',$request->amount[$i],$request->remarks[$i],1);
                    }
                }
                DB::commit();
    ...
    public function edit(Request $request,$id)
    {
       try{
        ...
        $editData = SysBankReceipt::find($id);
        $editDataList = SysChartofAccountsTransaction::where(['transaction_id' => $id, 'transaction_type' => 'bankreceipt'])->whereNotIn('account_id',$acc_id)->get();      
        ...
        return view('backEnd.bank-receipt.bankreceiptadd', compact('cust', 'receiptmode', 'editData','editDataList','currency'));
        }catch (\Exception $e) {
            return $e;
        Toastr::error('Operation Failed', 'Failed');
        return redirect()->back(); 
    ...
    {
       try{
        $cust = SysChartofAccounts::select('id','account_name')->where('subgroup',1)->where('status',1)->get(); //1cust, 3supp
        ...
        return view('backEnd.bank-receipt.bankreceiptadd', compact('cust', 'receiptmode', 'editData','editDataList','view'));
        }catch (\Exception $e) {
        Toastr::error('Operation Failed', 'Failed');
        return redirect()->back(); 
        }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $br->narration = $request->narration;
            $br->status = 1;
            $br->updated_by = Auth::user()->id;
            $br->updated_at = Carbon::now('+04:00');
            $br->company_id = session('logged_session_data.company_id');
            $br->update();

            SysChartofAccountsTransaction::query()
                ->where(['transaction_id' => $id, 'transaction_type' => 'bankreceipt'])
            ...
            DB::commit();
            Toastr::success('Operation successful', 'Success');
            return redirect('bankreceipt/'.$id.'/edit');
    ...
    public function getbrbalancelist(Request $request)
    {
        $items = DB::select("CALL get_bank_receipt_adjestments($request->account_id)");
