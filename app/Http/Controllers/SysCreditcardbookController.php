...
class SysCreditcardbookController extends Controller
{
    ...
    {
        try {
            $accounts = SysHelper::get_credit_card_account();
            if ($accounts === 0 || $accounts === null || !is_countable($accounts)) {
                $accounts = collect();
            }
            session()->forget('gl_data');

            if (session('credit_card_book_session_data')) {
                $from_date = session('credit_card_book_session_data.from_date');
                $to_date = session('credit_card_book_session_data.to_date');
                $account_id = session('credit_card_book_session_data.account_id');
            } else {
            ...
            if ($_POST) {
                ...
                session()->put('credit_card_book_session_data', $data1);
            }
            ...
            return view('backEnd.creditcardbook.view', compact('data', 'accounts', 'account_id', 'from_date', 'to_date', 'filter_by', 'receipt_pdc_list', 'payment_pdc_list','pdc_filter'));
        } catch (\Exception $e) {
            return $e;
