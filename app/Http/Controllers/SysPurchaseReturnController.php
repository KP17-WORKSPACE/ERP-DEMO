...
class SysPurchaseReturnController extends Controller
{
     ...
     */
    public function purchasereturnList(Request $request, $id = null)
    {
        try {
            ...
            if (SysHelper::get_pagination_post($request)) {
                ...
                if ($request->purchase_order_number != "") {
                    ...
                    $ctrl_po_number = $request->purchase_order_number;
                }
            ...
            if ($request->has('pr_action')) {
                ...
                if ($poAction === 'add') {
                    $action = 'add';
                    $addData = $this->purchasereturnAdd(); // Get all data for adding
                ...
                if ($id != null) {
                    ...
                    $firstRecord = $purchasereturn->first();
                    if ($firstRecord) {
                        $active_id = $firstRecord->id;
                        $data = $this->get_pr_pdf_data($active_id);
                    }
                }
            }
            ...
            return view('backEnd.purchasereturn.purchasereturnlist', compact('purchasereturn', 'supplier_list', 'data', 'active_id', 'action', 'editData', 'addData', 'ctrl_doc_number', 'ctrl_supplier', 'ctrl_customer', 'ctrl_po_number', 'ctrl_grn_number', 'ctrl_pi_number', 'ctrl_date'));
        } catch (\Exception $e) {
            return $e;
    ...
    public function get_pr_pdf_data($id)
    {
        try {
            ...
            $ship_mob = SmStaff::select('mobile')->where('user_id', $pr->created_by)->first();
            $ship_mob = $ship_mob->mobile;

            $bill_contact_name = $pr->createdby->full_name;
            if (!empty($pr)) {
                $company = SysCompany::find($pr->company_id);
                $pi_item = SysPurchaseReturnList::where('pr_id', '=', $pr->id)->orderBy('sort_id')->get();
    ...
    public function purchasereturnAdd()
    {
        ...
        try {
            ...
            return compact('purchasereturn', 'currency', 'vendors', 'paymentterms', 'suppliertype', 'purchasetype', 'countries', 'states', 'customs_freight_account', 'items', 'cart', 'company', 'salesman', 'customer_reference_list', 'customer');
        } catch (\Exception $e) {
            return $e;
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    {
        try {
            $ret = SysPurchaseInvoice::select('id', 'doc_number')->where('vendors', $request->id)->wherein('return_status', [0, 2])->where('status', 1)->where('company_id', session('logged_session_data.company_id'))->orderby('id', 'desc')->get();
            return json_encode(array('data' => $ret));
        } catch (\Exception $e) {
            $ret = 'ERROR';
            return json_encode(array('data' => $ret));
    ...
    {
        try {
            $ret = DB::table('sys_purchase_invoice_items as pi_items')->select('pi_items.*', 'pi.doc_number', 'pi.pi_date', 'pi.ref_company_id', 'pi.currency', 'pi.awbno', 'pi.lpo_number', 'pi.lpo_date', 'pi.bill_number', 'pi.bill_date', 'pi.shipping_supplier', 'pi.payment_terms', 'pi.payment_terms2', 'pi.reference', 'pi.location', 'pi.warehouse', 'pi.sales_person', 'pi.sales_person_name', 'pi.narration', 'deals.code', 'pi.ref_po_id', 'pi.ref_grn_id', 'items.part_number as part_number_txt', 'items.description', 'items.product_type', 'pi_items.sort_id')
    ...
    public function store(Request $request)
    {
        ...
        if ($request->part_number[0] != "none" && $request->qty[0] != "" && (float) str_replace(',', '', $request->unitprice[0] ?? 0) != 0) {
        } elseif (count($cart) > 0) {
        } else {
        ...
        try {
            ...
            $pr->pi_date = !empty($request->pi_date)
                ? Carbon::createFromFormat('d/m/Y', $request->pi_date)->format('Y-m-d') : null;
            ...
            $pr->shipping_supplier = $request->shipping_supplier;
            $pr->shipping_name = $request->shipping_name;
            $pr->shipping_email = $request->shipping_email;
            $pr->shipping_contact_no = $request->shipping_contact_no;
            $pr->shipping_address_1 = $request->shipping_address_1;
            $pr->supplier_country = $request->supplier_country;
            $pr->supplier_state = $request->supplier_state;
            $pr->vat_percent = $request->vat_percent;
            $pr->vat_number = $request->vat_number;
            $pr->supplier_type = $request->supplier_type;
            $pr->purchase_type = $request->purchase_type;
            $pr->debit_note = $request->debit_note;


            if ($request->has('ref_company_id')) {
                $pr->ref_company_id = implode(',', $request->ref_company_id);
            } else {
                $pr->ref_company_id = null;
            }
            ...
            if (!is_null($sales_person) && $sales_person !== '') {
                ...
                if (is_numeric($sales_person)) {
                    ...
                    $pr->sales_person = (int) $sales_person;
                    $pr->sales_person_name = null;
                } else {

                    // Manually entered name
                    $pr->sales_person = null;
                    $request->sales_person = null; // Clear the sales_person field to avoid confusion
                    $pr->sales_person_name = trim($sales_person);
                }
            }
            ...
            if (
                count(array_filter($request->part_number)) > 0 &&
                count(array_filter($request->qty)) > 0 &&
                count(array_filter($request->unitprice)) > 0
            ) {
            ...
            if ((count(array_filter($request->part_number)) > 0 && count(array_filter($request->qty)) > 0 && count(array_filter($request->unitprice)) > 0)) {
                for ($i = 0; $i < count($request->part_number); $i++) {
                    if ($request->part_number[$i] != "") {
                        ...
                        $prl->description = $request->description[$i];
                        $prl->qty = $request->qty[$i];
                        $prl->unitprice = (float) str_replace(',', '', $request->unitprice[$i] ?? 0);
                        $value = (float) str_replace(',', '', $request->value[$i] ?? 0);
                        $prl->value = $value;
                        $discount = (float) str_replace(',', '', $request->discount[$i] ?? 0);
                        $prl->discount = $discount;
                        $prl->taxableamount = (float) str_replace(',', '', $request->taxableamount[$i] ?? 0);
                        $prl->vat = (float) str_replace(',', '', $request->tax[$i] ?? 0);
                        $prl->vatamount = (float) str_replace(',', '', $request->vatamount[$i] ?? 0);
                        $prl->sort_id = $request->sort_id[$i];
                        $prl->remarks = "";
                        $prl->serialno = $request->serial_no[$i];
                        $prl->status = 1;
                        ...
                        // Only update return_qty if pi_id is valid
                        if (!empty($request->pi_id) && $request->pi_id != '') {
                            db::table('sys_purchase_invoice_items')
                                ->where('pi_id', $request->pi_id)
                                ->where('part_number', $request->part_number[$i])
                                ->update(['return_qty' => $request->qty[$i]]);
                        }
                        ...
                        $istock->price_out = ($value - $discount) / $request->qty[$i];
            ...
            DB::commit();
            Toastr::success('Operation successful', 'Success');
            return redirect('purchase-return/' . $pr->id);
    ...
    public function edit($id)
    {
        try {
            ...
            return compact('purchasereturn', 'currency', 'currencylist2', 'vendors', 'paymentterms', 'suppliertype', 'purchasetype', 'countries', 'states', 'customs_freight_account', 'edit', 'editList', 'invoice_amount', 'pri_adjestment', 'salesman', 'customer_reference_list', 'customer');
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // $pr->ref_company_id = $request->ref_company_id;
            ...
            for ($i = 0; $i < count($request->part_number); $i++) {
                ...
                if ($request->part_number[$i] != "" && $request->qty[$i] != "" && $request->unitprice[$i] != "") {
                    ...
                    $sii->unitprice = $unitprice;
                    $sii->value = $value;
                    $sii->discount = $discount;
                    $sii->taxableamount = (float) str_replace(',', '', $request->taxableamount[$i] ?? 0);
                    $sii->vatamount = (float) str_replace(',', '', $request->vatamount[$i] ?? 0);
                    $sii->status = 1;
                    $sii->created_by = Auth::user()->id;
                    $sii->sort_id = $request->sort_id[$i]; // or use another field like $request->sort_id[$i] if available
                    $sii->save();
                    ...
                    // Finalize staged license keys (selected in PR edit modal) to this PR.
            ...
            if ($return_status->pi_qty != null && $return_status->pr_qty != null) {
            ...
            DB::commit();
            Toastr::success('Operation successful', 'Success');
            return redirect('purchase-return/' . $id);
        } catch (\Exception $e) {
            return $e;
            DB::rollback();
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function search(Request $request)
    {
        ...
        $query = SysPurchaseReturn::with(['currency_name:id,code', 'accountname:id,account_code,account_name'])->select(DB::raw('sys_purchase_return.*, (SELECT GROUP_CONCAT(doc_number) as doc_number FROM sys_purchase_grn WHERE lpo_number = sys_purchase_return.lpo_number) AS grnno, (SELECT GROUP_CONCAT(doc_number) FROM sys_purchase_order WHERE id = sys_purchase_return.ref_po_id) AS po_no, (SELECT SUM(taxableamount)+SUM(vatamount) AS amount FROM sys_purchase_return_list WHERE pr_id = sys_purchase_return.id) AS amount, (SELECT GROUP_CONCAT(code) FROM sys_crm_deals WHERE id=sys_purchase_return.deal_id) AS code'));
        $query->where('company_id', session('logged_session_data.company_id'));

        $amc_list = $query->where(function ($query) use ($q, $formattedDate) {
            $query->where(function ($qsub) use ($q) {
                ...
                if ($q) {
                    $qsub->where('doc_number', 'like', "%{$q}%")
                        ...
                        ->orWhere('pi_number', 'like', "%{$q}%")
                        ->orWhereHas('accountname', function ($q1) use ($q) {
    ...
    public function download($id)
    {
        try {
            ...
            if (!empty($pr)) {
                ...
                $pdf = PDF::loadView('backEnd.pdf_print.pr_pdf', $data);
                $pdf->setPaper('A4', 'portrait');
                return $pdf->download($pr->doc_number . '-' . $pr->accountname->account_name . ".pdf");
