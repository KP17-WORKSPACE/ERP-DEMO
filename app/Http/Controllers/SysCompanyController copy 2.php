...
// =====================================================================
    ...
    public function storeBasic(Request $request)
    {
        ...
        try {
            ...
            if ($request->country == 'UAE' || $request->country == '231') { // 231 is UAE ID
                ...
                if ($request->has('compliance_documents') && is_array($request->compliance_documents)) {
                    ...
                    foreach ($request->compliance_documents as $idx => $doc) {
                        ...
                        // If a new file was uploaded for this index, move it to compliance uploads
                        if ($request->hasFile("compliance_documents.$idx.attachment")) {
                            $filePath = $this->uploadSingleFile($request, "compliance_documents.$idx.attachment", 'uploads/company/compliance');
                            $finalAttachmentPath = $filePath;
                        } elseif (!empty($doc['attachment'])) {
                            // Hidden input may carry an existing stored path
                            $finalAttachmentPath = $doc['attachment'];
                        }
                        ...
                        SysCompanyCompliance::updateOrCreate(
                            [
                                'company_id' => $companyId,
                                'trade_license_no' => $doc['document_number'] ?? null,
                            ],
                            [
                                'license_issue_date' => $this->parseDate($doc['issue_date'] ?? null),
                                'license_expiry_date' => $this->parseDate($doc['expiry_date'] ?? null),
                                'vat_issuing_authority' => $doc['issuing_authority'] ?? null,
                                'business_license_upload' => $finalAttachmentPath,
            ...
            while ($request->has("warehouses.$warehouseCount.warehouse_name")) {
                ...
                $warehouse->warehouse_country = $request->input("warehouses.$warehouseCount.warehouse_country");
                $warehouse->warehouse_state = $request->input("warehouses.$warehouseCount.warehouse_state");
                $warehouse->warehouse_city = $request->input("warehouses.$warehouseCount.warehouse_city");
                $warehouse->warehouse_area = $request->input("warehouses.$warehouseCount.warehouse_area");
                $warehouse->warehouse_building_name = $request->input("warehouses.$warehouseCount.warehouse_building_name");
                $warehouse->warehouse_flat_office_no = $request->input("warehouses.$warehouseCount.warehouse_flat_office_no");
    ...
    public function getcompanyShiftandWeekOff(Request $request)
    {
        ...
        $shiftId = SysCompany::where('id', $companyId)->value('shift_id');

        $weeklyOffs = SysCompanyHrPayrollSetting::where('company_id', $companyId)
            ->value('weekly_off_days');

        return response()->json([
            'shift_id' => $shiftId,
            'weekly_off_days' => $weeklyOffs ? json_decode($weeklyOffs, true) : [],
        ]);
    }
