...
class SysCashReceiptController extends Controller
{
     ...
     */
    public function cashreceiptList(Request $request)
    {
        $com_ids = SysHelper::get_company_access();
        try{
            $cashreceipt = SysCashReceipt::wherein('company_id',$com_ids)->get();
            //$cashreceipt_list = SysCashReceiptList::all();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse($cashreceipt, null);
            }
            return view('backEnd.cash-receipt.cashreceiptlist', compact('cashreceipt'));
        }catch (\Exception $e) {
    ...
    public function cashreceiptAdd(Request $request)
    {   
        try{
            ...
            return view('backEnd.cash-receipt.cashreceiptadd', compact('cust','receiptmode'));
        }catch (\Exception $e) {
           Toastr::error('Operation Failed', 'Failed');
           return redirect()->back();
        }
    ...
    public function store(Request $request)
    {
        try{
            ...
            if(count(array_filter($request->account_id))>0 && count(array_filter($request->amount))>0){
                $mxid = SysCashReceipt::max('id');
                $cr = new SysCashReceipt();
                $cr->doc_number = 'CR-'.($mxid+1);
                $cr->doc_date = date('Y-m-d', strtotime($request->doc_date));
                $cr->receipt_mode = $request->receipt_mode;
                $cr->narration = $request->narration;
                $cr->status = 1;
                $cr->created_by = Auth::user()->id;
                $cr->created_at = Carbon::now('+04:00');
                $cr->company_id = session('logged_session_data.company_id');
                $results = $cr->save();
                $cr->toArray();

                SysHelper::trn_chartof_accounts_transaction($request->receipt_mode,$cr->id,$cr->doc_number,$cr->doc_date,'cashreceipt',array_sum($request->amount),'0.00',$request->narration,1);

                for($i = 0; $i < count($request->account_id); $i++) {
                    if($request->account_id[$i] !="" && $request->amount[$i] !=""){
                        SysHelper::trn_chartof_accounts_transaction($request->account_id[$i],$cr->id,$cr->doc_number,$cr->doc_date,'cashreceipt','0.00',$request->amount[$i],$request->remarks[$i],1);
                    }
                }
                DB::commit();
    ...
    public function edit(Request $request,$id)
    {
       try{
        ...
        $editData = SysCashReceipt::find($id);
        $editDataList = SysChartofAccountsTransaction::where(['transaction_id' => $id, 'transaction_type' => 'cashreceipt'])->whereNotIn('account_id',$acc_id)->get();
        ...
        return view('backEnd.cash-receipt.cashreceiptadd', compact('cust', 'receiptmode', 'editData','editDataList'));
        }catch (\Exception $e) {
            return $e;
        Toastr::error('Operation Failed', 'Failed');
        return redirect()->back(); 
    ...
    public function view(Request $request,$id)
    {
       try{
        ...
        return view('backEnd.cash-receipt.cashreceiptadd', compact('cust', 'receiptmode', 'editData','editDataList','view'));
        }catch (\Exception $e) {
        Toastr::error('Operation Failed', 'Failed');
        return redirect()->back(); 
        }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            SysChartofAccountsTransaction::query()
                ->where(['transaction_id' => $id, 'transaction_type' => 'cashreceipt'])
            ...
            DB::commit();
            Toastr::success('Operation successful', 'Success');
            return redirect('cashreceipt/'.$id.'/edit');
    ...
    public function receiptadjustmentsstore(Request $request)
    {
        ...
        // $sle = new SysLedgerEntriesTemp();
        // $sle->transaction_id = $request->bi_doc_number;
        // $sle->transaction_type = $request->transaction_type;
        // $sle->account_id = $request->account_id;
        // $sle->entry_date = date('Y-m-d', strtotime($request->entry_date));
        // $sle->entry_type = $request->entry_type; //1 Debit, 2 Credit
        // $sle->amount = $request->bi_amount;
        // $sle->status = 1;
        // $sle->created_by = Auth::user()->id;
        // $sle->process_id = $request->process_id;
        // $sle->company_id = session('logged_session_data.company_id');
        // $sle->process_status = 1;
        // $sle->save();
    ...
    public function getcrbalancelist(Request $request)
    {
        $items = DB::select("CALL get_cash_receipt_adjestments($request->account_id)");
