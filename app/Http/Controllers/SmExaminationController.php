...
class SmExaminationController extends Controller
{
    ...
    public function examScheduleSearch(Request $request){
    	...
    	return view('backEnd.examination.exam_schedule_create', compact('classes', 'exams', 'assign_subjects', 'class_id', 'section_id', 'exam_id', 'exam_schedule_subjects', 'assign_subject_check','exam_types', 'exam_periods'));
    }
    ...
    public function marksRegisterSearch(Request $request){
        ...
        if($students->count() == 0 ){
            return redirect()->back()->with('message-danger', 'No data found !');
        }else{
            ...
            return view('backEnd.examination.masks_register_create', compact('exams', 'classes', 'students', 'exam_schedule_subjects', 'exam_id', 'class_id', 'section_id', 'marks_register_subjects', 'assign_subject_ids'));
        }
                ...
                if(!empty($request->marks[$sid])){
                    ...
                    foreach ($request->marks[$sid] as $part_mark) {
                        ...
                        $previous_record = SmMarkStore::where([
                            ...
                            ])->first();                                                    // Is previous record exist ?
                        ...
                        if($previous_record == "" || $previous_record == null){
                            ...
                            $marks_register = SmMarkStore::find($id);
                            $marks_register->total_marks            =       $mark_by_exam_part;
                            $marks_register->is_absent              =       $is_absent;
                            $marks_register->save();
                        }
...
//sm_mark_stores
        ...
        return view('backEnd.examination.masks_register', compact('exams', 'classes', 'marks_registers', 'marks_register', 'all_students', 'students','exam_id', 'class_id', 'section_id','exam_types'));
...
*/
        ...
        foreach ($eligible_students as $SingleStudent) { 
            foreach($eligible_subjects as $subject){
                ...
                $getMark            =  SmResultStore::where([
                    ...
                    if(empty($getMark->total_marks)){ $FinalMarks=0; }
                    else{$FinalMarks=$getMark->total_marks;}
            ...
            $exart_gp_point= SysHelper::com_curr_format($total_GPA,2,'.','');            //get gpa results
    ...
    $allresult_data = SmTemporaryMeritlist::orderBy('merit_order', 'asc')->get();
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $data['exams'] = $exams->toArray();
            $data['classes'] = $classes->toArray();
//            $data['marks_registers'] = $marks_registers->toArray();
//            $data['marks_register'] = $marks_register->toArray();
//            $data['all_students'] = $all_students->toArray();
...
//            $data['class_id'] = $class_id;
    ...
    return view('backEnd.reports.merit_list_report', compact('exams', 'classes', 'marks_registers', 'marks_register', 'all_students', 'subjects', 'class', 'section' ,'exam', 'exam_id', 'class_id','subjectlist','allresult_data','class_name','assign_subjects','exam_name'));
    }
...
//                $data['exam_id'] = $exam_id;
            ...
            return view('backEnd.reports.mark_sheet_report_student', compact('exam_types','classes','studentDetails','exams', 'classes', 'marks_register', 'subjects', 'class', 'section' ,'exam_detail', 'grades', 'exam_id', 'class_id', 'student_detail','is_result_available','exam_type_id','section_id','student_id','exam_details','class_name'));
        ...
        $exam_id = $request->exam;
        $class_id = $request->class;
        return view('backEnd.reports.mark_sheet_report_student', compact('exam_types','classes','studentDetails','exams', 'classes', 'marks_register', 'subjects', 'class', 'section' ,'exam_detail', 'grades', 'exam_id', 'class_id', 'student_detail'));
    }
