...
class SysCrmDealsController extends Controller
{
                ...
                if ($request->quotation_generated == 1) {
                    ...
                    if (count($request->part_number) > 0) {
                        ...
                        DB::table('sys_crm_deals')->where('id', $scd->id)
                            ->update(['estimated_close_date' => SysHelper::normalizeToYmd($request->estimated_close_date), 'quote_id' => $quote_id + 1, 'deal_discount' => (float) str_replace(',', '', $request->deal_discount ?? 0), 'terms_and_condition' => $request->terms_and_condition]);
    ...
    public function show(Request $request, $id = null) //kunal modified
    {
        try {
            ...
            if (SysHelper::get_pagination_post($request)) {
                ...
                if ($request->date != "") {
                    ...
                    if ($request->date2 != "") {
                        ...
                        $query->whereRaw("DATE_FORMAT(created_at, '%Y-%m-%d') = '" . $ctrl_date2 . "'");
                    }
                }
                ...
                if ($request->sort_id == 9) { //By Deal Value
                    ...
                    $deals = $query->orderby('code', 'desc')->paginate(50);
                    //$deals = $query->orderby('estimated_close_date','asc')->orderby('stage','asc')->get();
                }
            } else {
                $query = SysCrmDeals::select('id', 'code', 'deal_name', 'estimated_close_date', 'date', 'stage', 'deal_currency', 'company_id', 'deal_value', 'deal_profit', 'created_at', 'updated_at', 'cust_id', 'owner', 'deleted_at', 'quote_id','followup_date')->where('stage', '!=', 0);
            ...
            return view('backEnd.crm.DealList', compact('deals', 'vendors', 'staff', 'ctrl_cust_id', 'ctrl_stage', 'ctrl_source', 'ctrl_owner', 'ctrl_date', 'ctrl_date2', 'ctrl_deal_id', 'ctrl_isproject', 'brand', 'ctrl_brand', 'filter_by', 'product', 'designation', 'country', 'company', 'paymentterms', 'sales_person', 'pdfdata', 'active_id', 'ctrl_followup'));
...
$filter_by='';
    ...
    */
    public function edit(Request $request, $id, $qid = 0)
    {
        try {
            $r = SysHelper::get_data_by_role();
            $company_id = $r[0];
    ...
    public function createquote($id){
        ...
        try {
            ...
            return redirect('crm-deals/'.$id.'/edit/'.$newQuoteId.'?new=yes');
        }catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
        ...
        try {
           ...
           if (!empty($request->followup_date)) {
            ...
            if ($request->quotation_generated == 1) {
                ...
                DB::beginTransaction();
                SysCrmQuoteItems::where('deal_id', $scd->id)->where('quote_id', $request->quote_id)->delete();
                $customer = SysCustSuppl::where('id', $request->cust_id)->first();
                //return $customer;
                ...
                if ($request->part_number !=  null && count($request->part_number) > 0) {
                    ...
                    DB::table('sys_crm_deals')->where('id', $scd->id)
                        ->update(['estimated_close_date' => SysHelper::normalizeToYmd($request->estimated_close_date), 'quote_id' => $request->quote_id, 'deal_discount' => (float) str_replace(',', '', $request->deal_discount ?? 0), 'terms_and_condition' => $request->terms_and_condition]);
                    SysHelper::deal_updated_at($scd->id);
    ...
    public function dashboardsalesfilter(Request $request)
    {
        try {
            ...
            if ($company_id == 0) {
                $service =  SysHelper::get_total_sales_revenue_all($date_id, $company_id);
            } else {
                $service =  SysHelper::get_total_sales_revenue($date_id, $company_id);
            }

            $ret = [$service[0], $service[1], $service[2]];
            return json_encode(array('data' => $ret));
        } catch (\Exception $e) {
            return $e;
            $retData = $e;
