...
class SmStaffController extends Controller
{
    ...
    private function normalizeTargetValues($amount, $period)
    {
    ...
    public function getsalespersonlist(Request $request)
    {
        ...
        $query = SmStaff::select('sm_staffs.user_id as id', 'sm_staffs.full_name')
            ->join('sys_cust_suppl_assign', 'sys_cust_suppl_assign.user_id', 'sm_staffs.user_id')
            ->where('sm_staffs.active_status', 1)
            ->where('sys_cust_suppl_assign.cust_supp_id', $request->id)
            ->groupBy('sm_staffs.id', 'sm_staffs.full_name');
    ...
    /**
     * Return list of sales persons for multiple companies (AJAX)
     * Accepts POST param `company_ids` as array or comma-separated string
     */
    public function getSalesPersonsByCompanies(Request $request)
    {
        try {
            $companyIds = $request->input('company_ids', []);
            if (!is_array($companyIds)) {
                $companyIds = explode(',', (string) $companyIds);
            }
            ...
            $data = SmStaff::select('user_id as id', 'full_name')
...
// app/Http/Controllers/StaffController.php
    ...
    public function storeBasic(Request $request)
    {
        ...
        try {
            ...
            $staff->first_name_full = $request->input('first_name');
            $staff->first_name = $this->normalizeFirstName($request->input('first_name'));
            $staff->last_name = $request->input('last_name');
            $staff->full_name = trim(($staff->first_name ?: '') . ' ' . $request->input('last_name'));
           ...
           $staff->company_access =is_array($request->input('company_access')) ? implode(',', $request->input('company_access')) : $request->input('company_access');
            ...
            $staff->reporting_manager = $request->reporting_manager;
            ...
            $user->full_name = $fullName;
            $user->username = $username;
            $user->email = $request->input('company_email') ?: $staff->email;
            ...
            $staff->target_month_from = SysHelper::normalizeToYmd($jobPayload['target_month_from']);
            ...
            $period = strtolower(trim($request->input('target_period', '')));
            if (!array_key_exists($period, $targetPeriods)) {
                $period = 'monthly';
            }
            $targetType = strtolower(trim($request->input('target_type', '')));
            $storeRevenue = in_array($targetType, ['revenue', 'rev', 'both', 'all']);
            $storeGp = in_array($targetType, ['gp', 'gross profit', 'gptarget', 'gp target', 'both', 'all']);
            ...
            $revenueTargetValues = $this->normalizeTargetValues($request->input('revenue_target') ?? 0, $period);
            $gpTargetValues = $this->normalizeTargetValues($request->input('gp_target') ?? 0, $period);

            if ($storeRevenue && $request->filled('revenue_target')) {
                $staff->revenue_target_weekly = $revenueTargetValues['weekly'];
                $staff->revenue_target_monthly = $revenueTargetValues['monthly'];
                $staff->revenue_target_quaterly = $revenueTargetValues['quarterly'];
                $staff->revenue_target_yearly = $revenueTargetValues['yearly'];
                $staff->is_target = 1;
            }
            if ($storeGp && $request->filled('gp_target')) {
                $staff->gp_target_weekly = $gpTargetValues['weekly'];
                $staff->gp_target_monthly = $gpTargetValues['monthly'];
                $staff->gp_target_quaterly = $gpTargetValues['quarterly'];
                $staff->gp_target_yearly = $gpTargetValues['yearly'];
                $staff->is_target = 1;
            }
        ...
        $staff->save();
            ...
            if ($staff->is_target == 1) {
                $combind_user_id = "";
                if ($request->filled('combind_user_id')) {
                    $combind_user_id = implode(',', (array) $request->input('combind_user_id')) . ',' . $staff->user_id;
                }

                $salesTargetPayload = [
                    'user_id' => $staff->user_id,
                    'target' => $revenueTargetValues['monthly'],
                    'target_month' => date('Y-m-01'),
                    'status' => 1,
                    'created_by' => Auth::user()->id,
                    'created_at' => Carbon::now('+04:00'),
                    'company_id' => $staff->main_company,
                    'company_access' => $staff->company_access,
                    'type' => 1,
                    'revenue_target_weekly' => $staff->revenue_target_weekly,
                    'revenue_target_monthly' => $staff->revenue_target_monthly,
                    'revenue_target_quaterly' => $staff->revenue_target_quaterly,
                    'revenue_target_yearly' => $staff->revenue_target_yearly,
                    'gp_target_weekly' => $staff->gp_target_weekly,
                    'gp_target_monthly' => $staff->gp_target_monthly,
                    'gp_target_quaterly' => $staff->gp_target_quaterly,
                    'gp_target_yearly' => $staff->gp_target_yearly,
                    'target_month_from' => $staff->target_month_from,
                    'combind_user_id' => $combind_user_id,
                ];

                $check = SysCrmSalesTarget::where('user_id', $staff->user_id)
                    ->where('target_month_from', $staff->target_month_from)
                    ->get();

                if (count($check) === 0) {
                    SysCrmSalesTarget::insert($salesTargetPayload);
                } else {
                    SysCrmSalesTarget::where('id', $check->max('id'))->update($salesTargetPayload);
                }
            }
            ...
            // joining: support multiple files (e.g., iban_letter, prof_certs, academic, relieving_letter)
            foreach ($docs['joining'] ?? [] as $key => $row) {
                $remarks = $row['remarks'] ?? null;
                $number = $row['number'] ?? null;
                $expiry = $parseDate($row['expiry'] ?? null);
                $origLabel = $row['label'] ?? ucfirst(str_replace('_', ' ', $key));

                // Collect paths and names (handle uploaded files and existing paths). Allow multiple files per key.
                $paths = [];
                $names = [];

                if ($request->hasFile("docs.joining.$key.file")) {
                    $files = $request->file("docs.joining.$key.file");
                    if (!is_array($files)) $files = [$files];
                    foreach ($files as $file) {
                        if ($file && $file->isValid()) {
                            $names[] = $file->getClientOriginalName();
                            $paths[] = $storeFile($file, 'employee/docs/joining');
                        }
                    }
                }

                if (!empty($row['existing'])) {
                    if (is_array($row['existing'])) {
                        foreach ($row['existing'] as $ex) {
                            if ($ex) {
                                $names[] = basename($ex);
                                $paths[] = $ex;
                            }
                        ...
                        $names[] = basename($row['existing']);
                        $paths[] = $row['existing'];
                    }
                }

                // If nothing to save and no remarks, skip
                if (count($paths) === 0 && !$remarks)
                    continue;

                // If there are files/paths, create one DB row per file
                if (count($paths) > 0) {
                    ...
                    // No path but remarks present — persist a row to record the remarks
                    SmStaffDocument::create([
                        'staff_id' => $staff->id,
                        'group' => 'joining',
                        'key' => $key,
    ...
    public function updateBasic(Request $request)
    {
        ...
        try {
            ...
            $jobPayload = [
                'staff_id' => $staff->id,
                'date_of_joining' => $doj,
                'probation_end_date' => $probationFromRequest,
                'department_id' => $request->input('department_id'),
                'designation_id' => $request->input('designation_id'),
                'reporting_manager' => $request->input('reporting_manager'),
                'employment_type' => $request->input('employment_type'),
            ...
            // joining: support multiple files per key (e.g., iban_letter, prof_certs, academic, relieving_letter)
            foreach ($docs['joining'] ?? [] as $key => $row) {
                ...
                // uploaded files (may be single file or array of files)
                if ($request->hasFile("docs.joining.$key.file")) {
                    $files = $request->file("docs.joining.$key.file");
                    if (!is_array($files)) $files = [$files];
                    foreach ($files as $file) {
                ...
                // existing paths (may be single string or array)
                if (!empty($row['existing'])) {
                    if (is_array($row['existing'])) {
                        foreach ($row['existing'] as $ex) {
                            if ($ex) {
                ...
                // nothing to save and no remarks -> skip
                if (count($paths) === 0 && !$remarks)
                    continue;

                // create one DB row per file/path
                if (count($paths) > 0) {
                    foreach ($paths as $idx => $p) {
                        SmStaffDocument::create([
                            'staff_id' => $staff->id,
    ...
    public function getReportingManagersByGrade(Request $request)
    {
        try {
            ...
            $staff = SmStaff::select('id', 'user_id', 'full_name')
                ->where('company_id', session()->get('logged_session_data.company_id'))
                ->where('active_status', 1)
    ...
    public function getSalesPersonListByAjax(Request $request)
    {
        try {
            $companyIds = $request->input('company_ids', []);
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Unable to fetch sales persons'
            ], 500);
        }
    }
