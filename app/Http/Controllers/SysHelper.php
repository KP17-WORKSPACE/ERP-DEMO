...
class SysHelper extends Model
{
    ...
    {
        $staffs = SmStaff::select('user_id','full_name')->orderby('full_name','asc')->get();
        return $staffs;
    }
    ...
    {
        $com_id=session('logged_session_data.company_id');
        $staffs = SmStaff::select('user_id','full_name')->whereRaw("find_in_set($com_id,company_access)")->orderby('full_name','asc')->get();
        return $staffs;
    }
    ...
    {
        $ret=1;
        if($data->status==3){ $ret=0; }
        if($data->vat_country==""){ $ret=0; }
        if($data->vat_percentage==""){ $ret=0; }
        if($data->customer_type != 7){
            if(strlen($data->vat_number) < 5){ $ret=0; }
        }
        if($data->payment_terms=="0"){ $ret=0; }
        if(strlen($data->mobile) < 9){ $ret=0; }
        if(strlen($data->email) < 5){ $ret=0; }
        if(strlen($data->first_name) < 3){ $ret=0; }
        if(strlen($data->contcat_number) < 8){ $ret=0; }

        if($data->transaction_type==""){ $ret=0; }
        if($data->transaction_type=="Credit"){
            if($data->credit_limit=="" || $data->credit_limit=="0.00" || $data->credit_limit=="0"){ $ret=0; }
            if($data->credit_days=="" || $data->credit_days=="0"){ $ret=0; }
        }
        if($data->customer_salutation=="" || $data->customer_salutation=="."){ $ret=0; }
        if($data->is_file=="" || $data->is_file=="0"){ $ret=0; }
    ...
    public static function get_customer_contact_detail($account_code)
    {
        try {
            $ret_val="";
                $custid = DB::table('sys_cust_suppl')->where('code',$account_code)->first();
                $contact = DB::table('sys_cust_suppl_contact')->where('cust_suppl_id',$custid->id)->get();
                //return $contact;
                if(count($contact)>0){
                    foreach($contact as $p)
                    {
                        if(str_contains(strtolower($p->designation),'account')){
                            if($p->mobile==$p->work_phone){
                                $ret_val = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>'.$p->first_name.' '.$p->last_name.'</b> ('.$p->designation.')&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$p->mobile.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$p->email_address;
                            } else {
                                $ret_val = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>'.$p->first_name.' '.$p->last_name.'</b> ('.$p->designation.')&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$p->mobile.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$p->work_phone.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$p->email_address;
                            }
                        }
                    }
                    if($ret_val == ""){
                        if($p->contact[0]==$p->contact[0]){
                            $ret_val = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>'.$contact[0]->first_name.' '.$contact[0]->last_name.'</b> ('.$contact[0]->designation.')&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$contact[0]->mobile.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$contact[0]->email_address;
                        } else{
                            $ret_val = '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>'.$contact[0]->first_name.' '.$contact[0]->last_name.'</b> ('.$contact[0]->designation.')&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$contact[0]->mobile.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$contact[0]->work_phone.'&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;'.$contact[0]->email_address;
                        }
                    }
                }
                else{
                    $ret_val = "Contact details not available";
                }
            return $ret_val;
        } catch (\Throwable $th) {
            return "";
...
// deal track approval access end
    ...
    public static function get_dealid_from_code($deal_code){
        $deal_id =  DB::table('sys_crm_deals')->where('company_id',session('logged_session_data.company_id'))->where('code',$deal_code)->max('id');
        if($deal_id=="") {
            return "0";
        } else {
            return $deal_id;
    ...
    public static function get_code_from_dealid($deal_id){
        $deal_id =  DB::table('sys_crm_deals')->where('company_id',session('logged_session_data.company_id'))->where('id',$deal_id)->max('code');
        if($deal_id=="") {
            return "Without Deal";
        } else {
            return $deal_id;
    ...
    public static function get_dealid_from_code_list($deal_code){
        $dealid=[];
        $newArray =  array_map('intval', explode(',',$deal_code));
        if(count($newArray)>0){
            foreach ($newArray as $key) {
                $dealid[]=array($key);
            }
        }
        $deal_id =  DB::table('sys_crm_deals')->where('company_id',session('logged_session_data.company_id'))->wherein('code',$dealid)->pluck('id')->implode(',');
        if($deal_id=="") {
            return "0";
        } else {
            return $deal_id;
    ...
    public static function lead_type_new($id){
        if($id==1){
            return '<span class="bg-primary btn-small text-white">Reseller</span>';}
        else if($id==2){
            return '<span class="bg-primary btn-small text-white">Enduser</span>';}
        else if($id==3){
            return '<span class="bg-primary btn-small text-white">E-Commerce</span>';}
        else if($id==4){
            return '<span class="btn-xs btn-warning mt-2 pl-2 pr-2">Project</span>';}
        else{
            return '';}
    ...
    public static function deal_type_new($id){
        if($id==1){
            ...
            return '<span class="bg-primary btn-small text-white">E-Commerece</span>';}
        else if($id==4){
            return '<span class="bg-primary btn-small text-white">Project</span>';}
        else{
            return '';}
    }
    ...
    public static function get_total_revenue_all_by_user_nocombind($user_id,$from_month,$to_month,$ps_amc_deal_id){
    ...
    public static function calculateSalesTarget($userId, $startDate, $endDate,$filter_by)
    {
        try {
            ...
            $target = $target_query->whereRaw("DATE_FORMAT(target_month_from, '%Y-%m') <= '".date('Y-m', strtotime($end_date))."'")->orderby('target_month_from','desc')->first();
            ...
            if(isset($target)){
                ...
                $target = SysCrmSalesTarget::select(DB::raw('COALESCE(sum(revenue_target_monthly), 0) as rev_amount'),DB::raw('COALESCE(sum(gp_target_monthly), 0) as gp_amount'))
                ->wherein('user_id', $userId)->whereRaw("DATE_FORMAT(target_month_from, '%Y-%m') <= '".date('Y-m', strtotime($end_date))."'")->orderby('target_month_from','desc')->first();
                return $target;
            }
        } catch (\Throwable $th) {
    ...
    public static function get_total_revenue_all_by_user_sum($user_id,$from_month,$to_month,$company,$ps_amc_deal_id){
        $user=$user_id;
        $data1=[];
        $data2=[];
    ...
    public static function get_total_revenue_actual_all_by_user($user_id,$from_month,$to_month,$company,$ps_amc_deal_id){
    ...
    public static function get_total_on_process_all_by_user($user_id,$from_month,$to_month,$company,$ps_amc_deal_id){
    ...
    public static function get_total_forcast_all_by_user($user_id,$from_month,$to_month,$company_id,$ps_amc_deal_id){
        try {
    ...
    public static function set_deal_profit($deal_id){
        ...
        $quoteitems = SysCrmQuoteItems::select('sys_crm_quote_items.*','sm_items.part_number','sys_brand.title')
            ...
            if(count($purchase_auto)>0){
                $purchase_cost = SysPurchaseOrderItems::select(DB::raw('ifnull(sum(taxableamount),0) as total_cost'))->wherein('po_id',$purchase_auto)->get();
    ...
    public static function get_deal_count_by_user($user,$m1,$m2,$ps_amc_deal_id){
    ...
    {
        if($receivables==1){ return "<span class='success btn-badge py-0 px-1'>Payment Received</span>";}
        else if($receivables==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Payment Rejected</span>";}
        else if($receivables==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Payment Pending</span>";}
        else if($receivables==4){ 
            return "<span class='dark btn-badge py-0 px-1'>Order Cancelled</span>";}
        else if($delivery==1){ 
            return "<span class='success btn-badge py-0 px-1'>Delivery Completed</span>";}
        else if($delivery==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Delivery Rejected</span>";}
        else if($delivery==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Out For Delivery</span>";}
        else if($delivery==4){ 
            return "<span class='primary btn-badge py-0 px-1'>Pending For Delivery</span>";}
        else if($delivery==5){ 
            return "<span class='primary btn-badge py-0 px-1'>Ready For Delivery</span>";}
        else if($invoice==1){ 
            return "<span class='success btn-badge py-0 px-1'>Invoice Approved</span>";}
        else if($invoice==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Invoice Disapproved</span>";}
        else if($invoice==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Invoice Pending</span>";}
        else if($purchease==1){ 
            return "<span class='success btn-badge py-0 px-1'>Purchase Approved</span>";}
        else if($purchease==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Purchase Disapproved</span>";}
        else if($purchease==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Purchase Pending</span>";}
        else if($purchease==4){ 
            return "<span class='primary btn-badge py-0 px-1'>Partial Delivery</span>";}
        else if($sales==1){ 
            return "<span class='success btn-badge py-0 px-1'>Sales Approved</span>";}
        else if($sales==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Sales Disapproved</span>";}
        else if($sales==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Sales Pending</span>";}
        else if($accounts==1){ 
            return "<span class='success btn-badge py-0 px-1'>Accounts Approved</span>";}
        else if($accounts==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Accounts Disapproved</span>";}
        else if($accounts==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Accounts Pending</span>";}
        else if($accounts==0){ 
            return "<span class='success btn-badge py-0 px-1'>Accounts Pending</span>";}
        else if($sales==0){ 
            return "<span class='success btn-badge py-0 px-1'>Sales Pending</span>";}
        else if($purchease==0){ 
            return "<span class='success btn-badge py-0 px-1'>Purchase Pending</span>";}
        else if($invoice==0){ 
            return "<span class='success btn-badge py-0 px-1'>Invoice Pending</span>";}
        else if($delivery==0){ 
            return "<span class='success btn-badge py-0 px-1'>Delivery Pending</span>";}
        else if($receivables==0){ 
            return "<span class='success btn-badge py-0 px-1'>Payment Pending</span>";}
        else { 
            return "<span class='warning btn-badge py-0 px-1'>New</span>";}
    }
    public static function deal_stage($stage)
    {
        if($stage==1){ return "<span class='warning btn-badge py-0 px-1'>Prospecting</span>";}
        else if($stage==2){ 
            return "<span class='info btn-badge py-0 px-1'>Quote</span>";}
        else if($stage==3){ 
            return "<span class='primary btn-badge py-0 px-1'>Closure</span>";}
        else if($stage==4){ 
            return "<span class='success btn-badge py-0 px-1'>Won</span>";}
        else if($stage==1){ 
            return "<span class='danger btn-badge py-0 px-1'>Lost</span>";}
        else if($stage==2){ 
            return "<span class='danger btn-badge py-0 px-1'>Cancel</span>";}
        else { 
            return "<span class='warning btn-badge py-0 px-1'>New</span>";}
    }
...
///////////////////////////////
    ...
    {   //Assets-1 debit || Liabilities-2 credit || Expenses-3 debit || Incomes-4 credit
        ...
        try {
            ...
            $trn2 = SysChartofAccountsTransaction::select('debit_amount','credit_amount')->where('status',1)
            ->where('company_id',$com_id)
            ->wherein('account_id',$account_id)->where('transaction_type','openingbalance')->get();
            //return $trn2;
    ...
    public static function get_trial_balance_items($gid,$from_date,$to_date)
    {
        try {
            $r = SysHelper::get_data_by_role();
            $company_id = $r[0];
            ...
            if($gid==24){
                $data = DB::table('sys_chartofaccounts')->select('id','account_name','group','subgroup2')->wherein('account_name',['Opening Stock'])->where('company_id',session('logged_session_data.company_id'))->get();
            } else {
                $data = DB::table('sys_chartofaccounts')->select('id','account_name','group','subgroup2')->where('subgroup2',$gid)->whereRaw("find_in_set($com_id,sys_chartofaccounts.company_access)")->wherenotin('subgroup2',$subgroup2)->get();
            }
            ...
            if(count($data)>0){
                foreach ($data as $dt) {
                    ...
                    if($total[0] != "0.00" || $total[1] != "0.00"){
                        if($dt->group == 1 || $dt->group == 3){ // 1 Assets, 3 Expenses
                            if($dt->id == $purchase_return_account_id){
                                $sum=$total[0] + $total[1];
                                $retData .= "<tr class='collapse' id='collapse_sub_".$gid."'><td class='border pl-4'>".$dt->account_name."</td><td class='pr-2 border text-right'>0.00</td><td class='pr-2 border text-right'>".SysHelper::minus_format(SysHelper::com_curr_format($total[1], 2, '.', ','))."</td><td class='pr-2 border text-right'>0.00</td><td class='pr-2 border text-right'>".SysHelper::minus_format(SysHelper::com_curr_format($total[0], 2, '.', ','))."</td><td class='pr-2 border text-right'>0.00</td><td class='pr-2 border text-right'>".SysHelper::minus_format(SysHelper::com_curr_format($sum, 2, '.', ','))."</td></tr>";
                            } else {                                
                                $sum=$total[0] + $total[1];
                                $t=0; $c=0; $s=0;
                                if($dt->group==1){
                            ...
                            $retData .= "<tr class='collapse' id='collapse_sub_".$gid."'><td class='border pl-4'>".$dt->account_name."</td><td class='pr-2 border text-right'>".$t."</td><td class='pr-2 border text-right'>0.00</td><td class='pr-2 border text-right'>".$c."</td><td class='pr-2 border text-right'>0.00</td><td class='pr-2 border text-right'>".$s."</td><td class='pr-2 border text-right'>0.00</td></tr>";
                            }
                        }
    ...
    public static function get_account_balance_opening_by_account_id($account_id,$from_date,$to_date,$group)
    {
        try {
            ...
            $tot=0;
            $tot_opn=0;
            $trn = SysChartofAccountsTransaction::select('debit_amount','credit_amount')->where('account_id',$account_id)->wherenotin('transaction_type',['openingbalance','opbinvoice'])
            ...
            $trn_opn1 = SysChartofAccountsTransaction::select('debit_amount','credit_amount')->where('account_id',$account_id)->wherenotin('transaction_type',['openingbalance','opbinvoice','openingstock'])
            ->whereRaw("DATE_FORMAT(transaction_date, '%Y-%m-%d') < '".$from_date."'")->where('status',1)->where('company_id',$com_id)->get();
            
            $trn_opn2 = SysChartofAccountsTransaction::select('debit_amount','credit_amount')->where('account_id',$account_id)->where('transaction_type','openingbalance')->where('status',1)->where('company_id',$com_id)->get();
...
/*
    ...
    public static function get_list_of_unadjusted_include_removed_jv($account_ids,$company)
    {
        try {
                $unadjested_receipt = DB::table('sys_chartofaccounts_transaction as t')->select('t.account_id','c.account_name',
                    't.transaction_no as doc_number','t.transaction_date as doc_date','t.remarks',DB::raw('sum(t.credit_amount-t.debit_amount) AS amount'),
                    DB::raw('COALESCE(SUM(ra.bi_paid), 0) AS adj_amount'),DB::raw('COALESCE(sum(jv.account_amount), 0) AS removed_amount'))
                ->leftJoin('sys_chartofaccounts as c', 'c.id', '=', 't.account_id')
                ->leftJoin('sys_receipt_adjustments as ra', function ($join) {
                        $join->on('ra.bi_doc_number', '=', 't.transaction_no')
                            ->whereColumn('ra.account_id', '=', 't.account_id');
                ...
                ->leftJoin('sys_receipt_adjustments_jv as jv', function ($join) {
                    $join->on(DB::raw('jv.receipt_no COLLATE utf8mb4_general_ci'), '=', DB::raw('t.transaction_no COLLATE utf8mb4_general_ci'));
                })
    ...
    public static function get_list_of_unadjusted_pdc($account_ids,$company)
    {
        try {
            ...
            $unadjested_receipt = DB::table('sys_chartofaccounts_transaction as t')->select('t.account_id','c.account_name',
                't.transaction_no as doc_number','t.transaction_date as doc_date','t.remarks','sys_receipt.cheque_date','sys_receipt.cheque_number','sys_receipt.receipt_date','t.credit_amount AS amount',
                DB::raw('COALESCE(SUM(ra.bi_paid), 0) AS adj_amount'))
            ->leftJoin('sys_chartofaccounts as c', 'c.id', '=', 't.account_id')
            ->leftJoin('sys_receipt_adjustments as ra', function ($join) {
                    $join->on('ra.bi_doc_number', '=', 't.transaction_no')
...
//payable outatsnding end
    ...
    {
        $RET=0;
        $RET = SysHelper::get_aed_amount($currency,$deal_profit);
    ...
    public static function get_lead_filter($date_id, $company_id)
    {
        try {
            ...
            $qualified = $total_leads->where('status',2)->count();
            ...
            $ret = [$new,$qualified,$unqualified,$reseller,$enduser,$ecommerce];
