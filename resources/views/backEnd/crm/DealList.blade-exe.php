...
<div class="card shadow mb-4">
    <div class="card-body">
        <div class="table-responsive">
            ...
            <button class="btn-sm btn-primary" type="submit" style="float: left; left: 220px; position: absolute; z-index: 999;">View All</button>
            {{ Form::close() }}
            ...
            <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                <thead>
                    ...
                    <tr>
                        ...
                        @endif
                        <th style="width: 250px;">@lang('Deal Name')</th>
                        <th>@lang('Customer')</th>
                ...
                <tbody>
                    ...
                    @php $count =1; $total_deal=0; $total_amount=0; $deal_currency="AED"; @endphp
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <?php /*
                                <select class="form-control js-example-basic-single" name="deal_name" id="deal_name">
                                    <option value="" >Select</option>
                                    @foreach ($product as $value)
                                    <option value="{{ @$value->part_number }}" {{ isset($edit) ? (!empty($edit->deal_name) ? (@$edit->deal_name == @$value->part_number ? 'selected' : '') : '') : '' }}>{{ @$value->part_number }}</option>
                                    @endforeach
                                </select>
                                <div id="deal_name_list"></div>                            
                                    <script>
                                        $(document).ready(function(){                                        
                                         $('#deal_name').keyup(function(){ 
                                                ...
                                                if(query != '')
                                                {
                                                 ...
                                                 $.ajax({
                                                  url:"{{ route('autocomplete.fetch_deal_name') }}",
                                                  method:"POST",
                                                  data:{query:query, _token:_token},
                                                  success:function(data){
                                                   $('#deal_name_list').fadeIn();  
                                                            $('#deal_name_list').html(data);
                                                  }
                                                 });
                                                }
                                            });
                                            ...
                                            $(document).on('click', 'li', function(){  
                                                $('#deal_name').val($(this).text());  
                                                $('#deal_name_list').fadeOut();  
                                            });                                        
                                        });
                                        </script>
                                        */ ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                @php
                                $value = date('m-d-Y');
                                if(isset($edit) && $edit->estimated_close_date!="1970-01-01" ){ @$value =
                                date('Y-m-d', strtotime(@$edit->estimated_close_date)); }
                                else{ if(!empty(old('estimated_close_date'))){ @$value = old('estimated_close_date');
                                }else{
