...
@section('mainContent')
    ...
    <aside class="left-nav col-12" id="leftSidebar">
        ...
        <div class="long-list" id="filters-long">
            <div class="d-flex  justify-content-between ">
                <!-- Left: Heading -->
                <h4 class="mb-0">Technical Dashboard</h4>
                <input type="hidden" id="base_url" value="{{ url('/') }}" />

                <div class="search-filter-container mb-0">

                    <a class="btn btn-sm btn-light" href="{{ url('crm-deal-support-list') }}"><i class=""></i> Support
                        Desk</a>
                    <button class="btn btn-sm btn-light" onclick="window.location.reload();"><i class=""></i>
                        Refresh</button>
            ...
            <div class="row">
                ...
                @if (Auth::user()->id == 98)
                                ...
                                <div class="d-inline-block d-flex justify-content-center ">
                                    <div class="text-center mb-2 rounded__box d-flex align-items-center justify-content-center"
                                        ...
                                        <a href="#" onclick="sales_click('revenue')">
                                            ...
                                            <div class="mb-0 font-weight-bold text-gray-800 font-card-large" id="revenue">
                                                {{ $sales[0] }}</div>
                                        </a>
                                    </div>
                                </div>
                                ...
                                <div class="d-inline-block d-flex justify-content-center ">
                                    <div class="text-center mb-2 rounded__box d-flex align-items-center justify-content-center"
                                        ...
                                        <a href="#" onclick="sales_click('forcast')">
                                            <div class="text-xs font-weight-bold text-dark text-uppercase">Forcast</div>
                                            <div class="mb-0 font-weight-bold text-gray-800 font-card-large" id="forcast">
                                                {{ $sales[1] }}</div>
                                        </a>
                                    </div>
                                </div>
                    ...
                    <div class="col-lg-2 mb-4">
                        <div class="card card-fixed-lg icon__card__home shadow ">
                            <div class="card-header py-4">
                ...
                {{--  Jacob George --}}
                <div class="col-lg-4 mb-4">
                    <div class="card card-fixed-lg shadow">
                        <div class="card-header py-4">
                            ...
                            <div class="d-inline-block d-flex justify-content-center ">
                                <div class="text-center mb-2 rounded__box d-flex align-items-center justify-content-center"
                                    ...
                                    <a href="#" onclick="gp_click('service_revenue')">
                                        <div class="text-xs font-weight-bold text-dark text-uppercase">Gross Profit</div>
                                        <div class="mb-0 font-weight-bold text-gray-800 font-card-large"
                                            id="service_revenue">{{ $gp[0] }}</div>
                                    </a>
                                </div>
                            </div>
                                ...
                                <div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <p class="mb-1 font-semibold">
                                            {{ @App\SysHelper::com_curr_format($total_sales, 2, '.', ',') }}AED /
                                            {{ @App\SysHelper::com_curr_format($top->target * 3, 2, '.', ',') }}AED</p>
                                    </div>
                                    ...
                                    <tbody>
                                        @if (count($new_service) > 0)
                                            @foreach ($new_service as $val)
                                                <tr>
                                                    <td><a href="{{ url('crm-deal-service/' . $val->id . '/view') }}"
                                                            title="View Deal Track"
                                                            class="text-dark">{{ $val->id }}</a></td>
                                                    <td>
                                                        <div
                                                    ...
                                                    <td><a href="{{ url('crm-deal-service/' . $val->id . '/view') }}"
                                                            title="View Deal Track" class="text-dark">View</a></td>
                                                </tr>
                                            @endforeach
                                        @endif
                                    </tbody>
                                    ...
                                    <tbody>
                                        @if (count($collaboration) > 0)
                                            @foreach ($collaboration as $val)
                                                <tr>
                                                    <td><a href="{{ url('crm-deal-track/' . $val->id . '/view') }}"
                                                            title="View Deal Track"
                                                            class="text-dark">{{ $val->id }}</a></td>
                                                    <td>
                                                        <div
                                                            style="width:150px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">
                                                            <?php try {?>{{ $val->customername->name }}<?php } catch (\Exception $e) { }?>
                                                        </div>
                                                    </td>
                                    ...
                                    <tbody>
                                        @if (count($new_support) > 0)
                                            @foreach ($new_support as $val)
                                                <tr>
                                                    <td><a href="{{ url('crm-deal-support/' . $val->id . '/view') }}"
                                                            title="View Deal Track"
                                                            class="text-dark">{{ $val->id }}</a></td>
                                                    <td>
                                                        <div
                                                    ...
                                                    <td><a href="{{ url('crm-deal-support/' . $val->id . '/view') }}"
                                                            title="View Task" class="text-dark">View</a></td>
                                                </tr>
                                            @endforeach
                                        @endif
                                    </tbody>
                                        ...
                                        @if (count($new_service_pending) > 0)
                                            @foreach ($new_service_pending as $val)
                                                <tr>
                                                    <td><a href="{{ url('crm-deal-service/' . $val->id . '/view') }}"
                                                            title="View Deal Track"
                                                            class="text-dark">{{ $val->deal_id }}</a></td>
                                                    <td>
                                                        <div
                                                            style="width:150px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">
                                                            <?php try {?>
                        ...
                        <div class="card  max-height">
                            ...
                            <div>
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <p class="mb-1 font-semibold">
                                        {{ @App\SysHelper::com_curr_format($total_sales, 2, '.', ',') }}AED /
                                        {{ @App\SysHelper::com_curr_format(500000, 2, '.', ',') }}AED</p>
                                </div>
                                <div class="progress mb-3">
                                    <div class="progress-bar {{ $tpcolor }}" style="width:{{ $tp }}%">
                                        {{ $tp }}%</div>
                                </div>
                            </div>
                                            ...
                                            @if (count($amc_exp) > 0)
                                                @foreach ($amc_exp as $top)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            @if (@$top->to_date != null)
                                                                {{ date('d/m/Y', strtotime($top->to_date)) }}
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @endforeach
    ...
    <a data-bs-toggle="modal" data-bs-target="#exampleModalCenter" id="btn_model"></a>
    <!-- Modal -->
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content" style="background: #4e73de;">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLongTitle" style="color: #ffffff;">Hi
                        {{ session('logged_session_data.full_name') }}</h5>
