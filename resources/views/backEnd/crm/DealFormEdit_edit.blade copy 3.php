...
{{-- 
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" id="device_serial" readonly >
            ...
            <table class="table table-hover form-item-table" id="myTable">
                <thead>
                    <tr>
                        ...
                        <th class="resizable text-start" width="250px">@lang('Part No') <a
                                class="icon icon-outline-book text-dark" data-bs-toggle="modal"
                                data-bs-target="#addproductModal"></a>
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-start" width="250px">@lang('Description')
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="65px">@lang('Cost')
                            <div class="resizer"></div>
                        </th>
                        ...
                        <th class="resizable text-end" width="100px">@lang('Price')
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="100px">@lang('Value')
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="80px" scope="col">Dis <a
                                class="icon icon-outline-book text-dark" data-bs-toggle="modal"
                                data-bs-target="#discountModal"></a>
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="100px">@lang('Taxable')
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="100px">@lang('VAT')
                            <div class="resizer"></div>
                        </th>
                        <th class="resizable text-end" width="100px">@lang('Total')
                            <div class="resizer"></div>
                        </th>
                    </tr>
                </thead>
                    ...
                    <div class="row">
                        ...
                        <div class="col">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control" name="device_serial" id="device_serial" readonly style="cursor:pointer;" />
                                    <button type="button" class="btn btn-light border" >
                                        <i class="ico icon-outline-list-down"></i>
                                    </button>
                                </div>
...
<!-- Hidden inputs to store serial numbers per product (JSON format) -->
@foreach ($quotationitems as $item)
    @if($item->product_type == 2)
        ...
        @endphp
        <input type="hidden" 
               id="serial_data_{{ $item->product_id }}" 
               name="serial_numbers[{{ $item->product_id }}]" 
               class="serial-data-storage" 
               data-product-id="{{ $item->product_id }}"
               data-part-number="{{ $item->productname->part_number }}"
               data-qty="{{ $item->qty }}"
...
@if (isset($cart) && count($cart) > 0)
    @foreach ($cart as $cart_items)
        @php
            $cartProduct = \App\SmInventoryItem::find($cart_items->product_id);
            $cartProductType = $cartProduct ? $cartProduct->product_type : null;
        @endphp
        @if($cartProductType == 2)
            <input type="hidden" 
                   id="serial_data_{{ $cart_items->product_id }}" 
                   name="serial_numbers[{{ $cart_items->product_id }}]" 
                   class="serial-data-storage" 
                   data-product-id="{{ $cart_items->product_id }}"
                   data-part-number="{{ $cart_items->partnumber }}"
...
$(document).ready(function() {
    ...
    function populateSerialModal() {
        ...
        $('.serial-data-storage').each(function() {
            ...
            var sectionHtml = `
                <div class="part-serial-section" data-product-id="${productId}">
                    <div class="part-serial-header">
                        <div>
                            <div class="part-name">${partNumber}</div>
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2">
                        <button type="button" class="btn btn-sm btn-light border add-serial-btn">
                            <i class="ico icon-outline-add-square text-success"></i> Add Serial
                        </button>
            ...
            var $section = $container.find(`[data-product-id="${productId}"]`);
            var $inputsList = $section.find('.serial-inputs-list');
            
            // Load existing serials or create empty fields
            if (existingData) {
                loadSerialsForPart($inputsList, existingData, productId);
            } else {
                // Create qty number of empty fields
                for (var i = 1; i <= qty; i++) {
                    addSerialInput($inputsList, i, productId, '');
                }
            }
            
            updateSerialCount(productId);
        });
    }
    
    function loadSerialsForPart($list, data, productId) {
        try {
            var serials = JSON.parse(data);
            if (Array.isArray(serials)) {
                serials.forEach(function(serial, index) {
                    addSerialInput($list, index + 1, productId, serial);
                });
            } else {
            ...
            var serials = data.split(',').map(function(s) { return s.trim(); }).filter(function(s) { return s !== ''; });
            serials.forEach(function(serial, index) {
                addSerialInput($list, index + 1, productId, serial);
            });
        }
    ...
    function addSerialInput($list, index, productId, value) {
        value = value || '';
        var html = `
            <div class="serial-input-row" data-index="${index}">
                <span class="me-2 text-muted">${index}.</span>
                <input type="text" 
    ...
    $(document).on('click', '.add-serial-btn', function() {
        ...
        addSerialInput($list, currentCount + 1, productId, '');
    ...
    $(document).on('keydown', '.part-serial-input', function(e) {
        if (e.key === 'Enter') {
            ...
            if ($nextInput.length) {
                ...
                // Add new if at last
                var $section = $(this).closest('.part-serial-section');
                $section.find('.add-serial-btn').trigger('click');
            }
        }
    });
    
    function updateSerialCount(productId) {
        var $section = $(`[data-product-id="${productId}"]`);
    ...
    $(document).on('click', '#btn_save_all_serials', function() {
        $('.part-serial-section').each(function() {
            ...
            $('#serial_data_' + productId).val(JSON.stringify(serials));
        });
        ...
        $('#device_serial').val(allSerials.join(', '));
    ...
    $(document).on('change', 'input[name="qty[]"]', function() {
        ...
        qtyChangeTimeout = setTimeout(function() {
            ...
            if ($select.length) {
                ...
                var $storage = $('#serial_data_' + productId);
                if ($storage.length) {
