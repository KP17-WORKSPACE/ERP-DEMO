        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="white-box leadbox pt-3" >
                    <select class="float-right dynamicstxt" id="filter_date" style="width:150px !important;">
                        <option value="d">Day</option>
                        <option value="e">Yesterday</option>
                        <option value="w">This Week</option>
                        <option value="m">Monthly</option>
                        <option value="q">Quarterly</option>
                        <option value="y">Yearly</option>
                    </select>
                    <h6>Leads & Deals this Month</h6>
                    <hr />
<br />
                    <div class="row">
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #28a745; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_leads }}</b></div>
                            <b>Total Leads</b>
                        </div>
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #17a2b8; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_deals }}</b></div>
                            <b>Total Deals</b>
                        </div>
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #ffc107; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_deals_prospecting }}</b></div>
                            <b>New Deals</b>
                        </div>
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #6f42c1; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_deals_closure }}</b></div>
                            <b>Quote</b>
                        </div>
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #007bff; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_deals_won }}</b></div>
                            <b>Won</b>
                        </div>
                        <div class="col-lg-2 text-center">
                            <div class="rounded-circle" style="font-size:15px; border: solid 15px #dc3545; width: 100px; height: 100px; line-height: 70px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $total_deals_lost }}</b></div>
                            <b>Lost</b>
                        </div>
                    </div>
                    <br />
        ...
        <div class="row">
            ...
            <div class="col-lg-3 text-center">
                <div class="white-box leadbox text-xs pt-3"  style="height: 350px;">
                    <h6 class="text-left">Target This Month</h6>
                    <hr style="margin: 0px -15px 5px -15px;" />

                    @if(count($sales_target_indu)>0)
                    <?php $team_total_sales=0; $team_target=0; ?>
                    @foreach ($sales_target_indu as $top)
                    <?php $total_sales = App\SysHelper::get_total_sales_brand($top->user_id,$top->brand);
                        $tp = round($total_sales / $top->target * 100,0);
                        $tpcolor="#dc3545";
                        if($tp<40){$tpcolor="#dc3545";}
                        if($tp>=40 && $tp<80){$tpcolor="#ffc107";}
                        if($tp>=80 && $tp<=100){$tpcolor="#28a745";}
                        if($tp>100){$tpcolor="#007bff";}
                        ?><br /><br />
                    <div class="rounded-circle" style="font-size:15px; border: solid 20px {{ $tpcolor }}; width: 150px; height: 150px; line-height: 110px; text-align: center; vertical-align: middle; margin: auto;"><b>{{ $tp }}%</b></div><br />
                            <b>{{ @App\SysHelper::com_curr_format($total_sales, 2, '.', '') }}AED</b> / {{ $top->target }}AED</b>
                            @endforeach
            ...
            <div class="col-lg-6">
                <div class="white-box leadbox text-sm pt-3 pl-0 pr-0 pb-0" style="height: 350px; overflow-y: scroll;">
                    ...
                    <tr>
                        <td class="text-xs"><a href="{{url('crm-deal-track/'.$top->deal_id.'/view')}}" title="View Deal Track" class="text-xs">{{ $top->deal_id }}</a></td>
                        <td class="text-xs"><?php try {?>{{ $top->customername->name }}<?php } catch (\Exception $e) { }?></td>
                        <td class="text-xs"><?php try {?>{{ $top->ownername->full_name }}<?php } catch (\Exception $e) { }?></td>
                        <td class="text-xs">{{ date('d/m/Y', strtotime($top->created_at)) }}</td>
                        <td class="text-xs">                            
        ...
        <div class="row" style="display: none;">
            <div class="col-lg-4">
                <div class="white-box leadbox pt-3" style="height: 350px;">
                    <h6>Qualified @if(Auth::user()->id == 27 || Auth::user()->id == 33 || Auth::user()->id == 44)Team @endif Leads By Month</h6>
                    <hr style="margin: 0px -15px 5px -15px;" />
                    {{--  <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.9.4/Chart.js"></script>
            ...
            <div class="col-lg-4">
                <div class="white-box leadbox pt-3">
                    <h6>@if(Auth::user()->id == 27 || Auth::user()->id == 33 || Auth::user()->id == 44)Team @endif Deals By Month</h6>
                    <hr style="margin: 0px -15px 5px -15px;" />
        ...
        <div class="row" style="display: none;">
            <div class="col-lg-8">
                <div class="white-box leadbox text-sm pt-3 pl-0 pr-0 pb-0" style="height: 350px; overflow-y: scroll;">
                    <h6 class="pl-2">Deals by Month</h6>
                    <table id="table_custom" class="display school-table text-xs" cellspacing="0" width="100%">
                        <tr>
                            <td>Deal ID</td>
                            <td>Company</td>
                    ...
                    @foreach ($onprocess_deals as $top)
                    <tr>
                        <td class="text-xs"><a href="{{url('crm-deals/'.$top->id.'/view')}}" title="View Deal Track" class="text-xs">{{ $top->id }}</a></td>
                        <td class="text-xs"><?php try {?>{{ $top->customername->name }}<?php } catch (\Exception $e) { }?></td>
                        <td class="text-xs"><?php try {?>{{ $top->ownername->full_name }}<?php } catch (\Exception $e) { }?></td>
                        <td class="text-xs">{{ date('d/m/Y', strtotime($top->created_at)) }}</td>
                        <td class="text-xs">                            
