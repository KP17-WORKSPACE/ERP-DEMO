...
<?php try { ?>
    <div class="container-fluid">
        <div class="d-sm-flex justify-content-between">
            <div class="mb-3">
                <h2 class="page-heading m-0">AMC Table Requested List</h2>
                <span class="page-label">Home - AMC Reqested Table List</span>
            </div>
            <div>
                <!-- <a href="{{ url('crm-amc-form') }}" class="btn btn-info" type="button">Add New AMC</a> -->
                <a data-id="{{@$value->id}}" class=" btn btn-primary" style="cursor: pointer;" data-toggle="modal" data-target="#ModalDealAdd" title="Click to Fullfill">
                                        Add </a>
                <a type="button" href="{{ url('crm-amc-track-list') }}" class="btn btn-primary"><i class="fa fa-filter mr-1"></i>AMC List</a>
                    ...
                    <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <!-- <td>{{date('d-M-Y', strtotime(@$value->contact_person))}}</td>
                                <td>{{date('d-M-Y', strtotime(@$value->mobile))}}</td> -->

                                <td>{{@$value->contact_person}}</td>
                                <td>{{@$value->mobile}}</td>
                                <td>{{@$value->scope_work}}</td>

                                <td>{{date('d-M-Y', strtotime(@$value->service_date_time))}}</td>
                                <td>{{@$value->source}}</td>
                                <td>{{@$value->engineername->full_name}}</td>
                                ...
                                <td><input type="hidden" id="cid[]" value="{{@$value->id}}">
                                    ...
                                    <input type="hidden" name="copy_url[]" id="copy_url[]" class="copy_url" value="<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['SCRIPT_NAME'] . '/crm-amc-forms/' ?>" />
                                    <a data-id12="<?php echo $value->id; ?>" class="btn-badge btn btn-info py-1 px-2 cpblk" style="cursor: pointer;" id="copy-button"> Copy Url </a>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.crmajax').delay( 0 ).click(function() {
            ...
            $('#engineer1').click(function() {
                //id = $(this).data('eng')
                id = document.getElementById('engineer1').value;
                //alert('id is'+id)


                var action = "crm-amc-track-id2";
                $.ajax({
                    url: action,
                    type: "post",
                    data: {
...
<div class="modal fade" id="ModalDealAdd" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" style="min-width:50% !important;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Add AMC</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>

            {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'crm-amc-track-add','method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'crm-amc-track-submit123']) }}
                            ...
                            <div class="col">
                                <div class="input-effect">
                                    ...
                                    <!-- <input type="hidden" name="amcid" id="order-id"> -->
                                     <input class="form-control"  type="number" required name="deal_id" id="deal_id">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col">
                                <div class="input-effect">
                                    <label class="txtlbl">@lang('Customer Name')<span></span></label>
                                     ...
                                     <input class="form-control" id="sales_pe" type="text" required name="cust_name">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="ModalDealTrack" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" style="min-width:50% !important;">
        <div class="modal-content">
            ...
            {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'crm-amc-track-submit','method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'crm-amc-track-submit123']) }}
...
<div class="modal fade" id="ModalDealTrackEdit" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document" style="min-width:50% !important;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Edit AMC Requested</h5>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
                        ...
                        <div class="no-gutters input-right-icon">
                            <div class="col">
                                <div class="input-effect">
                                    ...
                                    <input class="form-control" id="amount1" type="text" required name="amount">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="no-gutters input-right-icon">
                            <div class="col">
                                <div class="input-effect">
                                    ...
                                    <select class="engs"  style="display: block;
  width: 100%;
  height: 35px;
...
" class="form-control engs  js-example-basic-single">
