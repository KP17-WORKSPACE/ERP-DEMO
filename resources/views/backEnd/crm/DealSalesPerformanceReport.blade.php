...
<div class="container-fluid">
    <div class="d-sm-flex justify-content-between">
        <div class="mb-3">
            <h2 class="page-heading m-0">Sales Performance Report @if ($id=="revenue")
                (Revenue)
            @else
                (Forcast)
            @endif</h2>
            <span class="page-label">Home - Sales Performance Report</span>
        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                                    <thead>
                                        ...
                                        <tr>
                                            ...
                                            <th>@lang('Company')</th>
                                            <th>@lang('Owner')</th>
                                            <th style="width: 100px;">@lang('Closing Date')</th>
                                            <th style="width: 100px;" class="text-right">@lang('Deal Value')</th>
                                            <th style="width: 100px;" class="text-right">Deal Detail</th>
                                        </tr>
                                    </thead>
                                ...
                                <tr>
                                    ...
                                    <td class="text-right">
                                        ...
                                        @php $deal_value= App\SysHelper::get_aed_amount($value->deal_currency,$value->deal_value); @endphp

                                        @if($id=='revenue')
                                        
                                        <?php /*
                                        @if(in_array($value->dealid, [8690,8660]))
                                            @php $tot = $deal_value; @endphp
                                        @elseif($value->source=="Fulfillment")
                                            @php $tot = $deal_value*20/100; @endphp
                                        @elseif(in_array($value->cust_id, [2568,4258,4382,5322,7347,8144,8145,8146,3711,4089,8142]))
                                            @php $tot = $deal_value*20/100; @endphp
                                        @elseif(in_array($value->cust_id, [8866]))
                                            @php $tot = $deal_value*30/100; @endphp
                                        @else
                                            @php $tot = $deal_value; @endphp
                                        @endif
                                        */ ?>

                                        @php $tot=@App\SysHelper::get_deal_value($value->deal_value,$value->source,$value->deal_currency,$value->deal_percent,$value->cust_id); @endphp
                                    ...
                                    <td class="text-right">
                                            <a target="_blank" href="{{url('crm-deals/'.$value->id.'/view')}}" class="btn btn-info pt-0 pb-0">View</a>
                                    </td>
