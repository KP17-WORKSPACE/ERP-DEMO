<div class="modal-header">
    <h4 class="modal-title" id="po-doc-number">{{ $dn->doc_number }}</h4>

    <div class="d-flex align-items-center gap-2">

        <a href="{{ url('delivery-note/' . $dn->id . '?di_action=edit') }}" class="btn btn-light text-dark"
        ...
        <a href="{{ url('delivery-note/' . $dn->id . '?di_action=add') }}" class="btn btn-light text-dark"
            style="font-size:12px;padding:0px 7px !important;min-height:19px">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu" style="">
                ...
                <li><a class="dropdown-item d-flex align-items-center" href="{{ url('delivery-note/' . $dn->id . '/delete') }}"><i
                            class="ico icon-outline-trash-bin-minimalistic text-danger title-15 me-2"></i> Cancel DN</a></li>
                @if (in_array(session('logged_session_data.company_id') ?? null, [2, 3, 5, 6]))
                    <li><a class="dropdown-item d-flex align-items-center" href="{{ url('delivery-note/' . $dn->id . '/download/t') }}"><i
                                class="ico icon-outline-document-medicine text-success title-15 me-2"></i> Download</a></li>
                @else
                    <li><a class="dropdown-item d-flex align-items-center" href="{{ url('delivery-note/' . $dn->id . '/download') }}"><i
                                class="ico icon-outline-document-medicine text-success title-15 me-2"></i> Download</a></li>
                @endif
          ...
          {{@$ship_address2}}, {{@$delivery_city}}<br>  --}}
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0" class=""
                            ...
                            <tr>
                                ...
                                <td width="20%" style="line-height: 12px; font-weight:bold;">Delivery Note Date
                                </td>
                            ...
                            <tr>
                                ...
                                <td style="line-height: 12px;">{{ $dn->payment_terms->title }}
                                    {{ $dn->payment_terms2 }}</td>
                            </tr>
                        </table>
                        <br />
                        ...
                        @if (count($dn_item) > 0)
                            @foreach ($dn_item as $item)
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tr>
                                        <td style="width: 10px; border-bottom: solid 1px #2c2b6d;">{{ $i }}
                                            <?php $i++; ?></td>
                                        <td style="width: 550px; border-bottom: solid 1px #2c2b6d; font-size: 10px;">
                                            ...
                                            <span
                                                style="width:auto; font-size: 8.5px; padding: 2px; margin-top:5px; font-weight: bold; font-style: italic;">{{ preg_replace('/\s*,\s*/', ', ', $item->serial_no) }}</span>
                                        </td>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td style="border-bottom: solid 1px #2c2b6d; font-weight: bold;" colspan="2"> Note:
                                    Goods Received in Good Condition <span class="text-right">Total</span></td>
                                <td style="border-bottom: solid 1px #2c2b6d; text-align: center; font-weight: bold;">
                                    {{ $qty }}</td>
                            </tr>
                        </table>
                        <br /><br />
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td style="border: none; width:35%; line-height: 20px;" align="left"
                                    valign="top"><b class="bottom_b">Received By:</b><br>
                                    <b class="bottom_b">Name:</b><br>
                                    <b class="bottom_b">Phone:</b><br>
                                    <b class="bottom_b">Signature and stamp:</b>
                                </td>
                                <td style="border: none; width:30%; line-height: 20px;" align="center"
                                    valign="bottom">{{ @$si->createdby->full_name }}<br /><b class="bottom_b"
                                        style="font-size: 10px;">Prepared By</b>
                                </td>
                                <td style="border: none; width:35%; line-height: 20px;" align="right"
                                    valign="top">
                                    <b class="bottom_b" style="font-size: 10px;">For
                                        {!! str_replace(
