                                    ...
                                    @foreach ($account as $a)
                                        <li class="list-group-item accounts d-flex justify-content-between align-items-center @if ($a->status != 1) inactive @endif"
                                            ...
                                            <span class="truncate-text-custom"> 
                                               @if (\Illuminate\Support\Str::startsWith($a->account_code, 'CUS'))
                                                    @if(@App\SysHelper::getCompanyCodeSettings()['is_customer_code'])
                                                        {{ $a->account_code }} 
                                                    @endif
                                                   {{ $a->account_name }}
                                                @elseif(\Illuminate\Support\Str::startsWith($a->account_code, 'SUP'))
                                                        @if(@App\SysHelper::getCompanyCodeSettings()['is_supplier_code'])
                                                        {{ $a->account_code }} 
                                                        @endif    
                                                        {{ $a->account_name }}
                                                @elseif(\Illuminate\Support\Str::startsWith($a->account_code, 'ACC'))
                                                        @if(@App\SysHelper::getCompanyCodeSettings()['is_account_code'])
                                                        {{ $a->account_code }} 
                                                        @endif    
                                                        {{ $a->account_name }}
                                                @elseif(\Illuminate\Support\Str::startsWith($a->account_code, 'ACC'))
                                                        @if(@App\SysHelper::getCompanyCodeSettings()['is_subaccount_code'])
                                                        {{ $a->account_code }} 
                                                        @endif    
                                                        {{ $a->account_name }}
                                                @else
                            ...
                            data-bs-placement="top"
                                    ...
                                    <ul class="list-group">
                                        @foreach ($subacc as $sa)
                                            <li class="list-group-item d-flex justify-content-between align-items-center @if ($sa->status != 1) inactive @endif">
