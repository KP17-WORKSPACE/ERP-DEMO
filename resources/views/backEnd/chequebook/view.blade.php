    ...
    $permissions = App\SmRolePermission::where('role_id', auth()->user()->role_id)->get();
        ...
        <style>
            ...
            #masterSearchDropdown {
                ...
                box-shadow: 0 8px 32px rgba(9,44,76,0.13);
                font-family: "Public Sans", sans-serif;
                font-size: 12.5px;
                overflow: hidden;
                width: min(90vw, 1040px);
            }
            #masterSearchDropdown .ms-section-header {
            ...
            #masterSearchDropdown .ms-section-header + div { border-top: none; }
            #masterSearchDropdown .ms-drop-table {
                width: 100%;
                border-collapse: collapse;
                font-size: 12px;
            }
            #masterSearchDropdown .ms-drop-table thead tr {
                background: #fafafa;
                border-bottom: 1px solid #e0e0e0;
            }
            #masterSearchDropdown .ms-drop-table thead th {
                padding: 5px 10px;
                font-size: 10.5px;
                font-weight: 600;
                color: #828282;
            ...
            #masterSearchDropdown .ms-drop-table tbody tr {
                border-bottom: 1px solid #f1f1f1;
                cursor: pointer;
                transition: background 0.1s;
            }
            #masterSearchDropdown .ms-drop-table tbody tr:hover {
                background: #eef2ff;
            }
            #masterSearchDropdown .ms-drop-table td {
                padding: 6px 10px;
                white-space: nowrap;
                color: #262c2e;
            }
            #masterSearchDropdown .ms-drop-table td.muted { color: #828282; }
            #masterSearchDropdown .ms-drop-table td.bold { font-weight: 600; }
            #masterSearchDropdown .ms-drop-table td.amt {
                text-align: right;
                font-variant-numeric: tabular-nums;
                font-weight: 600;
            }
            #masterSearchDropdown .ms-drop-table td.center { text-align: center; }
            #masterSearchDropdown .ms-drop-table td a {
                color: #1f468d;
                text-decoration: none;
            }
            #masterSearchDropdown .ms-drop-table td a:hover { text-decoration: underline; }
            #masterSearchDropdown .ms-drop-table td.remarks {
                max-width: 200px;
                overflow: hidden;
                text-overflow: ellipsis;
                color: #65666d;
            ...
            #masterSearchWrapper .input-group-text {
                height: 25px !important;
                        ...
                        <div class="card-body">
                            <form class="form-horizontal" method="POST" action="{{ url('chequebook') }}" enctype="multipart/form-data">
                                ...
                                <div class="col-md-4 mb-2">
                                    <label for="" class="form-check-label">Master Search <span class="text-muted" style="font-size:11px;">(chequebooks &amp; cheques)</span></label>
                                    <div id="masterSearchWrapper">
                                        <div class="input-group">
                                            <input type="text" id="tableSearch_custom" class="form-control" placeholder="Search..." autocomplete="off">
                                            <!-- <span class="input-group-text" id="masterSearchClear" style="cursor:pointer; display:none;"><i class="ico icon-outline-close-circle" style="font-size:14px;"></i>&nbsp;Clear</span> -->
                                        </div>
                                    </div>
                                    <div id="masterSearchStatus" class="text-muted mt-1" style="font-size:11px; min-height:16px;"></div>
                                </div>
                            </div>
                            </form>
                        </div>
        ...
        {{-- Master Search Dropdown (portalled to body to avoid overflow clipping) --}}
        <div id="masterSearchDropdown">
            <div id="msDropLoading"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>&nbsp;Searching...</div>
            <div id="msDropEmpty">No results found.</div>
            <div id="msDropChequebooksSection" style="display:none;">
                <div class="ms-section-header"><span>Chequebooks</span><span id="msCbCount"></span></div>
                <div style="overflow-x:auto;">
                    <table class="ms-drop-table">
                        <thead><tr>
                            <th>Doc No</th>
                            <th>Bank</th>
                            <th>Range</th>
                            ...
                            <th class="center">View Cheques</th>
                        </tr></thead>
                        <tbody id="msDropChequebooksList"></tbody>
                    </table>
                </div>
            </div>
            <div id="msDropChequesSection" style="display:none;">
                <div class="ms-section-header"><span>Cheques</span><span id="msChCount"></span></div>
                <div style="max-height:340px; overflow-y:auto; overflow-x:auto;">
                    <table class="ms-drop-table">
                        <thead><tr>
                            ...
                            <th class="center">Payment Doc</th>
                            <th class="center">Deal ID</th>
                            <th>Created By</th>
                            <th class="center">Print</th>
                            <th class="center"><i class="ico icon-outline-paperclip" style="font-size:13px;"></i></th>
                        </tr></thead>
                        <tbody id="msDropChequesList"></tbody>
                    </table>
                </div>
            </div>
            <div id="msDropFooter"><kbd style="font-size:10px;padding:1px 5px;background:#eee;border:1px solid #d9d9d9;border-radius:3px;">Esc</kbd> to close</div>
        </div>

        <script>
        $(document).ready(function () {
            var masterSearchTimer = null;
            var $dd = $('#masterSearchDropdown');

            // Portal both the dropdown and attachment modal to <body> so their z-index
            // is resolved in the same stacking context with no ancestor interference.
            $('body').append($dd);
            $('body').append($('#chequePaymentAttachmentsModal'));

            function positionDropdown() {
                var rect = document.getElementById('tableSearch_custom').getBoundingClientRect();
                var vpW  = window.innerWidth;
                var ddW  = Math.min(1040, vpW - 24);  // max 1040px, min margin 12px each side
                var left = rect.left;
                // If it would overflow right edge, shift left
                if (left + ddW > vpW - 12) {
                    left = Math.max(12, vpW - ddW - 12);
                }
                $dd.css({
            ...
            $(window).on('scroll.masterSearch resize.masterSearch', function () {
                if ($dd.is(':visible')) positionDropdown();
            });

            // Close on outside click
            $(document).on('click.masterSearch', function (e) {
                if (!$(e.target).closest('#masterSearchWrapper, #masterSearchClear, #masterSearchDropdown, #chequePaymentAttachmentsModal').length) {
                    hideDropdown();
                }
            });
            ...
            $('#masterSearchClear').on('click', function () {
                $('#tableSearch_custom').val('').trigger('input').focus();
            });

            // ── helpers ──
            function escHtml(str) { return $('<div>').text(str || '').html(); }

            function formatCurrency_2(value) {
                if (value === null || value === undefined || value === '') return '—';
                var normalized = String(value).replace(/,/g, '').trim();
                var n = parseFloat(normalized);
                if (Number.isNaN(n)) return escHtml(String(value));
                return n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            }

            function statusBadge(status) {
                var cfg = {
                    4: ['Issued',    '#dbeafe','#1d4ed8'],
                    2: ['Cleared',   '#d1fae5','#065f46'],
                    1: ['Cancelled', '#fee2e2','#b91c1c'],
                    3: ['Missed',    '#fef3c7','#92400e']
                };
                var s = cfg[status];
                if (!s) return '<span class="ms-status-badge" style="background:#f3f4f6;color:#6b7280;">—</span>';
                return '<span class="ms-status-badge" style="background:' + s[1] + ';color:' + s[2] + ';">' + s[0] + '</span>';
            }

            function cbActiveBadge(deleted) {
                return deleted
                    ? '<span class="ms-status-badge" style="background:#fee2e2;color:#b91c1c;">Deleted</span>'
                    : '<span class="ms-status-badge" style="background:#d1fae5;color:#065f46;">Active</span>';
            }

            // ── input handler ──
            $('#tableSearch_custom').on('input', function () {
                var term = $(this).val().trim();
                $('#masterSearchClear').toggle(term.length > 0);

                try {
                    $('#long-list').DataTable().search(term).draw();
                } catch (e) {
                    $('#long-list tbody tr').each(function () {
                        if (!term) { $(this).show(); return; }
                        $(this).toggle($(this).text().toLowerCase().indexOf(term.toLowerCase()) !== -1);
                    });
                }
                ...
                if (term.length < 2) {
                    hideDropdown();
                    $('#masterSearchStatus').text(term.length === 1 ? 'Type at least 2 characters...' : '');
                    return;
                }

                $('#msDropLoading').css('display', 'flex');
                $('#msDropEmpty, #msDropChequebooksSection, #msDropChequesSection, #msDropFooter').hide();
                showDropdown();
                $('#masterSearchStatus').text('');

                masterSearchTimer = setTimeout(function () { doMasterSearch(term); }, 350);
            });

            // ── AJAX ──
            function doMasterSearch(term) {
                $.ajax({
                    url: '{{ url("api/chequebook-global-search") }}',
                    method: 'GET',
                    data: { q: term },
                    success: function (res) {
                        $('#msDropLoading').hide();
                        if (!res.success) { $('#msDropEmpty').text('Search error.').show(); showDropdown(); return; }

                        var cbs     = res.chequebooks || [];
                        var cheques = res.cheques || [];

                        if (!cbs.length && !cheques.length) {
                            $('#msDropEmpty').show();
                            $('#msDropChequebooksSection, #msDropChequesSection, #msDropFooter').hide();
                            showDropdown();
                            $('#masterSearchStatus').text('No results.');
                            return;
                        }

                        // Chequebooks
                        if (cbs.length) {
                            var cbRows = cbs.map(function (cb) {
                                var range = (cb.start_no && cb.end_no)
                                    ? '<span style="font-variant-numeric:tabular-nums;">' + escHtml(cb.start_no) + ' – ' + escHtml(cb.end_no) + '</span>'
                                    : '<span class="muted">—</span>';
                                return '<tr class="ms-drop-row ms-drop-cb-row" data-id="' + escHtml(cb.id) + '">' +
                                    '<td class="bold">' + escHtml(cb.doc_number) + '</td>' +
                                    '<td>' + escHtml(cb.bank_name || '—') + '</td>' +
                                    '<td>' + range + '</td>' +
                                    '<td class="remarks">' + escHtml(cb.remarks || '—') + '</td>' +
                                    '<td class="text-center d-flex justify-content-center">' + cbActiveBadge(cb.deleted_at) + '</td>' +
                                    '<td class="text-center"><button type="button" class="btn btn-sm btn-light ms-drop-cb-row" data-id="' + escHtml(cb.id) + '" title="View used cheques" style="padding:2px 8px;"><i class="ico icon-outline-eye" style="font-size:13px;"></i></button></td>' +
                                    '</tr>';
                            }).join('');
                            $('#msDropChequebooksList').html(cbRows);
                            $('#msCbCount').text(cbs.length + ' result' + (cbs.length > 1 ? 's' : ''));
                            $('#msDropChequebooksSection').show();
                        } else {
                            $('#msDropChequebooksSection').hide();
                        }

                        // Cheques
                        if (cheques.length) {
                            var chRows = cheques.map(function (c) {
                                var payDoc = c.payment_doc
                                    ? '<a href="{{ url("get-url-payment") }}/' + encodeURIComponent(c.payment_doc) + '" target="_blank">' + escHtml(c.payment_doc) + '</a>'
                                    : '<span class="muted">—</span>';
                                var deal = c.deal_id
                                    ? '<a href="{{ url("crm-deals/show") }}/' + encodeURIComponent(c.deal_int_id) + '" target="_blank">' + escHtml(c.deal_id) + '</a>'
                                    : '<span class="muted">—</span>';
                                return '<tr class="ms-drop-row ms-drop-cheque-row" data-chequebook-id="' + escHtml(c.cheque_id) + '">' +
                                    '<td class="bold">'   + escHtml(c.cheque_number  || '—') + '</td>' +
                                    '<td>'                + escHtml(c.chequebook_doc || '—') + '</td>' +
                                    '<td class="center">' + statusBadge(c.cheque_status)     + '</td>' +
                                    '<td class="center">' + escHtml(c.doc_number     || '—') + '</td>' +
                                    '<td class="center muted">' + escHtml(c.doc_date || '—') + '</td>' +
                                    '<td>'                + escHtml(c.bank_name      || '—') + '</td>' +
                                    '<td class="center muted">' + escHtml(c.cheque_date || '—') + '</td>' +
                                    '<td class="remarks">' + escHtml(c.supplier_name || '—') + '</td>' +
                                    '<td class="amt">'    + formatCurrency_2(c.amount) + '</td>' +
                                    '<td class="center">' + payDoc                           + '</td>' +
                                    '<td class="center">' + deal                             + '</td>' +
                                    '<td class="muted">'  + escHtml(c.created_by    || '—') + '</td>' +
                                    '<td class="center"><a class="btn btn-sm btn-light" href="{{ url("payment-cheque-print") }}/' + escHtml(c.id) + '" target="_blank" title="Print cheque" style="padding:2px 8px;"><i class="ico icon-outline-printer-2" style="font-size:13px;"></i></a></td>' +
                                    '<td class="center">' + (c.payment_id ? '<button type="button" class="btn btn-sm btn-light cheque-payment-attachments-btn" data-payment-id="' + escHtml(c.payment_id) + '" title="Payment Attachments" style="padding:2px 8px;"><i class="ico icon-outline-paperclip" style="font-size:13px;"></i></button>' : '<span class="text-muted">—</span>') + '</td>' +
                                    '</tr>';
                            }).join('');
                            $('#msDropChequesList').html(chRows);
                            $('#msChCount').text(cheques.length + ' result' + (cheques.length > 1 ? 's' : ''));
                            $('#msDropChequesSection').show();
                        } else {
                            $('#msDropChequesSection').hide();
                        }

                        var parts = [];
                        if (cbs.length)     parts.push(cbs.length     + ' chequebook' + (cbs.length     > 1 ? 's' : ''));
                        if (cheques.length) parts.push(cheques.length + ' cheque'     + (cheques.length > 1 ? 's' : ''));
                        $('#masterSearchStatus').text(parts.join(' · '));
                        $('#msDropFooter').show();
                        $('#msDropEmpty').hide();
                        showDropdown();
                    },
                    error: function () {
                        $('#msDropLoading').hide();
                        $('#msDropEmpty').text('Search failed. Please try again.').show();
                        showDropdown();
                    }
                });
            }
            ...
            // ── row clicks ──
            $(document).on('click', '.ms-drop-cb-row', function (e) {
                e.stopPropagation();
                var id = $(this).data('id');
                hideDropdown();
                openChequebook(id);
            ...
            $(document).on('click', '.ms-drop-cheque-row', function (e) {
                if ($(e.target).closest('a').length) return;
                if ($(e.target).closest('.cheque-payment-attachments-btn').length) return;
                e.stopPropagation();
                var id = $(this).data('chequebook-id');
                hideDropdown();
                openChequebook(id);
            });

            function openChequebook(id) {
                if (!id) return;
                var $link = $('.chequebook-doc-link[data-id="' + id + '"]');
                if ($link.length) {
                    $link[0].click();
                } else {
                    try { $('#long-list').DataTable().search('').draw(); } catch(e) {}
                    setTimeout(function () {
                        var $l = $('.chequebook-doc-link[data-id="' + id + '"]');
        ...
        <script>
            function formatCurrency_2(value) {
                ...
                var normalized = String(value).replace(/,/g, '').trim();
                var n = parseFloat(normalized);
                if (Number.isNaN(n)) return $('<div>').text(String(value)).html();
                return n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
            }
                            ...
                            function renderRows(data) {
                                ...
                                data.forEach(function (item, idx) {
                                    ...
                                    rows += '<tr>' +
                                        ...
                                        '<td class="text-end">' + formatCurrency_2(item.amount) + '</td>' +
        ...
        <div class="modal fade" id="chequePaymentAttachmentsModal" style="z-index:99999" data-bs-backdrop="false" tabindex="-1" aria-labelledby="chequePaymentAttachmentsLabel" aria-hidden="true">
        ...
        <script>
            ...
            // Raise backdrop above search dropdown when attachment modal shows
            $('#chequePaymentAttachmentsModal').on('show.bs.modal', function () {
                setTimeout(function () { $('.modal-backdrop').last().css('z-index', '9150'); }, 0);
            });
