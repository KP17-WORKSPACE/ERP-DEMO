...
<input type="hidden" name="deal_page" id="deal_page" value="1">
                ...
                <input type="hidden" name="deal_id" id="deal_id" value="{{ $deal_id }}">
                            ...
                            data-bs-placement="top" data-bs-toggle="modal" data-bs-target="#accountSubModal"></a>
                                    ...
                                    <tr>
                                            ...
                                            <td><input type="text" class="form-control text-end" name="remarks[]" value="{{ @$editDataList[$roid-1]->remarks }}"></td>
                                    ...
                                    update_totals();
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" class="form-control  text-end" name="remarks[]"></td>
                                            <td>                                                                    
                                                <input class="form-control text-center" type="text" name="dealid[]" value="{{ $deal_id }}" autocomplete="off">
                                            </td>
                                            @if($company->company_vat_rate!=0)
...
$(document).on("change", 'input[name="remarks[]"], input[name="dealid[]"], input[name="amount_dr[]"]', function () {
    ...
    // Always append dealid if present and missing in remarks
    if (dealidVal) {
        let remarksVal = $remarks.val().trim();
        let tag = "(Deal Id: " + dealidVal + ")";

        // Add only if not already added
        if (!remarksVal.includes(tag)) {
            $remarks.val(remarksVal + (remarksVal ? " " : "") + tag);
        }
    }

    // Rebuild narration from all rows
    let narrations = [];
    ...
    if (e.key === "Enter") {
        ...
        if (name === "amount_dr[]") {
            row.find('input[name="amount_cr[]"]').focus();
                ...
                let $input = row.find('input[name="remarks[]"]');
    let input = $input[0]; // convert to DOM element

    input.focus();
    input.setSelectionRange(0, 0);  // move caret to start
        } 
...
function add_receipt_adjustment() {
    ...
    $.ajax({
        url: "{{ URL::to('add-receipt-adjustment-list-jv') }}",
        type: "POST",
        data: {
            _token: '{{ csrf_token() }}',
        ...
        success: function(dataResult) {
            dataResult = JSON.parse(dataResult);
            alert(dataResult == "SUCCESS" ? "Adjustment Added Successfully" : "Error: " + dataResult);
        },
        complete: function() {
...
function add_payment_adjustment() {
    ...
    $.ajax({
        url: "{{ URL::to('add-payment-adjustment-list-jv') }}",
        type: "POST",
        data: {
            _token: '{{ csrf_token() }}',
            set_amt: payload.set_amt,
                                    ...
                                    <table class="table table-hover form-item-table" id="crListBankBookAdjestUnadjusted" style="border: solid 1px #e3e6f0; width:100%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width:200px;" class="text-start">@lang('Remarks')</th>
                                            </tr>
                                        </thead>
                                        <tbody></tbody>
                                        <tfoot>
...
<div class="modal side-panel fade" id="ModalAdjustment" data-bs-backdrop="false" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal side-panel fade" id="ModalPaymentAdjustment" data-bs-backdrop="false" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg"> 
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal side-panel fade" id="importModal" data-bs-backdrop="false" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
