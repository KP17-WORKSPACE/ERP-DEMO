...
<input type="hidden" id="jv_id" value="{{ $editData->id }}">
    ...
    <div class="purchase-order-content-header-right">
        ...
        <a class="btn btn-light text-dark" href="{{ url('journalvoucher/' . $editData->id . '?jv_action=edit') }}">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu" style="">
                    <li><a class="dropdown-item d-flex align-items-center" href="{{url('journalvoucher/'.$editData->id.'/delete')}}"><i class="ico icon-outline-close-square title-15 me-2"></i> Cancel</a></li>
                    <li><a class="dropdown-item d-flex align-items-center" href="{{url('journalvoucher/'.$editData->id.'/download')}}"><i class="ico  icon-bold-download-minimalistic title-15 me-2"></i> Download</a></li>
    ...
    <div class="card mb-3 card-min-height">
      ...
      <tr>
          ...
          <td align="right"><b style="font-size: 30px; font-weight: 400;">Journal Voucher</b></td>
      </tr>
  </table>
  <br />
    ...
    <br />
      ...
      <tr>
        ...
        <td style="line-height: 18px;">{{ date('d/m/Y', strtotime(@$editData->doc_date)) }}</td>
        <td style="line-height: 18px;">{{ $editData->doc_number }}</td>
        <td style="line-height: 18px;">{{ @App\SysHelper::com_curr_format($editDataList->sum('debit_amount'),2,'.',',') }}</td>
        <td style="line-height: 18px;">{{ @$editData->currency_name->code }}</td>
        <td style="line-height: 18px;">{{ @$editData->createdby->full_name }}</td>
      </tr>
      <tr>
        <td colspan="5" style="line-height: 18px; text-align: left;"><span style="line-height: 18px; font-weight:bold;">Remarks :</span> {{ isset($editData) ? (!empty(@$editData->narration) ? @$editData->narration : old('narration')) : old('narration') }}</td>
      </tr>
    </table>
    <br />
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr style="background: #2c2b6d; color: #ffffff;">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
        <td style="width: 30px; border-bottom: solid 1px #2c2b6d;">{{$i}}. <?php $i++;?></td>
        <td style="width: 200px; border-bottom: solid 1px #2c2b6d; font-size: 11px;" class="">{{ $item->accounts->account_name }}</td>
          <td style="width: 100px; border-bottom: solid 1px #2c2b6d; text-align: right;">{{ @App\SysHelper::com_curr_format(abs($item->debit_amount),2,'.',',') }}</td>
          <td style="width: 100px; border-bottom: solid 1px #2c2b6d; text-align: right;">{{ @App\SysHelper::com_curr_format(abs($item->credit_amount),2,'.',',') }}</td>
          <td style="border-bottom: solid 1px #2c2b6d; font-size: 10px; padding-left: 10px;" class="">@if($item->remarks !=""){{ $item->remarks }}@endif</td>
      
          <td style="width: 100px; border-bottom: solid 1px #2c2b6d;" class="text-center">@if($item->transaction_ref===0) -- @else {{ $item->transaction_ref }} @endif</td>
            <?php            
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td style="border-bottom: solid 1px #2c2b6d; text-align: left; font-weight: bold; width: 230px;"></td>
          <td style="border-bottom: solid 1px #2c2b6d; text-align: right; font-weight: bold; width: 100px;">{{ @App\SysHelper::com_curr_format($sum_d,2,'.',',') }}</td>
          <td style="border-bottom: solid 1px #2c2b6d; text-align: right; font-weight: bold; width: 100px;">{{ @App\SysHelper::com_curr_format($sum_c,2,'.',',') }}</td>
          <td style="border-bottom: solid 1px #2c2b6d; text-align: right; font-weight: bold;"></td>
          <td style="border-bottom: solid 1px #2c2b6d; text-align: right; font-weight: bold; width: 100px;"></td>
        </tr>
      </table>
      <br ><br ><br ><br />
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
