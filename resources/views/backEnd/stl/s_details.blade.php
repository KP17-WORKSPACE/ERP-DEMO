  ...
  <style>
    .pdfarea header { position: fixed; left: 20px; top: -50px; right: 20px; height: 80px; background-color: white; text-align: center; border-bottom:solid 0px #808080; color:#555555;  }
    .pdfarea footer { position: fixed; left: 0px; bottom: -100px; right: 0px; height: 100px; background-color: white; }
    .pdfarea footer .page:after { content: counter(page, upper-roman); }

    .pdfarea{font-family: Verdana, sans-serif; font-size:12px; color:#555555; background-image: url('{!! asset("public/".$company->pdf_watermark."") !!}');}
    .pdfarea .pdf-border {border-collapse: collapse;}
    .pdfarea .pdf-border, .pdfarea .pdf-border th, .pdfarea .pdf-border td {border: 1px solid #9e9e9e;}
    .pdfarea th, .pdfarea td {padding: 5px 5px;}
    ...
    <div class="purchase-order-content-header">
        ...
        <div class="purchase-order-content-header-right">
            ...
            <a class="btn btn-light" href="{{url('stl/'.$stl->id.'/edit')}}">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="{{url('stl/'.$stl->id.'/delete')}}"><i class="ico icon-outline-trash-bin-minimalistic text-danger"></i> Cancel</a></li>
                    <li><a class="dropdown-item" href="{{url('stl/'.$stl->id.'/download')}}"><i class="ico icon-outline-document-medicine text-success"></i> Download</a></li>
                </ul>
            </div>
        </div>
    </div>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td>
          <table class="pdf-border" border="1" cellspacing="0" cellpadding="0">
...
<br>
            <table class="pdf-border" width="100%" border="1" cellspacing="0" cellpadding="0">
              <tr style="color: #000000;">
...
@if (count($stl_items) > 0)
  @foreach ($stl_items as $index => $item)
    @if ($inv != $item->pi_inv_no)
      ...
      <br />
      <table class="pdf-border" width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td class="item-head-row" style="width: 7%; font-weight: bold; text-align: center;">Sl.No</td>
          <td class="item-head-row" style="width: 30%; font-weight: bold; text-align: left;">Item Part Number</td>
          <td class="item-head-row" style="width: 50%; font-weight: bold; text-align: left;">Description of Goods</td>
          @if ($with_with_out == 1)
...
<br /><br />
@if ($with_with_out == 1)
<table class="pdf-border" width="50%" border="1" cellspacing="0" cellpadding="0">
  <tr style="color: #000000;">
