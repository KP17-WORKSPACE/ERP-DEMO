...
@section('mainContent')
    ...
    <?php try { ?>
        ...
        <div class="d-sm-flex justify-content-between">
                <div class="mb-3">
                    <h2 class="page-heading m-0">Receivable Outstanding</h2>
                    <span class="page-label">Home - Receivable Outstanding</span>
                </div>
                <div>
                    ...
                    <a class="btn btn-primary" data-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="fa fa-filter"></i> Search</a>
                    @endif
                ...
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'receivable-outstanding', 'method' => 'POST', 'enctype' => 'multipart/form-data']) }}
                                    ...
                                    <div class="row">
                                            ...
                                            <div class="no-gutters input-right-icon">
                                                <div class="col">
                                                    <div class="form-check-label">
                                                        ...
                                                        <input class="form-control" id="till_date" type="date" name="till_date" value="{{ @$till_date }}" autocomplete="off">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>                                       
                                        ...
                                        <div class="col-md-3 mb-2">
                                            ...
                                            <select class="form-control" name="overdue" id="overdue">
                                                <option value="">-Select-</option>
                                        ...
                                        <div class="col-md-3 mb-2">
                                            ...
                                            <select class="form-control" name="ageing" id="ageing">
                                                <option value="">-Select-</option>
                                        ...
                                        <div class="col-md-3 mb-2">
                                            ...
                                            <select class="form-control" name="list_option" id="list_option">
                                                <option value="">-Select-</option>
                                        ...
                                        <div class="col-md-3 mb-2">
                                            ...
                                            <select class="form-control" name="list_in_ex" id="list_in_ex">
                                                <option value="">-Select-</option>
                                        ...
                                        <div class="col-md-3 mt-4" >
                                            <div class="input-effect">
                                                <button class="btn btn-primary" ><span class="ti-search"></span>@lang('Filter')</button>
                                            </div>
                                        </div>
                                    </div>
...
//                     vv+="</table>";
                    ...
                    var i=1;
                        dataResult.forEach((re) => {
                            ...
                            $("#mydiv").append("<p class='text-right m-0'><i>"+re.username+" | On "+new Date(re.created_at).toLocaleDateString('en-US')+" "+new Date(re.created_at).toLocaleTimeString('en-US')+"</i></p><hr class='mt-0'>");
                            i++;
                        });
                    $("#mydiv").append("</div>");
...
//                     vv+="</table>";
            ...
            <!-- <a data-id="hello"  class="btn-badge btn btn-info  py-1 px-2" >
                                    Comment</a> -->
...
function check_total(id, amount) {
    ...
    if(currentTotal != additionalAmount){
        $('#sum_' + id).css('color', 'red');
                ...
                <table id="account_table{{ $aname->id }}" class="table main_table" style="border: solid 1px #e3e6f0; margin-bottom: -1px !important;"> <?php /* display: none; */ ?>
                    <thead>
                      <tr>
                          <th class="border text-center" width="100px"><a href="{{url('get-url-customer/'.$aname->account_code)}}" target="_blank">{{ $aname->account_code }}</a></th>
                          <th class="border text-left">
                            
                            
                            <a class="text-left" type="button" data-toggle="collapse" data-target="#collapse{{ $aname->id }}" aria-expanded="true" aria-controls="collapse{{ $aname->id }}">{{ $aname->account_name }} <span style="font-weight: normal; color: #3d3d3d;">{!! $cust_det !!}</span></a>


                          <a data-id="{{@$aname->id}}" id="crmajax" class="btn-badge btn btn-info  py-1 px-2" style="  font-weight: 500;  border: 1px solid transparent;  padding: 0.375rem 0.75rem;  font-size: 10px;  line-height: .7;  border-radius: 2px;cursor: pointer;" data-toggle="modal" data-target="#ModalTrackComment">
                                    Comments</a>
                                    
                        <a class="text-danger ml-2 float-right" title="Download" onclick="download_outstanding({{ $aname->id }})"><i class="fa fa-download" aria-hidden="true"></i></a>
                ...
                {{ Form::open(['class' => 'form-horizontal m-0', 'files' => true, 'url' => 'generalledger', 'target' => '_blank', 'method' => 'POST', 'enctype' => 'multipart/form-data']) }}
                    ...
                    <button class="btn-sm btn-primary p-0 pl-1 pr-1 cursor-pointer">&nbsp;Ledger&nbsp;</button>
                {{ Form::close() }}
                </div>


                        </th>  
                          <th class="border text-right" width="100px"><label class="main_sum" id="sum_{{ $aname->id }}"></label></th>
                      </tr>
                    </thead>
                </table>
                
                  <div id="collapse{{ $aname->id }}" class="collapse {{ $list_option }}" data-parent="#accordionExample" style="margin-left: 100px;"> <?php /* display: none; */ ?>
			  ...
			  <th class="border text-center" width="150px">>90</th>
                  ...
                  <?php $unadj = $unadjested_list->where('account_id',$aname->id); ?>
                  <?php $unadj2 = $unadjested_list2->where('account_id',$aname->id); ?>
                  <?php $unadj3 = $unadjested_list_return->where('customer',$aname->id); ?>
                  <?php $unadj4 = $unadjested_list_jv->where('account_id',$aname->id); ?>
                  <?php $unadj5 = $opb_inv_list->where('account_id',$aname->id); ?>
                  <?php $unadj_list = $list_of_unadjusted->where('account_id',$aname->id); ?>
                  
                  
                  @if (count($unadj)>0 || count($unadj2)>0 || count($unadj3)>0 || count($unadj4)>0 || count($unadj5)>0)
                  @if ($list_option == "pdc")
                    <script>
...
@endif
                            ...
                            <td class="border">{{ @App\SysHelper::com_curr_format($p->rec_balance,2,'.',',') }}</td>
                            <script>
                                set_total_lessmore({{ $aname->id }},{{ $p->rec_balance }})
                            </script>
                        </tr>
                        @endforeach
                        @endif
                        ...
                        <tr>
                            ...
                            @if(Illuminate\Support\Str::contains($p->doc_number, ['BR', 'CR']))
                            <td class="border"><a href="{{url('get-url-receipt/' . $p->doc_number)}}" target="_blank">{{ $p->doc_number }}</a></td>
                            @endif
                            @if(Illuminate\Support\Str::contains($p->doc_number, ['JV']))
                            <td class="border"><a href="{{url('get-url-journalvoucher/' . $p->doc_number)}}" target="_blank">{{ $p->doc_number }}</a></td>
                            @endif
                            @if(Illuminate\Support\Str::contains($p->doc_number, ['SR']))
                            <td class="border"><a href="{{url('get-url-sales-return/' . $p->doc_number)}}" target="_blank">{{ $p->doc_number }}</a></td>
                            @endif
                            <td class="border">{{ $p->main_account_id }}</td>
                            <td class="border">{{ @App\SysHelper::com_curr_format(@$p->amount - @$p->adj_amount,2,'.',',') }}</td>
                            <script>
                        ...
                        <tr>
                            ...
                            <td class="border"><a href="{{url('get-url-sales-return/' . $p->doc_number)}}" target="_blank">{{ $p->doc_number }}</a></td>
                            @endif
                            <td class="border">{{ @$p->accountname->account_name }}</td>
                            <td class="border">{{ @App\SysHelper::com_curr_format(@$p->amount,2,'.',',') }}</td>
                            <script>
                                set_total_lessmore({{ $aname->id }},{{ $p->amount }})
                            </script>
                        </tr>
                        @endforeach
                        @endif
                        ...
                        <tr>
                            ...
                            <td class="border"><a href="{{url('get-url-sales-return/' . $p->doc_number)}}" target="_blank">{{ $p->doc_number }}</a></td>
                            @endif
                            <td class="border">{{ @$p->accounts->account_name }}</td>
                            <td class="border">{{ @App\SysHelper::com_curr_format(@$p->amount,2,'.',',') }}</td>
                            <script>
                                set_total_lessmore({{ $aname->id }},{{ $p->amount }})
                            </script>
                        ...
                        @foreach ($unadj5 as $p)
                        <tr>
                            <td class="border">{{ date('d/m/Y', strtotime($p->transaction_date)) }}</td>
                            <td class="border">{{ $p->transaction_no }}</td>
                            <td class="border">Opening Balance</td>
                            <td class="border">{{ @App\SysHelper::com_curr_format(abs(@$p->credit_amount-@$p->debit_amount),2,'.',',') }}</td>
                            <script>
                                set_total_lessmore({{ $aname->id }},{{ abs(@$p->credit_amount-@$p->debit_amount) }})
                            </script>
                        </tr>
                        @endforeach
                        @endif
                  ...
                  <?php $pdc = $pdc_list->where('account_id',$aname->id)->where('bi_doc_no','=',null); ?>
                  @if (count($pdc)>0)
                  ...
                  <table class="table" style="border: solid 1px #e3e6f0; width:auto; width:100%;">
                    ...
                    <tbody>
                        ...
                        <tr id="row_pdc_received_{{ $p->doc_number }}">
                            ...
                            <td class="border">{{ @App\SysHelper::com_curr_format($p->credit_amount,2,'.',',') }}</td>
                            ...
                            <td class="border"><a class="btn-sm btn-danger" id="btn_pdc_received_{{ $p->doc_number }}" onclick="pdc_update('{{ $p->doc_number }}','{{ $p->receipt_date }}')">Update</a></td>
                        </tr>
                        @endforeach
                    </tbody>
                  </table>
                  ...
                  <table class="table" style="border: solid 1px #e3e6f0; width:auto; width:100%;">
                    ...
                    <tbody>
                        ...
                        <tr id="row_pdc_received_{{ $p->doc_number }}">
                            ...
                            <td class="border">
                                {{ @App\SysHelper::com_curr_format(@$p->bi_cheque_amount - @$p->bi_amount_adjusted,2,'.',',') }}
                            </td>
                            <td class="border"><a class="btn-sm btn-danger" id="btn_pdc_received_{{ $p->doc_number }}" onclick="pdc_update('{{ $p->doc_number }}','{{ $p->receipt_date }}')">Update</a></td>
                            
                            <script>
                                set_total_addmore({{ $aname->id }},{{ $p->credit_amount }})
                            </script>
                        </tr>
                                    ...
                                    <table class="table" style="border: solid 1px #e3e6f0; width:auto; width:100%;" id="row_det_table_{{ $p->doc_number }}">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="border text-right">Adjustments</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                  ...
                  <table class="table" style="border: solid 1px #e3e6f0;">
                    <thead>
                        <tr>
                            ...
                            <th class="border text-right" width="85px"><?php /*{{ $all_total }}*/ ?> <label id="lbl_all_total"></label></th>
            ...
            function row_det_fun(id,docs){
                ...
                for (var i = 0; i < doc.length; i++) {
                    ...
                    var htm = "<tr>\
                        ...
                        <td class='border text-right'>"+inv_e_amount+"</td>\
                        <td class='border text-right'>"+inv_e_adjustment+"</td>\
                        </tr>"
...
<div class="modal fade" id="ModalTrackComment" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document" style="min-width:50% !important;">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Payment Follow-up Remark</h5>
