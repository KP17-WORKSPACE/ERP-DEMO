    ...
    {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'goods-receipt-note-update', 'method' => 'POST', 'id' => 'goods-receipt-note-update','novalidate' => true]) }}
    ...
    <input type="hidden" name="net_vat" id="net_vat" value="{{ optional(optional($grn->accountname)->cust_suppl)->vat_percentage ?? 0 }}">
    ...
    <div class="purchase-order-content-header">
        ...
        <div class="purchase-order-content-header-right">
            ...
            <a type="submit" class="btn btn-light text-dark" href="{{ url('goods-receipt-note-list/' . @$grn->id) }}?grn_action=add">
...
@endphp
  ...
  <script>
                            ...
                            @foreach ($customer_reference_list as $value)
                            <option value="{{ @$value->id }}"   @if(in_array($value->id, $selectedCompanies)) selected @endif >{{ @$value->name }} 
                                ...
                                <option value=""></option>
                                    ...
                                    @if(isset($grn) && $grn->sales_person)
                                        ...
                                        @if(!$exists)
                                            ...
                                            @endif
                                        @endif
                                    @elseif(isset($grn) && !is_null($grn->sales_person_name) && $grn->sales_person_name !== '')
                                        {{-- If sales_person_name is set (manual entry), append it as a manual option --}}
                                        <option value="{{ $grn->sales_person_name }}" selected>{{ $grn->sales_person_name }}</option>
                                ...
                                <div class="form-group">
                                     <select class="form-control js-example-basic-single" required name="warehouse" id="warehouse">
                                        ...
                                        @foreach ($warehouses as $value)
                                            <option value="{{ @$value->id }}" @if (@$grn->warehouse == $value->id) selected
                        ...
                        <div class="form-group">
                            <select class="form-control js-example-basic-single" name="shipping_supplier"
                                ...
                                @foreach ($customer as $value)
                                    ...
                                    <option value="{{ @$value->id }}" {{ $s }} @if($grn->shipping_supplier == $value->id) selected @endif>
                                        {{ @$value->account_name }} 
                                        @if(@App\SysHelper::getCompanyCodeSettings()['is_customer_code'])
                                        ({{ @$value->account_code }})
                                        @endif
                    ...
                    <div class="col-2">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="shipping_contact_no"
                                id="shipping_contact_no" value="{{ $grn->shipping_contact_no }}" />
                        </div>
                    </div>
                    <div class="col-3">
                        ...
                        <div class="form-group">
                            <input type="text" class="form-control" name="shipping_address_1"
                                id="shipping_address_1" value="{{ $grn->shipping_address_1 }}" />
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row row-cols-6 gap-rows">
                            ...
                            <select class="form-control js-example-basic-single" style="width: 100%;"
                                ...
                                @foreach ($countries as $key => $value)
                                    <option value="{{ @$value->id }}" <?php        try {?>
                                        @if (isset($grn)) @if (@$grn->supplier_country == $value->id) selected @endif
                    ...
                    <div class="input-effect">
                        ...
                        <div id="sectionStateDiv">
                         <select class="form-control js-example-basic-single" name="supplier_state" id="state">
                            ...
                            @foreach ($states as $value)
                                <option 
                                    value="{{ $value->id }}" @if( $grn->supplier_state == $value->id) selected @endif>
                                    {{ $value->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="form-group">
                           ...
                           <input class="form-control" type="number"  name="vat_percent" id="vat_percent" value="{{ @$grn->vat_percent }}">
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                           ...
                           <input class="form-control" type="number"  name="vat_number" id="vat_number" value="{{ @$grn->vat_number }}">
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            <select class="form-control js-example-basic-single {{ $errors->has('supplier_type') ? ' is-invalid' : '' }}"
                                ...
                                @foreach ($suppliertype as $value)
                                    <option value="{{ @$value->id }}"
                                        {{ isset($grn) ? (!empty(@$grn->supplier_type) ? (@$grn->supplier_type == @$value->id ? 'selected' : '') : '') : '' }}>
                                        {{ @$value->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <select name="purchase_type" id="purchase_type"
                                ...
                                @foreach ($purchasetype as $value)
                                    <option value="{{ @$value->id }}"
                                        {{ isset($grn) ? (!empty(@$grn->purchase_type) ? (@$grn->purchase_type == @$value->id ? 'selected' : '') : '') : '' }}>
                                        {{ @$value->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @foreach ($grn_items as $items)
                            <tr>
                                <td><input type="text" class="form-control text-center" name="sort_id[]"
                                        value="{{ $i }}" />
                                    <input type="hidden" id="product_type[]" name="product_type[]" value="{{ $items->product_type }}" />
                                    <input type="hidden" name="item_po_id[]" value="{{ $items->po_id }}" />
                                    <input type="hidden" name="part_number_txt[]"
                                        value="{{ $items->part_number }}" />
                                    <input type="hidden" name="grn_line_item_id[]" value="{{ $items->id }}" />
                                </td>
                                <td>
                                    <select class="form-control noborder " name="part_number[]">
                                        <option value="{{ $items->part_no }}">{{ $items->part_number ?? 0 }}</option>
                                    </select>
                                </td>



                                <td><textarea class="form-control" name="description[]" rows="1">{{ $items->description ?? 0 }}</textarea></td>

                                @if (session('logged_session_data.company_id') == 2)
                                    <td class="text-center">
                        <input class="form-control text-center" type="text" value="{{ $items->hscode }}" name="hscode_txt[]" autocomplete="off" readonly="true">
                                ...
                                <td><input type="text" class="form-control text-center" data-enter-skip   name="qty[]"
                                        value="{{ $items->qty }}" onkeypress="return set_license_key(this, event)" onchange="calc_change_new(this)" /></td>
                                ...
                                <td><input type="text" class="form-control text-end" step="Any"
                                        id="unitprice_{{ $i }}" name="unitprice[]" onblur="formatCurrency(this)"
...
@endphp
@if (!empty($purchase_invoice) && !empty($purchase_invoice->id))
    <input type="hidden" name="cfc_pi_id" value="{{ $purchase_invoice->id }}" />
@endif
                ...
                @if(isset($edit_cfc) && $edit_cfc->count() > 0)
                    @foreach($edit_cfc as $cfc)
                        <tr id="fright_row_{{ $loop->iteration }}">
                            ...
                            <td>
                                <select class="form-control js-example-basic-single" name="cfc_credit_account[]" id="cfc_credit_account_{{ $loop->iteration }}">
                                    <option value=""></option>
