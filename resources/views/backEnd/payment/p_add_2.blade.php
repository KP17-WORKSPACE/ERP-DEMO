                ...
                $(document).ready(function () {
                    ...
                    $(document).on('change', '#mode', function () {
                        ...
                        if (mode == 1) {
                            ...
                            $('#chequebook').val(''); $('#chequebook_label').hide();
                            ...
                            $('#div_payment_mode').css('display', '');  // restore container hidden by STL mode
                            ...
                            // Fire native event so the vanilla-JS payment_through handler responds
...
<div class="tab-wrap mb-3">
    ...
    <div class="tab-content" id="purchaseDetailsTabsContent">
            ...
            <div class="row gap-rows">
                ...
                <div class="col-12 mb-2">
                    <div class="row gap-rows">
                        ...
                        <script>
                            // payment_through — vanilla JS delegated handler
                            // Cheque fields are ONLY shown when mode=Bank(2) AND payment_through=Cheque(3)/CDC Cheque(2)
                            document.addEventListener('change', function (e) {
                                if (!e.target.matches('#payment_through')) return;
                                var paymentthrough = String(e.target.value);
                                ...
                                function hideChequeParts() {
                                    chequeFieldIds.forEach(function (id) { var el = document.getElementById(id); if (el) el.style.display = 'none'; });
                                    chequeInputIds.forEach(function (id) { var el = document.getElementById(id); if (el) { el.required = false; el.value = ''; } });
                                    var cb = document.getElementById('chequebook'); if (cb) cb.value = '';
                                ...
                                if (currentMode === '2' && (paymentthrough === '2' || paymentthrough === '3')) {
                                    chequeFieldIds.forEach(function (id) { var el = document.getElementById(id); if (el) el.style.display = ''; });
                                    chequeInputIds.forEach(function (id) { var el = document.getElementById(id); if (el) el.required = true; });
                                    var ac = document.getElementById('addCheque'); if (ac) ac.style.display = '';
                                    var bwh = document.getElementById('bill_wise_heading'); if (bwh) bwh.textContent = '@lang("Cheque Amount")';
                                    var acb = document.getElementById('add_cheque_btn'); if (acb) acb.style.display = '';
                                    fetchNextAvailableCheque(bankSel ? bankSel.value : '');
                                    return;
                                }

                                // All other cases: hide cheque parts
                                hideChequeParts();
                                if (currentMode === '2' && paymentthrough === '1') {
                                    var bwh = document.getElementById('bill_wise_heading'); if (bwh) bwh.textContent = '@lang("Bank Transfer Amount")';
                                }
                            });
                        </script>
                        <div class="col-1-5 mb-4" id="div_payment_days">
                            ...
                            <script>
                                function days_fun() {
                                    var daysToAdd = parseInt(document.getElementById('payment_days').value);
                        ...
                        <div class="col-2 mb-4" id="div_cheque_number">
                            <div class="input-effect">
                                ...
                                <small class="text-muted" id="chequebook_label"
                                    style="display:none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 100%;"></small>
                        ...
                        <div class="col-2 mb-4" id="div_remarks">
                            <div class="input-effect">
                        ...
                        @if (session('logged_session_data.company_id') == 6)
                            <script>
                                ...
                                var _bankSel = document.querySelector('select[name="payment_mode_bank"]');
                                if (_bankSel) { _bankSel.value = 8064; _bankSel.dispatchEvent(new Event('change', { bubbles: true })); }
                            </script>
                        @endif
...
TT Value to "+ vend.trim() + " to be " + curr.trim() + " " + usd.trim() + " as per below details";
        ...
        <table class="table table-hover form-item-table" id="myTable">
            <thead>
                <tr>
                    ...
                    <th class="resizable text-center" width="450px">@lang('Account Name')
                    ...
                    <th class="resizable text-center">@lang('Narration')
                        <div class="resizer"></div>
                    </th>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <input class="form-control text-end" type="decimal" name="amount[]" data-enter-skip autocomplete="off"
        ...
        function validateAttachForm() {
            ...
            // show toastr
                if (typeof toastr !== 'undefined') {
                    toastr.success('Bill adjustments saved.');
                }
    ...
    <script>
        ...
        function generate_narration() {
            var gn_mode = document.getElementById('mode').value;
            var n1El = document.getElementById('narration_1');
            var n2El = document.getElementById('narration_2');
            var narEl = document.getElementById('narration');
            var narActEl = document.getElementById('narration_actual');
            if (gn_mode == 1) {
                n1El.value = 'Paid Cash';
                narEl.value = n1El.value + ' ' + n2El.value;
                narActEl.value = narEl.value;
            }
            if (gn_mode == 2) {
                var sel = document.getElementById('payment_mode_bank');
                var gn_bank_name = sel && sel.selectedIndex >= 0 ? sel.options[sel.selectedIndex].text : '';
                n1El.value = 'Paid From ' + gn_bank_name;
                narEl.value = n1El.value + ' ' + n2El.value;
                narActEl.value = narEl.value;
            }
        }
        ...
        function generate_narration_fa() {
            var inputs = document.querySelectorAll('input[name="remarks[]"]');
            var remarksArr = [];
            inputs.forEach(function (el) { var v = el.value.trim(); if (v) remarksArr.push(v); });
            if (remarksArr.length === 0) return;
            var n1 = document.getElementById('narration_actual').value;
            document.getElementById('narration').value = n1 + ' ' + remarksArr.join(' | ');
        }

        // ── Next Available Cheque (auto-assign) ──────────────────
        function fetchNextAvailableCheque(bankId) {
            console.log('[debug] fetchNextAvailableCheque bankId', bankId);
            ...
            xhr.onload = function () {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    console.log('[debug] fetchNextAvailableCheque response', response);
                    if (response.success) {
                    ...
                    console.error('[debug] fetchNextAvailableCheque HTTP error', xhr.status, xhr.responseText);
                    document.getElementById('cheque_number').value = '';
                    if (label) { label.textContent = 'Error loading cheque number'; label.style.display = ''; }
                }
            };
            xhr.onerror = function () {
                console.error('[debug] fetchNextAvailableCheque network error');
                document.getElementById('cheque_number').value = '';
                if (label) { label.textContent = 'Error loading cheque number'; label.style.display = ''; }
            };
            xhr.send();
        ...
        function getChequebookByChequeNumber(bankId, chequeNumber) {
            ...
            xhr.onload = function () {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    if (response.success) {
                        ...
                        if (label) {
                            label.textContent = response.message || 'No matching chequebook found';
                            label.style.display = '';
                        }
                    }
                } else {
                    if (label) {
                        label.textContent = 'Cheque lookup failed';
                        label.style.display = '';
                    }
                }
            };
            xhr.onerror = function () {
                if (label) {
                    label.textContent = 'Cheque lookup network error';
                    label.style.display = '';
                }
            };
            xhr.send();
        ...
        // update chequebook when user manually enters cheque number (for the selected bank)
        document.addEventListener('change', function (e) {
        ...
        // ── Datepicker reinit for dynamically cloned rows ─────────
        function reinitDatePickers(context) {
            var ctx = context || document;
            ctx.querySelectorAll('.form-control.date-picker').forEach(function (el) {
                if (!el.classList.contains('hasDatepicker')) {
                    $(el).datepicker({ onSelect: function (dateText) { el.classList.toggle('active', !!dateText); } });
                }
            });
        }

        // ── Register delegated listeners immediately (no DOMContentLoaded needed for document-level delegation) ──
        document.addEventListener('change', function (e) {
            if (e.target.matches('#mode')) generate_narration();
        });
        ...
        document.addEventListener('change', function (e) {
            ...
            console.log('[payment_mode_bank] changed bank=' + bank + ' pt=' + pt);
            generate_narration();
            if (pt === '2' || pt === '3') {
        ...
        // payment_through: generate narration (show/hide handled above)
        document.addEventListener('change', function (e) {
            if (!e.target.matches('#payment_through')) return;
            generate_narration();
            var bankSel = document.querySelector('select[name="payment_mode_bank"]');
            var bank = bankSel ? bankSel.value : '';
            var pt = String(e.target.value);
            if (bank && (pt === '2' || pt === '3')) {
                fetchNextAvailableCheque(bank);
            } else {
                document.getElementById('cheque_number').value = '';
                document.getElementById('chequebook').value = '';
                var lbl = document.getElementById('chequebook_label'); if (lbl) lbl.style.display = 'none';
            }
        });

        // ── Init on load (runs immediately since this script is at bottom of page) ──
        (function init() {
            generate_narration();
            reinitDatePickers();
            var bankSel = document.querySelector('select[name="payment_mode_bank"]');
            ...
            if (ptSel) ptSel.dispatchEvent(new Event('change', { bubbles: true }));
        })();
    </script>

    <!-- Payment Attachments Modal -->
    <div class="modal fade" id="paymentAttachmentsModal" data-bs-backdrop="false" tabindex="-1"
        aria-labelledby="paymentAttachmentsModalLabel" aria-hidden="true">
