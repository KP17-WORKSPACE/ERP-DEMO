...
@section('mainContent')
    ...
    <div class="container-fluid">
        <div class="d-sm-flex justify-content-between">
            <div class="mb-3">
                <h2 class="page-heading m-0">Payments</h2>
                <span class="page-label">Home - Payment</span>
            </div>
            <div>
                <a href="{{ url('stl') }}" target="_blank" type="button" class="btn btn-warning"><i class="fa fa-plus"></i> STL</a>
                <a href="{{ url('payment-cheque-list') }}" target="_blank" type="button" class="btn btn-primary"><i class="fa fa-plus"></i> Cheque</a>
                <a href="{{ url('payment-add') }}" target="_blank" type="button" class="btn btn-info"><i class="fa fa-plus"></i> New</a>
            ...
            {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'payment', 'method' => 'post', 'id' => 'payment-search']) }}
                <div class="row">
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-control js-example-basic-single" name="payment_mode" id="payment_mode">
                            <option value="">-Select-</option>
                            @if (count($payment_mode_list)>0)
                            @foreach ($payment_mode_list as $li)
                                <option value="{{ $li["id"] }}" @if($ctrl_payment_mode==$li["id"]) selected @endif>{{ $li["account_name"] }}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
                    <div class="col-md-4 mb-2">
                        ...
                        <select class="form-control js-example-basic-single" name="payment_through" id="payment_through">
                            <option value="">-Select-</option>
                    ...
                    <div class="col-md-4 mb-2">
                        ...
                        <input class="form-control datepicker" id="payment_date" type="date" autocomplete="off" name="payment_date" value="{{ $ctrl_payment_date }}">
                    </div>
                    ...
                    @if (isset($payment))
                        @foreach ($payment as $value)
                            <tr @if($value->status == 2) class="bg-dark" @endif @if(@$value->type==2) class="text-danger" @endif>
                                <td><a href="{{url('payment/' . @$value->id . '/view')}}">{{ @$value->doc_number }}</a></td>
                                <td>
                                ...
                                <td>
                                    @if(@$value->mode == 1) Cash
                                    @else
                                        @if(@$value->payment_through == 1) Bank Transfer
                                        @elseif(@$value->payment_through == 2) CDC Cheque
                                        @else PDC Cheque
                                        @endif
                                    @endif
                                </td>
                                ...
                                <td>@if(@$value->mode == 2 && @$value->payment_through != 1) {{date('d/m/Y', strtotime(@$value->cheque_date))}} @endif</td>
                                <td>{{ @$value->cheque_number }}</td>
                                <td>
                                    <a href="{{url('get-url-deal-track/'.@$value->deal_code->code)}}" target="_blank">{{ @$value->deal_code->code }}</a>
                                </td>
                                <td>{{ @$value->full_name }}</td>
                                <td>{{ @$value->narration }}</td>
                                <td>
                                    <a class="btn-sm btn-info" href="{{url('payment/'.$value->id.'/download')}}" class="btn-small"><i class="fa fa-download" aria-hidden="true"></i></a>

                                    <a class="btn-sm btn-primary" href="{{url('payment/' . @$value->id . '/edit')}}"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    @if (@$value->status == 2)
                                        <a class="btn-sm btn-warning" href="{{url('restore-payment/'.$value->id)}}" onclick="return confirm('Are you sure you want to restore this item?');"><i class="fa fa-undo" aria-hidden="true"></i></a>
                                    @else
                                        <a class="btn-sm btn-danger" href="{{url('delete-payment/'.$value->id)}}" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash" aria-hidden="true"></i></a>
                                    @endif
                                </td>
                            </tr>
