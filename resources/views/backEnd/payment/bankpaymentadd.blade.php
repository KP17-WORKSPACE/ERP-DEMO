...
@section('mainContent')
    ...
    <div class="container-fluid">
        <div class="d-sm-flex justify-content-between">
            <div class="mb-3">
                <h2 class="page-heading m-0">Bank Payment</h2>
                <span class="page-label">Home - Bank Payment</span>
            </div>
            <div>
                <a href="{{ url('bankpayment-add') }}" type="button" class="btn btn-info"><i class="fa fa-plus"></i> New</a>
                <a href="{{ url('bankpayment') }}" type="button" class="btn btn-info"><i class="fa fa-list"></i> List</a>
            </div>
        </div>
        
        <div class="card p-4 mb-2">
            @if(isset($editData))
            {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'bankpayment-update/'. @$editData->id, 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'id' => 'bankpayment-create-form']) }}
            <input type="hidden" value="{{@$editData->id}}" name="cust_id">
            @else
            {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'bankpayment-store', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'bankpayment-create-form']) }}
            @endif
                ...
                <div class="col-lg-12">
                  <div class="white-box">
                                ...
                                <div class="no-gutters input-right-icon">
                                    <div class="col">
                                        <div class="input-effect">
                                            ...
                                            <input class="form-control" type="text" id="doc_number" name="doc_number" value="{{isset($editData)?@$editData->doc_number:'BP-'.sprintf('%03d', @App\SysBankPayment::max('id') + 1) }}" readonly>
                                            <span class="focus-border"></span>
                        ...
                        <div class="equipment comon-status row mt-4 d-block">
                            <table class="table table-bordered table-striped" id="bp-table" width="100%" cellspacing="0">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    <td>{{$roid}}</td>
                                        ...
                                        <td>
                                            <input class="form-control" type="number" id="amount_{{$roid}}" name="amount[]" autocomplete="off" min="0" value="{{@$editDataList[$roid-1]->debit_amount}}" onchange="calc_total()" onkeypress="bp_popup_fun({{$roid}})">
                                        </td>
                                        <td>
                                    ...
                                    @endfor
                                    <?php $roid--;?>
                                    <input type="hidden" id="bp-row-count" value="{{$roid}}">
                                    <a data-modal-size="modal-md" data-target="#bp_popup_win" id="addCtrlBankPaymentAdjest" data-toggle="modal"></a>
                                    <script>                                        
                            ...
                            <div style="display: none;">
                                @if(!isset($view))
                                    <button type="button" class="primary-btn small fix-gr-bg" id="addRowBP"><span class="ti-plus pr-2"></span>@lang('lang.item')</button>
                                @endif
                            </div>
...
function fn_addRow(id) {
    var rownum = document.getElementById('bp-row-count').value;
    if(id==rownum) {
     document.getElementById('bp-row-count').value = (Number(rownum) + Number(1));
        document.getElementById('addRowBP').click();
    }
}

function calc_total(){
    var countrow = document.getElementById('bp-row-count').value;
    var t1=0;
    for(var i=1; i<=countrow; i++)
    {
        t1 += Number($('#amount_'+i).val());
    }
    $('#amount_total').text(t1.toFixed(2));
}
</script>
                ...
                <div class="row mt-40">
                    <div class="col-lg-12 text-center">
                        ...
                        <button class="btn btn-primary">
                            ...
                            @if(isset($editData)) @lang('lang.update') @else @lang('lang.add') @endif @lang('Bank Payment')
                        </button>
                        @endif
                    </div>
                </div>
    ...
    <form id="ta" >
    <div class="modal fade admin-query" id="bp_popup_win">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 80%;">
            <div class="modal-content">
                ...
                <div class="modal-body m-0 p-3">
                    <input type="hidden" id="bp_account_id">
                    <input type="hidden" id="b_account_id_amount">
                    <div class="container-fluid">
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="equipment comon-status row mt-40 d-block">
                                    <table class="sstable" cellspacing="0" width="100%" id="cpListBankPaymentAdjest">
                                        <thead>
                                            <tr>
...
<script>
    function BankPaymentAdjestBalance(id) {
        var bi_total = $('#bi_total_'+id).val();
        var bi_paid = $('#bi_paid_'+id).val();
        var tot = (parseFloat(bi_total)-parseFloat(bi_paid)).toFixed(2);
        $('#bi_balance_'+id).val(tot);
        $('#bi_amount_'+id).val(bi_paid);
    }

    function validateBankPaymentAdjestForm(id) {
    ...
    var account_id = $('#bp_account_id').val();
    var entry_date = $('#doc_date').val();
    var transaction_type ='bankpayment';
    var entry_type =1; //1 Debit, 2 Credit
    var process_id = $('#bankpayment_process_id').val();
    ...
    $(".btn_ajax_bp").prop('disabled', true);
    ...
    $.ajax({
        ...
        success: function(response) {  
            ...
            {
                $("#btn_close2").click();
                $("#addCtrlBankPaymentAdjest").click();
            }            
        },
    ...
    function bp_popup_fun(id)
    {
        $('#amount_'+id+'').keypress(function (e) {
        var key = e.which;
        if(key === 13)  // the enter key code
            {
                var bp_account_id = $('#account_id_'+id+'').val();
                var bp_account = $('#amount_'+id+'').val();
                if(bp_account_id != "" && bp_account != ""){
                    $('#bp_account_id').val(bp_account_id);
                    $('#bp_account_id_amount').val(bp_account);
                    $('#bi_adjusted_amount').val(bp_account).focus();
