    ...
    <div class="tab-content mb-3" id="purchaseDetailsTabsContent">
        ...
        <script>
            ...
            $(document).on('change', '#payment_through', function () {
                ...
                if (paymentthrough === '1') {
                    $('#div_cheque_date, #div_cheque_number, #div_payment_days, #div_chequebook, #div_cheque_status').hide();
                ...
                if (paymentthrough === '2' || paymentthrough === '3') {
                    $('#div_cheque_date, #div_cheque_number, #div_payment_days, #div_chequebook, #div_cheque_status').show();
...
TT Value to "+ vend.trim() + " to be " + curr.trim() + " " + usd.trim() + " as per below details";
        ...
        <table class="table table-hover form-item-table" id="myTable">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="no_days_col"><input type="text" class="form-control text-center "
                            name="no_of_days_grid[]"></td>
                    <td class="cheque_date_col"><input type="text" class="form-control date-picker text-center"
                            name="cheque_date_grid[]"></td>
                    <td class="cheque_number_col">
                        <input type="text" class="form-control text-center" name="cheque_number_grid[]">
                        <input type="hidden" name="chequebook_id_grid[]" value="">
                    </td>
                    <td class="status_col">
    ...
    <script>
        $(document).on('focus', 'select[name="account_id[]"]', function () {
            ...
            if (!$select.hasClass('js-account-select')) {
                ...
                // Initialize Select2 (initAccountSelect2 is module-level — always in scope)
    ...
    <script>
        ...
        // ---------------------------------------------------------------------------
        // Chequebook cache
        // Structure: { bankId: int, data: [{ id, doc_number, start_no, end_no, used_numbers: [] }] }
        // ---------------------------------------------------------------------------
        var _chequebookCache = null;

        /**
         * Fetch chequebook data for the given bank and cache it.
         * Subsequent calls for the same bankId are no-ops (served from cache).
         * @param {string|number} bankId
         * @param {Function}      [callback]  called once cache is ready
         */
        function fetchChequebookData(bankId, callback) {
            bankId = parseInt(bankId, 10);
            if (!bankId) {
                _chequebookCache = null;
                if (callback) callback();
                return;
            }
            if (_chequebookCache && _chequebookCache.bankId === bankId) {
                if (callback) callback();
                return;
            }
            $.ajax({
                url: '{{ url("api/chequebook-data-by-bank") }}/' + bankId,
                method: 'GET',
                success: function (response) {
                    _chequebookCache = response.success
                        ? { bankId: bankId, data: response.data }
                        : null;
                    if (callback) callback();
                },
                error: function () {
        ...
        /**
         * Returns the next available cheque number by walking chequebooks oldest→newest.
         * Skips numbers already saved in the DB AND numbers already typed into other
         * rows of this form (prevents duplicate assignment in the same session).
         *
         * @param  {jQuery} $excludeRow  the row currently being filled (excluded from dup-check)
         * @returns {string|null}
         */
        function getNextAvailableChequeNumber($excludeRow) {
            if (!_chequebookCache || !_chequebookCache.data || !_chequebookCache.data.length) {
                return null;
            }

            // Numbers already assigned to OTHER rows in the open form
            var assignedInForm = new Set();
            $('input[name="cheque_number_grid[]"]').each(function () {
                if ($excludeRow && $(this).closest('tr').is($excludeRow)) return;
                var val = $.trim($(this).val());
                if (val !== '') assignedInForm.add(val);
            });

            for (var i = 0; i < _chequebookCache.data.length; i++) {
                var cb = _chequebookCache.data[i];

                // Support very large cheque numbers via BigInt and string handling.
                var start = BigInt(cb.start_no);
                var end = BigInt(cb.end_no);

                // Used from DB as string set
                var usedSet = new Set(
                    (cb.used_numbers || []).map(function (n) { return String(n).trim(); })
                );

                var candidate = start;
                while (candidate <= end) {
                    var candidateStr = String(candidate);
                    if (!usedSet.has(candidateStr) && !assignedInForm.has(candidateStr)) {
                        return {
                            cheque_number: candidateStr,
                            chequebook_id: cb.id || ''
                        };
                    }
                    candidate = candidate + BigInt(1);

                    // safety break: avoid infinite loop if start/end mis-specified
                    if (candidate > start + BigInt(1000000) && (end - start > BigInt(1000000))) {
                        // for very large ranges, move to the first missing block by scanning next 1m only.
                        break;
                    }
                }
            }
            return null; // all cheques exhausted
        }

        /**
         * Initialise (or re-initialise) a Select2 supplier search on the given element.
         * Also binds the cheque-number auto-fill on select2:select.
         * Defined at module level so it is accessible from ALL callers (focus handlers,
         * document.ready initialisation, and cloned-row handlers).
         *
        ...
        function initAccountSelect2(selector) {
            ...
            $el.select2({
                ajax: {
                    ...
                    data: function (params) {
                        return { search_text: params.term };
                    },
            ...
            // ---------------------------------------------------------------
            // Auto-fill cheque number in the same row when a supplier is picked
            // ---------------------------------------------------------------
            $el.on('select2:select', function (e) {
                var $row = $(this).closest('tr');
                console.log('Supplier selected (account_id=' + e.params.data.id + '), checking if cheque number auto-fill is needed for row:', $row);

                // if (String($('#payment_through').val()) !== '3') return;

                console.log('Payment through is Cheque, proceeding with auto-fill check.');

                var $chequeInput = $row.find('input[name="cheque_number_grid[]"]');
                console.log('Supplier selected, attempting to auto-fill cheque number. Current value:', $chequeInput.val());
                if ($chequeInput.val().trim() !== '') return; // don't overwrite manual entry

                var doFill = function () {
                    var next = getNextAvailableChequeNumber($row);
                    if (next !== null) {
                        $chequeInput.val(next.cheque_number);
                        $row.find('input[name="chequebook_id_grid[]"]').val(next.chequebook_id);
                    }
                };

                var bankId = parseInt($('#payment_mode_bank').val(), 10);
                if (_chequebookCache && _chequebookCache.bankId === bankId) {
                    doFill();
                } else {
                    fetchChequebookData(bankId, doFill);
                }
            });
        }

// Keep chequebook_id in sync with manually entered cheque numbers
        $(document).on('change', 'input[name="cheque_number_grid[]"]', function () {
            var chequeNo = $.trim($(this).val());
            var $row = $(this).closest('tr');
            var $hidden = $row.find('input[name="chequebook_id_grid[]"]');
            $hidden.val('');

            if (!chequeNo || !_chequebookCache || !_chequebookCache.data || !_chequebookCache.data.length) {
                return;
            }

            try {
                var candidate = BigInt(chequeNo);
            } catch (e) {
            ...
            // Invalidate cache and pre-load whenever the bank account changes
        $(document).on('change', '#payment_mode_bank', function () {
            _chequebookCache = null;
            fetchChequebookData($(this).val());
        });

        $(document).ready(function () {

            // Initialise Select2 on any selects that already have the class
            initAccountSelect2('.js-account-select');

            // Auto-open the first row's supplier select on page load
            var $firstAccountSelect = $('select[name="account_id[]"]').first();
            ...
            // Re-open dropdown on repeated focus (after it has already been initialised)
            ...
            // Ensure click also opens the dropdown
            $(document).on('click', '.js-account-select', function () {
                $(this).select2('open');
            });

            // Auto-focus the search field when any Select2 dropdown opens
            ...
            // Pre-load chequebook data for the bank already selected on page load
            var initialBankId = $('#payment_mode_bank').val();
            if (initialBankId) {
                fetchChequebookData(initialBankId);
            }
        });
    </script>
