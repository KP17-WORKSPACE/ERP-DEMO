   ...
   <script src="https://cdn.jsdelivr.net/npm/exceljs@4.3.0/dist/exceljs.min.js"></script>
    ...
    <aside class="left-nav col-3" id="leftSidebar">
                        ...
                        <div class="row">
                            ...
                            <div class="col-1-5 mb-2">
                                ...
                                <input class="form-control date-picker" type="text" autocomplete="off" name="to_date"
                                    value="{{ @App\SysHelper::normalizeToDmy($ctrl_date2) }}">
                            </div>
                            <div class="col-1-5 mb-2">
                                ...
                                <select class="form-control js-example-basic-single" name="sales_person"
                                    ...
                                    @foreach ($sales_person_list as $value)
                                        <option value="{{ @$value->user_id }}" {{ @$ctrl_sales_person == @$value->user_id ? 'selected' : '' }}>{{ @$value->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-1-5 mb-2">
                                ...
                                <select class="form-control js-example-basic-single" name="currency" id="currency">
                                    ...
                                    @foreach ($currency as $value)
                                        <option value="{{ @$value->id }}" {{ @$ctrl_currency == @$value->id ? 'selected' : '' }}>{{ @$value->code }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-1-5 mb-2">
                                ...
                                <select class="form-control js-example-basic-single" name="attachments" id="attachments">
                                    <option value="">Select</option>
                                    <option value="1" {{ @$ctrl_attachments == 1 ? 'selected' : '' }}>With Attachments Only</option>
                                    <option value="2" {{ @$ctrl_attachments == 2 ? 'selected' : '' }}>Without Attachments Only</option>
                                    <option value="3" {{ @$ctrl_attachments == 3 ? 'selected' : '' }}>All</option>
                                </select>
                            </div>
        ...
        <div class="left-nav-list">
            ...
            <div class="table-responsive mb-4 mt-4">
                <table id="long-list" class="table table-hover d-none data-table" style="table-layout: fixed;width:100%">
                    ...
                    <thead>
                        <tr >
                            <th class="text-center" style="width:70px">@lang('PIV No')</th>
                            <th class="text-center" style="width:100px">@lang('PIV Date')</th>
                            <th style="width:100px">@lang('Supplier')</th>
                            <th style="width:100px">@lang('Customer')</th>

                            <th style="width:100px" class="text-end">@lang('Taxable Amount')</th>
                            <th style="width:100px" class="text-end">@lang('Tax')</th>
                            <th style="width:100px" class="text-end">@lang('Amount')</th>
                            <th class="text-center" style="width:70px">@lang('Deal ID')</th>
                            <th style="width:100px">@lang('Salesman')</th>
                            <th style="width:70px">@lang('Bill No')</th>
                            <th class="text-center" style="width:100px">@lang('Bill Date')</th>
                            <th class="text-center" style="width:70px">@lang('LPO&nbsp;No')</th>
                            <th class="text-center" style="width:70px">@lang('GRN&nbsp;No')</th>
                            <th class="text-center" style="width:70px">@lang('PRT&nbsp;No')</th>
                            <th class="text-center" style="width:70px">@lang('Currency')</th>
                            <th style="width:70px">@lang('Payment')</th>
                        ...
                        @foreach ($purchaseinvoice as $value)
                            ...
                            <tr @if (@$value->status == 2) class="bg-dark" @endif>
                                <td class="text-center"><a href="javascript:void(0)" onclick="list_style_new()"
                                        class="data-item" data-id="{{ $value->id }}">{{ @$value->doc_number }}</a>
                                </td>
                                <td class="text-center">{{ date('d/m/Y', strtotime(@$value->pi_date)) }}</td>
        ...
        : [];
                                ...
                                <td class="text-end">
                                    {{ @App\SysHelper::com_curr_format(@$value->total_taxableamount, 2, '.', ',') }}<?php $total_taxable_amount += $value->total_taxableamount; ?>
                                </td>
                                <td class="text-end">
                                    ...
                                    @else
                                        @foreach (explode(',', $value->grn_no) as $grn)
                                            <a href="javacript:void(0);" onclick="list_style()" class="grn-item"
                                                data-id="{{ @App\SysHelper::getGRNID($grn) }}">{{ trim($grn) }}</a>
                                            @if (!$loop->last)
                                                ,
                                            @endif
                                        @endforeach
                                ...
                                <td class="text-center">
                                    ...
                                    @else
                                        @foreach (explode(',', @$value->attach) as $att)
                                            <a href="{{ url(trim($att)) }}" target="_blank"><i class="ico icon-bold-paperclip"
                                ...
                                <td class="text-center">
                                       ...
                                       <a href="{{ url('purchase-invoice/' . $value->id . '?pi_action=edit') }}"
                                                    class="btn btn-sm btn-light " title="Comments">
                                                    <i class="ico icon-outline-pen-2" style="font-size: 16px;"></i>
                                                </a>

                                                <a href="{{ url('purchase-invoice/' . $value->id . '/download') }}"
                                                    class="btn btn-sm btn-light" title="Comments">
                                                ...
                                                @if (@$value->status == 2)
                                                    <a class="btn btn-light btn-sm"
                                                        href="{{url('purchase-invoice/'.$value->id.'/restore')}}"
                                                ...
                                                @else
                                                    <a class="btn btn-light btn-sm"
                                                        href="{{url('purchase-invoice/'.$value->id.'/delete')}}"
    ...
    <div class="content-container col-9">
        <div class="tab-content display-flex-tabs" id="purchaseOrderTabContent">
            ...
            <div class="" role="tabpanel" aria-labelledby="data-tab" id="data-details">
                @if ($action === 'add')
                @include('backEnd.purchaseinvoice.pi_add', is_array($addData) ? $addData : [])
                @elseif($action === 'edit')
                @include('backEnd.purchaseinvoice.pi_edit', is_array($editData) ? $editData : [])
                    {{-- @include('backEnd.purchaseorder.manage_purchase_order_edit', $editData) --}}
                @elseif (!empty($data) && is_array($data))
                    @include('backEnd.purchaseinvoice.pi_details', $data)
                @else
                    <div onclick="window.location.href='{{ url('purchase-invoice?pi_action=add') }}'"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#search_pi').on('input', function() {
                ...
                $.ajax({
                    url: "{{ route('purchase-invoice.search') }}",
                    type: "GET",
                    data: {
                        query: query
                    },
...
$(document).ready(function() {
    $('#exportExcelPurchaseInvoices').on('click', function(e) {
        ...
        var totalPI = @json($purchaseinvoice->count() ?? 0);
