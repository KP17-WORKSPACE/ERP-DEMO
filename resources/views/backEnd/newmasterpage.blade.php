...
{{-- Toastr End --}}
    ...
    <div class="venus-app page-purchase-order">
        ...
        <main class="show-nav">
            ...
            @elseif ($currentSubdomain === 'marketing')
                @include('backEnd.partials.sidenavmarketing')
            @elseif ($currentSubdomain === 'hrms1')
                @include('backEnd.partials.sidenavhrms')
            @else
                @include('backEnd.partials.sidenavnew')
            @endif
        ...
        @include('components.center-popup')
...
function pollNotifications() {
    $.get("{{ url('/notifications/pending') }}", function(data) {
        if (data.length > 0) {
            location.reload();
        }
    });
...
<script src="{{asset('public/backEnd/')}}/js/draggable-modal.js"></script>
