...
<script>
    $(document).ready(function() {
        ...
        autocomplete.addListener("place_changed", () => {
            ...
            setFieldvalue("mobile_code", place.international_phone_number || "");
            setFieldvalue("company_mobile_phone", place.international_phone_number || "");
...
{{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'customer-store', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'customer-save-form', 'novalidate' => true]) }}
...
<div class="tab-pane fade show active" id="purchase-order-1" role="tabpanel" aria-labelledby="purchase-order-1-tab">
    <div class="purchase-order-content-header d-flex align-items-center justify-content-between mb-1 gap-2">
        ...
        <div class="purchase-order-content-header-right d-flex align-items-center justify-content-end gap-2">
          ...
          <button type="submit" name="btnSubmit" value="createsupplier" class="btn btn-light">
                <i class="ico icon-outline-bookmark-opened text-warning"></i> Save & Create Supplier
            </button>
                ...
                <div class="col-12">
                    <div class="row gap-rows g-2">
                        ...
                        <div class="col-4">
                            ...
                            <!-- Hidden fields to store selected business data -->
                            ...
                            <input class="form-control capitalize-words" type="text" name="customer_name"
                        ...
                        <div class="col-4">
                            ...
                            <input class="form-control" type="text" name="customer_name_display"
                                id="customer_name_display" placeholder="" required>
                        </div>
                        ...
                        <div class="col-1">
                            ...
                            <select class="form-control js-example-basic-single" id="salutation_cust"
                        ...
                        <div class="col-1">
                            ...
                            <select class="form-select js-example-basic-single" style="width:30px"
                                ...
                                @foreach ($countries as $key => $value)
                                    <option value="{{ @$value->iso2 }}|{{ @$value->id }}" @if($value->id==@$currentCompanyCountry) selected @endif>{{ @$value->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <script>
                            $(document).ready(function() {
                                ...
                                $('#country_telephone').on('change', function() {
                                    ...
                                    var currentNumber2 = $('#company_mobile_phone').val().replace(/^\+\d+\s?/,
                                        ''); // remove previous code

                                    $('#mobile_code').val(code ? '+' + code + ' ' + currentNumber : currentNumber);
                                    $('#company_mobile_phone').val(code ? '+' + code + ' ' + currentNumber2 : currentNumber2);
                        ...
                        <div class="col-3">
                            ...
                            <input class="form-control" type="text" id="company_mobile_phone" name="mobile"
                                placeholder="">
                        </div>

                        <div class="col-3">
                            <label for="" class="form-label">Customer Email</label>
                            <input class="form-control" type="text" name="email" id="company_email"
                                placeholder="" required>
                        </div>
                        ...
                        <div class="col-3 mt-0">
                            ...
                            <div class="form-group">
                                <select class="form-control js-example-basic-single" id="company_designation"
                                    name="designation" required>
                                    <option value="">Select</option>
                                    @if (count($designation) > 0)
                                        @foreach ($designation as $val)
                                            <option value="{{ $val->title }}"
                                                @if (str_replace(' ', '', strtolower($val->title)) == str_replace(' ', '', strtolower('procurement manager'))) selected @endif>{{ $val->title }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select>
...
<div class="tab-wrap mb-3">
    ...
    <div class="tab-content mb-3" id="purchaseDetailsTabsContent">
                                    ...
                                    @if (count($department) > 0)
                                        @foreach ($department as $val)
                                            <option value="{{ $val->name }}"
                                                @if (str_replace(' ', '', strtolower($val->name)) == str_replace(' ', '', strtolower('procurement'))) selected @endif>{{ $val->name }}
                                            </option>
                                        @endforeach
                                    @endif
                                </select></td>
        ...
        <div class="tab-pane fade" id="document-details" role="tabpanel" aria-labelledby="document-details-tab">
            ...
            <div class="row">
                <div class="col-md-12">
                    <a class="btn btn-light float-end text-dark" style="cursor: pointer;" onclick="add_doc_row()"><i
                            class="ico icon-outline-add-square text-success" style="font-size:15px"
            ...
            <div class="row pb-2">
                <div class="col-md-3">
                    <input class="form-control" type="text" name="doc_name[]" value="Trade License" readonly />
                </div>
                <div class="col-md-3">
...
<div class="modal side-panel fade" id="addCityModal" data-bs-backdrop="false" tabindex="-1"
    aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-sm" style="height: 245px !important;">

        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="addCityModalLabel">Add City</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body m-0 p-0">
                ...
                <div class="card mb-0 mt-0">
                    <div class="card-body">
                        <div class="row">
                            @php
                                $modal_countries = @App\SysCountries::select('id', 'name')->get();
                                $modal_states = @App\SysStates::select('id', 'name')->get();
                            @endphp
                            <div class="col-12 mb-2">
                                <label class="form-label">City:</label>
                                <div class="form-group">
                                    <input class="form-control" type="text" name="modal_city" required>
                                </div>
                            </div>
                            <div class="col-12 mb-2">
                                <label class="form-label">Country:</label>
                                <div class="form-group">
                                    <select class="form-control js-example-basic-single" name="modal_country"
                                        id="modal_country" required>
                                        <option data-display="" value=""></option>
                                        @foreach ($modal_countries as $key => $value)
                                            <option value="{{ @$value->id }}">{{ @$value->name }} </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <div class="form-group">
                                    <select class="form-control js-example-basic-single" name="modal_state"
                                        id="modal_state" required>
                                        <option data-display="" value=""></option>
                                        @foreach ($modal_states as $key => $value)
                                            <option value="{{ @$value->id }}">{{ @$value->name }} </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-light add-btn ms-2" id="save_modal_btn">
                    <i class="ico icon-outline-bookmark-opened text-success"></i> Save
                </button>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#save_modal_btn').click(function(e) {
                e.preventDefault();

                $('#loading_bg').css('display', 'block');

                var city = $('input[name="modal_city"]').val();
                var country_id = $('#modal_country').val();
                var state_id = $('#modal_state').val();
                var token = '{{ csrf_token() }}';

                console.log("City:", city, "Country ID:", country_id, "State ID:", state_id);
                // Basic validation
                if (city == '' || country_id == '' || state_id == '') {
                    alert('Please fill all fields!');
                    return;
                }

                $.ajax({
                    url: '{{ route('modal.city.store') }}', // Laravel route
                    type: 'POST',
                    data: {
                        _token: token,
                        city: city,
                    ...
                    beforeSend: function() {
                        ...
                        $('#save_modal_btn').prop('disabled', true).text('Saving...');
                    },
                    success: function(response) {
                        ...
                        if (response.status == 'success') {
                            ...
                            $('input[name="modal_city"]').val('');
                            $('#modal_country').val('').trigger('change');
                            $('#modal_state').val('').trigger('change');
                        } else {
                    ...
                    complete: function() {
                        $('#save_modal_btn').prop('disabled', false).text('Save');
                        $('#loading_bg').css('display', 'none');
                    }
                });
            });
    ...
    function add_address() {
        ...
        $.ajax({
            ...
            success: function(dataResult) {
                ...
                var len = 0;
                console.log("dataresukt", dataResult);
                ...
                if (len > 0) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        console.log(flat_office_shop_no);
...
<script>
    $(document).ready(function() {
        ...
        FormValidator.init('customer-save-form', {
            showAllErrors: true,
            scrollToFirst: true,
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('keydown',
            '#customer-save-form input, #customer-save-form select, #customer-save-form button',
            function(e) {
                if (e.key === 'Enter') {
                    ...
                    var focusable = $form.find(':input:visible:not([disabled]):not([type=hidden])').filter(
                        ':not(button)');
