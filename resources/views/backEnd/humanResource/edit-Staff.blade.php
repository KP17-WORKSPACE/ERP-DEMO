...
@section('mainContent')
    ...
    <div class="content-container col-12">
        <div class="tab-content display-flex-tabs" id="purchaseOrderTabContent">
            ...
            <form id="staffAllForm" action="{{ route('staff.basic.update') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <!-- DATA DETAILS -->
                <div role="tabpanel" aria-labelledby="data-tab" id="data-details" class="tab-pane show active">
                    <div class="purchase-order-content-header sticky-top" style="background-color: #f7f8fd">
                        <h4 class="purchase-order-content-header-left">Edit ({{ $staffRow->staff_no }})
                    ...
                    <div class="card mb-3">
                        <div class="card-body">
                            ...
                            <div class="row gy-3 mt-1">
                                ...
                                <input type="hidden" name="staff_id" value="{{ $staffRow->id }}">
                                ...
                                <div class="col-lg-2">
                                    ...
                                    <select class="form-select form-select-sm js-example-basic-single" name="salutation">
                                        ...
                                        <option value="Mr."
                                            {{ old('salutation', $staffRow->employee_salutation ?? '') == 'Mr.' ? 'selected' : '' }}>
                                            Mr.</option>
                                        <option value="Mrs."
                                            {{ old('salutation', $staffRow->employee_salutation ?? '') == 'Mrs.' ? 'selected' : '' }}>
                                            Mrs.</option>
                                        <option value="Miss."
                                            {{ old('salutation', $staffRow->employee_salutation ?? '') == 'Miss.' ? 'selected' : '' }}>
                                            Miss.</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-lg-2">
                                    ...
                                    <input class="form-control form-control-sm" type="text" name="first_name"
                                        value="{{ old('first_name', $staffRow->first_name_full ?? '') }}" required>
                                </div>
                            ...
                            <div class="row row-cols-6 mt-4 spouse-section">
                                ...
                                <div class="col">
                                    <label class="form-label d-flex justify-content-between align-items-center">
                                        ...
                                        @php
                                            $spouseDoc = optional($staffRow->documents)->firstWhere(
                                                'key',
                                                'spouse_attachment',
                                            );
                                        @endphp

                                        @if (($spouseDoc && $spouseDoc->path) || !empty($staffRow->spouse_attachment))
                                            @php
                                                $spUrl = null;
                                                if ($spouseDoc && $spouseDoc->path) {
                                                    $spUrl = asset('storage/app/public/' . $spouseDoc->path);
                                                } elseif (!empty($staffRow->spouse_attachment)) {
                                                    if (strpos($staffRow->spouse_attachment, 'public/') === 0) {
                                                        $spUrl = asset($staffRow->spouse_attachment);
                                                    } elseif (
                                                        Storage::disk('public')->exists($staffRow->spouse_attachment)
                                                    ) {
                                                        $spUrl = asset(
                                                            'storage/app/public/' . $staffRow->spouse_attachment,
                                                        );
                                                    } elseif (file_exists(public_path($staffRow->spouse_attachment))) {
                                                        $spUrl = asset($staffRow->spouse_attachment);
                                                    }
                                                }
                                            @endphp
                                                                            ...
                                                                            $(document).ready(function() {
                                                                                ...
                                                                                $('#designation_id').on('change', function() {
                                                                                    ...
                                                                                    $.get("{{ url('designation') }}/" + id + "/grade")
                                                                                        .done(function(res) {
                                                                                            if (res && res.status) {
                                                                                                // Prefer the server-provided grade when the page first loads
                                                                                                var gradeToSet = initialDesignationGrade ? initialDesignationGrade : (res
                                                                                                    .grade || '');
                                                                                ...
                                                                                $('#designation_grade').on('change', function() {
                                                                                    ...
                                                                                    $.get("{{ url('hrms/reporting-managers') }}", {
                                                                                        ...
                                                                                        if (res && res.status == 'success') {
                                                                                            ...
                                                                                            // Preserve an initial reporting manager value (if present on page load)
                                                                                            if (initialReportingManager) {
                                                                                                var txt = initialReportingManagerText || 'Selected';
                                                                                                if ($('#reporting_manager option[value="' + initialReportingManager +
                                                                                                        '"]').length === 0) {
                                                                                                    $('#reporting_manager').append('<option value="' +
                                                                                                        initialReportingManager + '">' + txt + '</option>');
                                                                                                }
                                                            ...
                                                            <div class="accordion-body p-2">
                                                                <div class="row gy-1">
                                                                    ...
                                                                    @php
                                                                        ...
                                                                        $selectedBrands =
                                                                            old('brands') ?? ($job->brand_ids ?? []);
                                                                        if (!is_array($selectedBrands)) {
                                                                            $selectedBrands = $selectedBrands
                                                                                ? explode(',', $selectedBrands)
                                                                                : [];
                                                                        }
                                                                    ...
                                                                    <div class="col-lg-1" id="sales_target_div"
                                                                        ...
                                                                        <div class="input-effect">
                                                                            ...
                                                                            <select
                                                                                ...
                                                                                onchange="fn_role_id()">
                                                                                <option value="0"
                                                                                    {{ (string) $isTargetVal === '0' ? 'selected' : '' }}>
                                                                                    No</option>
                                                                                <option value="1"
                                                                                    {{ (string) $isTargetVal === '1' ? 'selected' : '' }}>
                                                                                    Yes</option>
                                                                            </select>
                                                                        </div>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-1" id="target_type_div"
                                                                        ...
                                                                        <select
                                                                            ...
                                                                            <option value=""
                                                                                {{ $targetTypeVal === '' ? 'selected' : '' }}>
                                                                                -Select-</option>
                                                                            <option value="revenue"
                                                                                {{ $targetTypeVal === 'revenue' ? 'selected' : '' }}>
                                                                                Revenue</option>
                                                                            <option value="gp"
                                                                                {{ $targetTypeVal === 'gp' ? 'selected' : '' }}>
                                                                                GP</option>
                                                                            <option value="both"
                                                                                {{ $targetTypeVal === 'both' ? 'selected' : '' }}>
                                                                                Both</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-2" id="brands_div"
                                                                        ...
                                                                        <div class="input-effect">
                                                                            ...
                                                                            <select
                                                                                ...
                                                                                <option value="all"
                                                                                    {{ in_array('all', $selectedBrands) ? 'selected' : '' }}>
                                                                                    All</option>
                                                                                @foreach ($brand_list as $value)
                                                                                    <option value="{{ $value->id }}"
                                                                                        {{ in_array((string) $value->id, array_map('strval', $selectedBrands)) ? 'selected' : '' }}>
                                                                                        {{ $value->title }}
                                                                                    </option>
                                                                                @endforeach
                                                                            </select>
                                                            ...
                                                            @if (!empty($eduRows) && $eduRows->count() > 0)
                                                                @foreach ($eduRows as $i => $edu)
                                                                    <tr class="education-row">
                                                                        ...
                                                                        <td>{{ $edu->gpa }}</td>
                                                                        <td>{{ $edu->mode }}</td>
                                                                        <td>{{ optional(App\SysCountries::find($edu->country))->name ?? ($edu->country ?? '') }}
                                                                        </td>
                                                                        <td>{{ (int) $edu->duration_years }} Years</td>
                                                                        <td>
                                                                            @if ($edu->certificate_path)
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table table-hover table-bordered align-middle"
                                                        ...
                                                        <tbody id="experienceTableBody">
                                                            @if (isset($expRows) && !empty($expRows) && $expRows->count() > 0)
                                                    ...
                                                    <div class="accordion-item">
                                                        ...
                                                        <div id="collapseResignationInfo"
                                                            ...
                                                            <div class="accordion-body">
                                                                @php
                                                                    $resMain = $eosData['main'] ?? null;
                                                                    $resNotice = $eosData['notice'] ?? null;
                                                                    $resHandover = $eosData['handover'] ?? null;

                                                                    $resignationType =
                                                                        old('resignation_type') ??
                                                                        optional($resMain)->resignation_type;
                                                                    $resignationReason =
                                                                        old('resignation_reason') ??
                                                                        optional($resMain)->reason_category;
                                                                    $resignationRemarks =
                                                                        old('resignation_remarks') ??
                                                                        optional($resMain)->detailed_reason;
                                                                    $resignationSubmitted =
                                                                        old('resignation_submitted_date') ??
                                                                        optional($resNotice)
                                                                            ->resignation_submitted_date;
                                                                    $noticePeriodDays =
                                                                        old('notice_period_days') ??
                                                                        optional($resNotice)->notice_period_days;
                                                                    $lastWorkingDay =
                                                                        old('last_working_day') ??
                                                                        optional($resNotice)->last_working_day;
                                                                    $relievingDate =
                                                                        old('relieving_date') ??
                                                                        optional($resNotice)->relieving_date;
                                                                    $knowledgeCompleted =
                                                                        old('knowledge_transfer_completed') ??
                                                                        optional($resHandover)
                                                                            ->knowledge_transfer_required;
                                                                    $handoverTo =
                                                                        old('handover_to') ??
                                                                        optional($resHandover)->handover_to;
                                                                    $handoverNotes =
                                                                        old('handover_notes') ??
                                                                        optional($resHandover)->handover_notes;
                                                                    // Map stored clearance_status (pending|completed) back to yes/no for the assets_returned field
                                                                    $assetsReturned =
                                                                        old('assets_returned') ??
                                                                        (isset($eosData['asset_clearance'])
                                                                            ? (optional($eosData['asset_clearance'])
                                                                                ->clearance_status == 'completed'
                                                                                ? 'yes'
                                                                                : 'no')
                                                                            : '');
                                                                    $resignationStatus =
                                                                        old('resignation_status') ??
                                                                        optional($resMain)->status;
                                                                @endphp
                                                                ...
                                                                <div class="row gy-3">
                                                                    ...
                                                                    <div class="col-lg-2">
                                                                        ...
                                                                        <select class="form-select form-select-sm"
                                                                            ...
                                                                            <option value=""
                                                                                {{ $resignationType == '' ? 'selected' : '' }}>
                                                                                Select Type</option>
                                                                            <option value="voluntary"
                                                                                {{ strtolower($resignationType) == 'voluntary' ? 'selected' : '' }}>
                                                                                Voluntary</option>
                                                                            <option value="involuntary"
                                                                                {{ strpos(strtolower($resignationType), 'invol') !== false ? 'selected' : '' }}>
                                                                                Involuntary /
                                                                                Termination</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-2">
                                                                        ...
                                                                        <select class="form-select form-select-sm"
                                                                            ...
                                                                            <option value=""
                                                                                {{ $resignationReason == '' ? 'selected' : '' }}>
                                                                                Select Reason
                                                                            </option>
                                                                            <option value="personal"
                                                                                {{ strtolower($resignationReason) == 'personal' ? 'selected' : '' }}>
                                                                                Personal</option>
                                                                            <option value="better_opportunity"
                                                                                {{ strtolower($resignationReason) == 'better_opportunity' || strtolower($resignationReason) == 'better opportunity' ? 'selected' : '' }}>
                                                                                Better Opportunity
                                                                            </option>
                                                                            <option value="health"
                                                                                {{ strtolower($resignationReason) == 'health' ? 'selected' : '' }}>
                                                                                Health</option>
                                                                            <option value="relocation"
                                                                                {{ strtolower($resignationReason) == 'relocation' ? 'selected' : '' }}>
                                                                                Relocation</option>
                                                                            <option value="other"
                                                                                {{ strtolower($resignationReason) == 'other' ? 'selected' : '' }}>
                                                                                Other</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-8">
                                                                        ...
                                                                        <textarea class="form-control form-control-sm" name="resignation_remarks" rows="3"
                                                                            placeholder="Additional details about resignation">{{ $resignationRemarks }}</textarea>
                                                                    </div>
                                                                ...
                                                                <div class="row gy-3">
                                                                    ...
                                                                    <div class="col-lg-2">
                                                                        ...
                                                                        <select class="form-select form-select-sm"
                                                                            ...
                                                                            <option value=""
                                                                                {{ $knowledgeCompleted == '' ? 'selected' : '' }}>
                                                                                Select Status
                                                                            </option>
                                                                            <option value="yes"
                                                                                {{ strtolower($knowledgeCompleted) == 'yes' ? 'selected' : '' }}>
                                                                                Yes</option>
                                                                            <option value="no"
                                                                                {{ strtolower($knowledgeCompleted) == 'no' ? 'selected' : '' }}>
                                                                                No</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-2">
                                                                        ...
                                                                        <select class="form-select form-select-sm"
                                                                            ...
                                                                            <option value=""
                                                                                {{ $assetsReturned == '' ? 'selected' : '' }}>
                                                                                Select Status
                                                                            </option>
                                                                            <option value="yes"
                                                                                {{ strtolower($assetsReturned) == 'yes' ? 'selected' : '' }}>
                                                                                Yes</option>
                                                                            <option value="no"
                                                                                {{ strtolower($assetsReturned) == 'no' ? 'selected' : '' }}>
                                                                                No</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="col-lg-4">
                                                                        ...
                                                                        <textarea class="form-control form-control-sm" name="handover_notes" rows="3"
                                                                            placeholder="Details of knowledge transfer and handover process">{{ $handoverNotes }}</textarea>
                                                                    </div>
                                                                ...
                                                                <div class="row gy-3">
                                                                    ...
                                                                    <div class="col-lg-2">
                                                                        ...
                                                                        <select class="form-select form-select-sm"
                                                                            ...
                                                                            <option value="draft"
                                                                                {{ strtolower($resignationStatus) == 'draft' ? 'selected' : '' }}>
                                                                                Draft</option>
                                                                            <option value="submitted"
                                                                                {{ strtolower($resignationStatus) == 'submitted' ? 'selected' : '' }}>
                                                                                Submitted</option>
                                                                            <option value="approved"
                                                                                {{ strtolower($resignationStatus) == 'approved' ? 'selected' : '' }}>
                                                                                Approved</option>
                                                                            <option value="withdrawn"
                                                                                {{ strtolower($resignationStatus) == 'withdrawn' ? 'selected' : '' }}>
                                                                                Withdrawn</option>
                                                                            <option value="completed"
                                                                                {{ strtolower($resignationStatus) == 'completed' ? 'selected' : '' }}>
                                                                                Completed</option>
                                                                        </select>
                                                                    </div>
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.photo.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.cv.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.passport_visa.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.offer_letter.remarks', $existing->remarks ?? '') }}">

                                                                </td>
                                                            </tr>

                                                            <tr>
                                                                <td>Bank Account Details (IBAN Letter)</td>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @if ($existing && $existing->path)
                                                                        <input type="hidden"
                                                                            name="docs[joining][iban_letter][existing]"
                                                                            value="{{ $existing->path }}">
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td><input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.iban_letter.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @if ($existing && $existing->path)
                                                                        <input type="hidden"
                                                                            name="docs[joining][prof_certs][existing]"
                                                                            value="{{ $existing->path }}">
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td><input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.prof_certs.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @if ($existing && $existing->path)
                                                                        <input type="hidden"
                                                                            name="docs[joining][academic][existing]"
                                                                            value="{{ $existing->path }}">
                                                                    @endif
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.academic.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.medical_fit.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <input type="text" class="form-control"
                                                                        ...
                                                                        value="{{ old('docs.joining.emp_contract.remarks', $existing->remarks ?? '') }}">
                                                                </td>
                                                            </tr>
                                                ...
                                                <div class="table-responsive mb-3">
                                                    <table class="table table-bordered align-middle">
                                                        ...
                                                        <tbody>
                                                            ...
                                                            @endphp
                                                            @php $employmentDocs = $docRows->where('group', 'employment')->keyBy('key'); @endphp
                                                            @foreach ($empDocs as $doc)
                                                                @php $existing = $employmentDocs[$doc['key']] ?? null; @endphp
                                                                <tr>
                                                                    <td>{{ $doc['label'] }}</td>
                                                                    <td>
                                                                        <input type="text" class="form-control"
                                                                            ...
                                                                            value="{{ old('docs.employment.' . $doc['key'] . '.number', $existing->document_number ?? '') }}">
                                                                    </td>
                                                                    <td>
                                                                        @if ($existing && $existing->path)
                                                                            @php $fileName = basename($existing->path); @endphp
                                                                        ...
                                                                        @if ($existing && $existing->path)
                                                                            <input type="hidden"
                                                                                name="docs[employment][{{ $doc['key'] }}][existing]"
                                                                                value="{{ $existing->path }}">
                                                                        @endif
                                                                    </td>
                                                                    <td>
                                                                        <input type="text" class="form-control"
                                                                            ...
                                                                            value="{{ old('docs.employment.' . $doc['key'] . '.remarks', $existing->remarks ?? '') }}">
                                                                    </td>
                                                                </tr>
                                                            @endforeach
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table table-bordered align-middle"
                                                        ...
                                                        <tbody>
                                                            ...
                                                            @endphp
                                                            @php
                                                                $existingOthers = $docRows
                                                                    ->where('group', 'others')
                                                                    ->values();
                                                            @endphp

                                                            {{-- Existing 'Others' documents (from DB) --}}
                                                            @foreach ($existingOthers as $i => $other)
                                                                <tr>
                                                                    <td><input type="text" class="form-control"
                                                                            name="docs[others][existing_{{ $i }}][name]"
                                                                            value="{{ old('docs.others.existing_' . $i . '.name', $other->name ?? '') }}">
                                                                    </td>
                                                                    <td><input type="text" class="form-control"
                                                                            name="docs[others][existing_{{ $i }}][number]"
                                                                            value="{{ old('docs.others.existing_' . $i . '.number', $other->document_number ?? '') }}">
                                                                    </td>
                                                                    <td>
                                                                        @if ($other->path)
                                                                            @php $fileName = basename($other->path); @endphp
                                                                            <div class="mb-1"><a
                                                                                    href="{{ asset('storage/app/public/' . $other->path) }}"
                                                                                    target="_blank">{{ $fileName }}</a>
                                                                            </div>
                                                                        @endif
                                                                        <input type="file" accept=".pdf,image/*"
                                                                            class="form-control"
                                                                            name="docs[others][existing_{{ $i }}][file]">
                                                                        @if ($other->path)
                                                                            <input type="hidden"
                                                                                name="docs[others][existing_{{ $i }}][existing]"
                                                                                value="{{ $other->path }}">
                                                                        @endif
                                                                    </td>
                                                                    <td><input type="text" class="form-control"
                                                                            name="docs[others][existing_{{ $i }}][remarks]"
                                                                            value="{{ old('docs.others.existing_' . $i . '.remarks', $other->remarks ?? '') }}">
                                                                    </td>
                                                                    <td class="text-center">
                                                                        <button type="button"
                                                                            class="btn btn-light text-dark btn-sm delOtherRow"><i
                                                                                class="ico icon-outline-trash-bin-minimalistic"></i></button>
                                                                    </td>
                                                                </tr>
                                                            @endforeach

                                                            {{-- Preset optional rows --}}
                                                            {{-- @foreach ($otherPreset as $k => $row)
                                                                <tr>
                                                                    <td><input type="text" class="form-control" name="docs[others][{{ $k }}][name]" value="{{ $row[0] }}">
                                                                    </td>
                                                                    <td><input type="text" class="form-control" name="docs[others][{{ $k }}][number]" value="">
                                                                    </td>
                                                                    <td><input type="file" accept=".pdf,image/*" class="form-control" name="docs[others][{{ $k }}][file]">
                                                                    </td>
                                                                    <td><input type="text" class="form-control" name="docs[others][{{ $k }}][remarks]" value="{{ $row[1] }}">
                                                                    </td>
                                                                    <td class="text-center">
                                                                        <button type="button" class="btn btn-light text-dark btn-sm delOtherRow">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(function() {
                ...
                // Show a listing when user picks a file in the visible IBAN docs input (manual selection)
                ...
                // ---- ACADEMIC DOCS: manual / duplicated handlers ----
                // Allow user to remove a duplicated academic doc created from Education add
                ...
                // Show list when user selects manual academic docs
                ...
                // ---- PROF CERTS: manual / duplicated handlers ----
                // Allow user to remove a duplicated prof cert doc created from Experience add
                ...
                // Show list when user selects manual prof cert files
                ...
                // Remove a manual prof cert selected in the visible input (rebuild FileList without that index)
              ...
              $('#main_company').on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        /* ---------------------------
                           Set shift dropdown (preserve existing selection when possible)
                        --------------------------- */
                        const $shiftSelect = $('#shift_id');
                        const prevShift = $shiftSelect.val();
                        ...
                        if (res.shifts && res.shifts.length) {
                            ...
                            // Restore previous selection if still available, otherwise only auto-select the first if there was no prior selection
                            const hasPrev = prevShift && res.shifts.some(s => String(s.id) === String(prevShift));
                            if (prevShift && hasPrev) {
                                $shiftSelect.val(prevShift).trigger('change');
                            } else if (!prevShift && res.shifts[0]) {
                                $shiftSelect.val(res.shifts[0].id).trigger('change');
                            }

                        } else {
                            console.log('No shifts returned from server. Leaving current selection untouched to avoid overwriting user choice.');
                           // 🔴 No shifts → make dropdown blank
    $shiftSelect.empty();

    // Optional placeholder (remove this line if you want it 100% blank)
    ...
    $shiftSelect.val('').trigger('change');
                        ...
                        const prevWeekly = $weeklyOffSelect.val() || [];
            ...
            // Prevent double-submit on the main staff form
            // NOTE: Do not disable the submit button on edit pages (forms containing staff_id)
            $('#staffAllForm').on('submit', function() {
                var $btn = $(this).find('button[type="submit"]');
                // If this is an edit/update (has staff_id), skip disabling the button so Update stays enabled
                var isEdit = $(this).find('input[name="staff_id"]').length > 0;
                if ($btn.length && !isEdit) {
    ...
    <script>
        // Mirror staff photo into docs[joining][photo][file] in edit form and show preview
        $(document).ready(function() {
            ...
            function clearStaffPreview() {
                ...
                if (staffPreview) {
                    // reset to original image or hide if none
            ...
            if (staffInput && joiningInput) {
                ...
                $(joiningInput).on('change', function() {
                    if (joiningInput.files && joiningInput.files.length > 0) {
                        ...
                        // keep staff preview in sync when user chooses via joining input
