...
{{-- 
        @if ($employee->approved_by)
                                 <a
                                    class="btn btn-light text-dark" href="{{ url('staff-directory/'.$employee->staff_rec_id) }}">
 ...
 <a href="{{ url('onboarding-employee-edit/' . $employee->id) }}" class="btn btn-light text-dark">
        ...
        <div class="dropdown">
            ...
            aria-labelledby="religionAddModalLabel" aria-hidden="true" id="approveConfirmModal">
                                <div class="modal-dialog modal-md modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <p class="mb-2">Are you sure you want to <strong>approve</strong> the following onboarding?</p>
                                            <div class="border rounded p-3 bg-light">
                                                <div><strong>Applicant</strong>: <span class="modal-employee-name">{{ $employee->full_name }}</span></div>
                                                <div><strong>Document</strong>: <span class="modal-document-number">{{ $employee->document_number }}</span></div>
                                                <div class="text-muted mt-2">Approving will create the staff record and notify relevant teams.</div>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                                            <button type="button" id="confirmApproveBtn" class="btn btn-light">
                                                <span class="btn-text">Yes, approve</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <script>
                                (function() {
                                    // Show confirmation modal and store target URL
                                    $(document).on('click', '.approve-btn', function(e) {
                                        e.preventDefault();
                                        var url = $(this).data('url');
                                        var modal = $('#approveConfirmModal');
                                        modal.data('url', url);
                                        // update name/document inside modal (in case view lists different employee)
                                        modal.find('.modal-employee-name').text('{{ $employee->full_name }}');
                                        modal.find('.modal-document-number').text('{{ $employee->document_number }}');
                                        var bsModal = new bootstrap.Modal(modal[0], { keyboard: false });
                                        bsModal.show();
                                    });

                                    // Confirm approve: call endpoint via AJAX
                                        ...
                                        $.ajax({
                                            ...
                                            success: function(res) {
                                                if (res.ok) {
                                                    ...
                                                    // close modal
                                                    var bs = bootstrap.Modal.getInstance(modal[0]);
                                                    if (bs) bs.hide();
                                                    ...
                                                    toastr.error(res.message || 'Failed to approve onboarding.');
                                                    $btn.prop('disabled', false).html(oldHtml);
                                                }
                                            },
                                            error: function(xhr) {
                                                var msg = 'An error occurred while approving onboarding.';
            ...
            <ul class="dropdown-menu" style="">
                ...
                {{-- @if ($employee->approved_by != null || $employee->approved_by != 0)
                @else
                    <form action="{{ route('onboard-employee.approve', ['id' => $employee->id]) }}" method="POST">
                        ...
                        <button type="submit" value="approve"
                            ...
                            <i class="ico icon-outline-check-circle text-success title-15 me-2"></i>
                            Approve
                        </button>
                    </form>
                @endif --}}
                ...
                <li><a href="{{ url('staff-directory') }}" class="dropdown-item d-flex align-items-center text-dark"><i
                ...
                <div class="staff-photo d-flex align-items-center justify-content-center rounded-circle overflow-hidden bg-light"
                    ...
                    @php
                        ...
                        if (empty($employee->staff_photo)) {
                            $documentPhoto = optional($employee->documents)->firstWhere('key', 'photo')
                                ?? optional($employee->documents)->firstWhere('group', 'joining');
                        }
                    @endphp
                    @if (!empty($employee->staff_photo))
                        <img src="{{ asset('storage/app/public/' . ltrim($employee->staff_photo, '/')) }}" alt="Staff Photo"
                            style="width:100%;height:100%;object-fit:cover;display:block;">
                    @elseif (!empty($documentPhoto->path))
                        <img src="{{ asset('storage/app/public/' . ltrim($documentPhoto->path, '/')) }}" alt="Staff Photo"
                            style="width:100%;height:100%;object-fit:cover;display:block;">
                    @else
                ...
                <div class="row row-cols-5">
                    <div class="col">
                        ...
                        <div class="form-control-plaintext truncate-text-custom">
                          {{ $employee->employee_salutation }}  {{ $employee->first_name ?? '' }}
                        </div>
                    </div>
...
<style>
    ...
    #employee-details .form-control-plaintext {
        text-align: center !important;
    }
</style>
                ...
                <div class="col-10">
                    <div class="row row-cols-5">
                        ...
                        <div class="col">
                            ...
                            <div class="form-control-plaintext truncate-text-custom">{{ optional($job)->work_hours ?: '—' }}</div>
                        </div>
                        ...
                        <div class="col">
                            ...
                            <div class="form-control-plaintext truncate-text-custom">
                                @php
                                    $weeklyRaw = optional($job)->week_off ?? null;
                                    $weeklyArr = [];
                                    if ($weeklyRaw) {
                                        if (is_array($weeklyRaw)) {
                                            $weeklyArr = $weeklyRaw;
                                        } elseif (is_string($weeklyRaw)) {
                                            $weeklyArr = array_filter(array_map('trim', explode(',', $weeklyRaw)));
                                        }
                                    }

                                    $weeklyMap = [
                                        'monday_all' => 'Monday (All)',
                                        'tuesday_all' => 'Tuesday (All)',
                                        'wednesday_all' => 'Wednesday (All)',
                                        'thursday_all' => 'Thursday (All)',
                                        'friday_all' => 'Friday (All)',
                                        'saturday_all' => 'Saturday (All)',
                                        'sunday_all' => 'Sunday (All)',
                                        '1_3_monday' => '1 & 3 Monday (Only 1 & 3)',
                                        '2_4_monday' => '2 & 4 Monday (Only 2 & 4)',
                                        '1_3_tuesday' => '1 & 3 Tuesday (Only 1 & 3)',
                                        '2_4_tuesday' => '2 & 4 Tuesday (Only 2 & 4)',
                                        '1_3_wednesday' => '1 & 3 Wednesday (Only 1 & 3)',
                                        '2_4_wednesday' => '2 & 4 Wednesday (Only 2 & 4)',
                                        '1_3_thursday' => '1 & 3 Thursday (Only 1 & 3)',
                                        '2_4_thursday' => '2 & 4 Thursday (Only 2 & 4)',
                                        '1_3_friday' => '1 & 3 Friday (Only 1 & 3)',
                                        '2_4_friday' => '2 & 4 Friday (Only 2 & 4)',
                                        '1_3_saturday' => '1 & 3 Saturday (Only 1 & 3)',
                                        '2_4_saturday' => '2 & 4 Saturday (Only 2 & 4)',
                                        '1_3_sunday' => '1 & 3 Sunday (Only 1 & 3)',
                                        '2_4_sunday' => '2 & 4 Sunday (Only 2 & 4)',
                                    ];

                                    $weeklyLabels = [];
                                    foreach ($weeklyArr as $w) {
                                        if (isset($weeklyMap[$w])) {
                                            $weeklyLabels[] = $weeklyMap[$w];
                                        } else {
                                            $weeklyLabels[] = ucwords(str_replace(['_', '-'], ' ', $w));
                                        }
                                    }
                                @endphp
