...
@section('mainContent')
    ...
    <?php try { ?>
        <div class="container-fluid">
            <div class="d-sm-flex justify-content-between">
                <div class="mb-3">
                    <h2 class="page-heading m-0">Cash Payment</h2>
                    <span class="page-label">Home - Cash Payment</span>
                </div>
                <div>
                    <a href="{{ url('cashpayment-add') }}" type="button" class="btn btn-info"><i class="fa fa-plus"></i> New</a>
                    <a href="{{ url('cashpayment') }}" type="button" class="btn btn-info"><i class="fa fa-list"></i> List</a>
                </div>
            </div>
            ...
            <div class="card p-4 mb-2">
                ...
                @if(isset($editData))
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'cashpayment-update/'. @$editData->id, 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'id' => 'cashpayment-create-form']) }}
                <input type="hidden" value="{{@$editData->id}}" name="cust_id">
                @else
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'cashpayment-store', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'cashpayment-create-form']) }}
                @endif
    
                <input type="hidden" id="cashpayment_process_id" name="process_id" value="{{Auth::user()->id . date("YmdHis")}}">
                    ...
                    <div class="col-lg-12">
                      <div class="white-box">
                                    ...
                                    <div class="no-gutters input-right-icon">
                                        <div class="col">
                                            <div class="input-effect">
                                                ...
                                                <input class="form-control" type="text" id="doc_number" name="doc_number" value="{{isset($editData)?@$editData->doc_number:'CP-'.sprintf('%03d', @App\SysCashPayment::max('id') + 1) }}" readonly>
                                                <span class="focus-border"></span>
                                        ...
                                        <div class="col-auto" style="display: none;">
                                            ...
                                            <script>
                                                function fn_cr_search_btn()
                                                {
                                                    ...
                                                    window.location.href = url+"/cashpayment/"+cr_search;
                                                }
                                            </script>
                                        </div>
                                    </div>
                                    ...
                                    <div class="input-effect">
                                        ...
                                        <select
                                            ...
                                            @foreach ($receiptmode as $val)
                                                <option value="{{ @$val->id }}"
                                                    @if(isset($editData)) @if(@$editData->payment_mode == @$val->id) selected @endif @endif
                            ...
                            <div class="equipment comon-status row mt-4 d-block">
                                <table class="table table-bordered table-striped" id="cp-table" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        <td>{{$roid}}</td>
                                            ...
                                            <td>
                                                <input class="form-control" type="number" id="amount_{{$roid}}" name="amount[]" autocomplete="off" min="0" value="{{@$editDataList[$roid-1]->debit_amount}}" onchange="calc_total()" onkeypress="cp_popup_fun({{$roid}})">
                                            </td>
                                            <td>
                                        ...
                                        @endfor
                                        <?php $roid--;?>
                                        <input type="hidden" id="cp-row-count" value="{{$roid}}">
                                        <a data-modal-size="modal-md" data-target="#cp_popup_win" id="addCtrlCashPaymentAdjest" data-toggle="modal"></a>
                                        <script>                                        
                                ...
                                <div style="display: none;">
                                    @if(!isset($view))
                                        <button type="button" class="primary-btn small fix-gr-bg" id="addRowCP"><span class="ti-plus pr-2"></span>@lang('lang.item')</button>
                                    @endif
                                </div>
    ...
    function fn_addRow(id) {
        var rownum = document.getElementById('cp-row-count').value;
        if(id==rownum) {
         document.getElementById('cp-row-count').value = (Number(rownum) + Number(1));
            document.getElementById('addRowCP').click();
        }
    }
    
    function calc_total(){
        var countrow = document.getElementById('cp-row-count').value;
        var t1=0;
                    ...
                    <div class="row mt-40">
                        <div class="col-lg-12 text-center">
                            ...
                            <button class="btn btn-primary">
                                ...
                                @if(isset($editData)) @lang('lang.update') @else @lang('lang.add') @endif @lang('Cash Payment')
                            </button>
                            @endif
                        </div>
                    </div>
    ...
    <form id="ta" >
    <div class="modal fade admin-query" id="cp_popup_win">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 80%;">
            <div class="modal-content">
                ...
                <div class="modal-body m-0 p-3">
                    <input type="hidden" id="cp_account_id">
                    <input type="hidden" id="cp_account_id_amount">
                    <div class="container-fluid">
                        <div class="row">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="equipment comon-status row mt-40 d-block">
                                    <table class="sstable" cellspacing="0" width="100%" id="cpListCashPaymentAdjest">
                                        <thead>
                                            <tr>
...
<script>
    function CashPaymentAdjestBalance(id) {
    ...
    function validateCashPaymentAdjestForm(id) {
    ...
    var account_id = $('#cp_account_id').val();
    var entry_date = $('#doc_date').val();
    var transaction_type ='cashpayment';
    var entry_type =1; //1 Debit, 2 Credit
    var process_id = $('#cashpayment_process_id').val();
    ...
    $(".btn_ajax_cp").prop('disabled', true);
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            {
                $("#btn_close2").click();
                $("#addCtrlCashPaymentAdjest").click();
            }
        },
    ...
    function cp_popup_fun(id)
    {
        var txtbx = $('#amount_'+id);
        txtbx.keypress(function (e) {

        var key = e.which;
        if(key === 13)  // the enter key code
            {
                var cp_account_id = $('#account_id_'+id+'').val();
                var cp_account = $('#amount_'+id+'').val();

                if(cp_account_id != "" && cp_account != ""){
                    $('#cp_account_id').val(cp_account_id);
                    $('#cp_account_id_amount').val(cp_account);
                    $('#bi_adjusted_amount').val(cp_account).focus();
