...
<body>
  ...
  <main class="m2">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td align="left"><img  src="{!! asset($company->company_logo) !!}" width="200px"/></td>
        <td align="right"><b style="font-size: 20px; font-family: 'Lucida Sans', 'Lucida Sans Regular', 'Lucida Grande', 'Lucida Sans Unicode', Geneva, Verdana, sans-serif; font-weight: 400;">TAX INVOICE</b></td>
    </tr>
  </table>
  <br />
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td  width="30%" valign="top" style="line-height: 18px;">
            ...
            Invoice No: {{@$si->doc_number}}<br>
            Date: {{date('d/m/Y', strtotime(@$si->doc_date))}}<br>
            Ref No: {{@$si->lpo_number}}<br>
            Ref Date: {{date('d/m/Y', strtotime(@$si->lpo_date))}}<br>
            Payment Terms: {{ $si->paymentterms->title }} {{ $si->payment_terms2 }}<br>
