...
{{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'stock-out/' . $edit->id, 'method' => 'PUT', 'id' => 'tender-create-form']) }}
                ...
                <div class="col-1-5">
                    ...
                    <div class="form-group">
                        <select class="form-control js-example-basic-single" name="mode" id="mode">
                            ...
                            <option value="SH" @if ($mode == 'SH') selected @endif>Shortage Stock
                            </option>
                            <option value="DO" @if ($mode == 'DO') selected @endif>Demo Out</option>
                            <option value="RO" @if ($mode == 'RO') selected @endif>RMA Out</option>
                ...
                <script>
                    $(document).ready(function() {
                        function toggleRMAFields() {
                            let mode = $('#mode').val();
                            // Always reset first
                            $('#RO_DOC, #DO_DOC, #EO_DOC').addClass('d-none');
        ...
        <table class="table table-hover form-item-table" id="myTable">
            ...
            <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input class="form-control" type="text" name="product_type[]" autocomplete="off"
                                    value="{{ $dt->product_type }}" readonly="true" hidden>
                            ...
                            <td><input class="form-control text-end serial-no-column" value="{{ $dt->serialno }}"
                ...
                <tr>
                    ...
                    <td><input class="form-control serial-no-column text-end" type="text" name="serial_no[]"></td>
                    <td><input class="form-control text-end" id="reference" type="text" name="narration[]"></td>
                </tr>
...
<script>
    var stockOutProductTypes = @json($items->pluck('product_type', 'id'));

    function getStockOutRowProductType($row) {
        ...
        productType = parseInt(stockOutProductTypes[partId], 10);
    ...
    function getStockOutLicenseKeyCount($row) {
        var seen = {};
        var count = 0;
    ...
    function stockOutLicenseAppendSelectedKeysToSerial() {
        ...
        if ($inp && $inp.length) {
            $inp.val(keys.join(', '));
            applyStockOutLicenseQtyState($inp.closest('tr'));
        }
    }
    ...
    function set_license_key_normal(e, el) {
        ...
        if (getStockOutRowProductType($row) !== 2) return true;
    ...
    function get_license_key(part_id) {
        ...
        if (stockOutId > 0) requestData.stock_out_id = stockOutId;
        ...
        $.ajax({
            ...
            success: function(dataResult) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    var docUrl = '';
                    if (docNo) {
                        if (isSalesReturn) docUrl = srDocUrlBase + "/" + encodeURIComponent(docNo);
                        else if (isStockIn) docUrl = stkinEditUrlBase + "/" + encodeURIComponent(docNo);
                        else if (isOpeningStock) {
                            var opsId = parseInt(row.opening_stock_id, 10) || 0;
                            if (opsId > 0) docUrl = opsEditUrlBase + "/" + opsId + "/edit";
                        } else docUrl = grnDocUrlBase + "/" + encodeURIComponent(docNo);
                    }
    ...
    function key_select_change(id) {
        ...
        $(".chk_key").each(function() {
            if (this.checked) {
                ...
                if (itm_id == 0) itm_id = $('#item_key_id_' + b).val();
                else itm_id += ',' + $('#item_key_id_' + b).val();
            }
            b++;
        });
    ...
    function set_license_key() {
        ...
        var requestData = {
            ...
            item_id: $('#part_no').val(),
            qty_limit: qtyLimit,
            stock_out_id: parseInt($('#stock_out_id').val() || 0, 10),
            keep_stock_out_key_ids: keepStockOutKeyIds.join(',')
        };
        $.ajax({
    ...
    $(document).ready(function() {
        ...
        $(document).on('shown.bs.modal', '#ModalLicenseKey', function() {
            ...
            if (partId) {
                ...
                if ($sel.length) {
                    stockOutLicenseSetSerialTargetFromRow($sel.closest('tr'));
                }
            }
            setTimeout(function() {
            ...
            if (e.key === "Enter") {
                ...
                var name = $(this).attr("name");
                 if (name === "unitprice[]") {
                    ...
                    if (nextRow.length) {
                        var partNumberSelect = nextRow.find('select[name="part_number[]"]');
    ...
    $(function() {
        ...
        $form.on('change input', 'table#myTable tbody input[name="qty[]"], table#myTable tbody input[name="serial_no[]"]', function() {
            applyStockOutLicenseQtyState($(this).closest('tr'));
        });
        ...
        $form.on('submit.stockOutLicenseQtyValidation', function(e) {
            if (validateStockOutLicenseQuantities($form)) return true;

            e.preventDefault();
            e.stopImmediatePropagation();
            $table.find('input[name="qty[]"].license-qty-invalid').first().focus();
            var message = 'Select license keys equal to Qty for every red item before updating.';
            if (window.toastr) {
