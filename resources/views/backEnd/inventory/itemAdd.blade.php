                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-2 mb-2 ">
                                ...
                                <input class="form-control" type="hidden" id="part_number_array1">
                              ...
                              <script>
                                    $(document).ready(function() {
                                        ...
                                        $(document).on('click', 'li', function() {
                                            var current = $('#part_number1').val(); // existing input value
                                            ...
                                            $('#part_number1').val(finalVal); // update visible input
                                            $('#part_number_array1').val(finalVal); // update hidden field
                                    ...
                                    <div class="form-group">
                                        <select class="form-control" name="filter_product_type" id="filter_product_type">
                                            ...
                                            @foreach ($producttype as $key => $value)
                                                <option @if ($ctrl_product_type == $value->id) selected @endif value="{{ @$value->id }}">{{ @$value->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="col-12 filter-field d-none">
                ...
                <table id="long-list" class="table table-hover data-table" style="table-layout: fixed;width:100%">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-start" width="125px">Type</th>
                            <th class="text-center" width="70px">Status</th>
                            <th class="text-start" width="170px">Created By</th>
                            <th class="text-start" width="170px">Updated By</th>
                        ...
                        @if (isset($items))
                            @foreach ($items as $value)
                                <tr>
                                    ...
                                    <td>{{ App\SmItem::getProductType(@$value->product_type) }}</td>
                                    ...
                                    <input type="hidden" id="created_at_{{ @$value->id }}"
                                        value="{{ date('d/m/Y h:i A', strtotime(@$value->created_at)) }}">

                                    @php $cb2 = $user_list->where('user_id',$value->updated_by)->max('full_name'); @endphp

                                    <input type="hidden" id="updated_by_{{ @$value->id }}"
                                        value="{{ @$cb2 }}">
                                    <input type="hidden" id="updated_at_{{ @$value->id }}"
                                        value="{{ date('d/m/Y h:i A', strtotime(@$value->updated_at)) }}">
...
<script>
    window.shouldRefreshAfterProductAdd = true;
</script>
    ...
    $(document).ready(function () {
        $('#exportExcelItems').on('click', function (e) {
            ...
            var totalItems = @json($items->count() ?? 0);
            ...
            $table.find('thead tr th').each(function (i) {
                ...
                if ($(this).css('display') !== 'none') {
                    ...
                    if (['actions', 'action'].includes(label.toLowerCase().trim())) return;
            ...
            rows.push(['Product List (' + totalItems + ')']);
            if (dateFrom || dateTo) {
                var parts = [];
            ...
            workbook.xlsx.writeBuffer().then(function (buffer) {
                ...
                var d = new Date();
                var filename = 'products_' + pad(d.getDate()) + '-' + pad(d.getMonth() + 1) + '-' + d.getFullYear() + '.xlsx';
