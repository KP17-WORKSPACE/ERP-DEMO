...
@section('mainContent')
    ...
    <aside class="left-nav col-3" id="leftSidebar">
        ...
        <div class="short-list" id="filters-short">
            <h4 class="mb-2">Excess Stock (Stock In)
            </h4>

            
            <div class="search-filter-container mb-4">
                <div class="input-group flex-nowrap">
                    <input type="text" name="documents_number" id="search_stockin" class="form-control"
        ...
        <div class="long-list  d-none" id="filters-long">
            <div class="d-flex align-items-center justify-content-between">
                <h4 class="mb-2">Excess Stock (Stock In) List
                </h4>
                ...
                @if (count($data) > 0)
                    @foreach ($data as $item)
                        <li class="nav-item w-100" role="presentation">
                            <button class="nav-link stock-item {{ $active_id == $item->id ? 'active' : '' }}"
                ...
                <table id="long-list" class="table table-hover d-none data-table" style="table-layout: fixed;width:100%">
                    ...
                    <thead class="text-center">
                        <tr class="text-center">
                            ...
                            <th width="300px" class="text-start">Remarks</th>
                            <th class="text-start" width="200px">Created By</th>
                        ...
                        @foreach ($data as $value)
                            <tr @if ($value->status == 2) class="bg-dark" @endif>
                                ...
                                <td class="text-center"> <a
                                        href="{{ url('stock-in/' . $value->id) }}">{{ @$value->doc_number }}</a> </td>
                                <td>{{ @$value->remarks }}</td>
                                <td>{{ @$value->createdby->full_name }}</td>
                                <td>
                                    <div class="d-flex justify-content-center align-items-center gap-1">
                                        {{-- <a class="btn btn-sm btn-light" href="{{ url('stock-in/' . $value->id . '/view') }}"
                                            class="btn-small"><i class="fa fa-eye" aria-hidden="true"></i></a> --}}
                                        <a class="btn btn-sm btn-light"
                                            href="{{ url('stock-in/' . $value->id . '?stockin_action=edit') }}"
                                            class="btn-small"><i class="ico icon-outline-pen-2"
    ...
    <div class="content-container col-9">
        <div class="tab-content display-flex-tabs" id="purchaseOrderTabContent">
            ...
            <script>
                $(document).ready(function() {
                    $(document).on('click', '.stock-item', function() {
                        ...
                        $('.stock-item[data-id="' + id + '"]').addClass('active');

                        // Update the browser URL to include selected ID (without reloading)
                        var newUrl = "{{ url('stock-in') }}/" + id;
                        window.history.pushState({
                            path: newUrl
                        }, '', newUrl);

                        var action = "{{ URL::to('stock-in') }}/" + id + "/view";
                        $('#loading_bg').show();
            ...
            <div class="" role="tabpanel" aria-labelledby="po-tab" id="stock-details">
                @if ($action === 'add')
                    @include('backEnd.inventory.StockInForm', $createData)
                @elseif($action === 'edit')
                    @include('backEnd.inventory.StockInFormEdit', $editData)
                @elseif (!empty($selectedStock))
                    @include('backEnd.inventory.StockInFormView', $selectedStock)
                @else
                    <form id="supplierForm" method="GET" action="{{ url('stock-in') }}">

                        <input type="hidden" name="stockin_action" value="add">
                        ...
                        <div onclick="document.getElementById('supplierForm').submit();"
                            ...
                            <div class="text-center mb-4">
                                ...
                                <h1 class="fw-bold mt-3" style="cursor:pointer"> Stock In</h1>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#search_stockin').on('keyup', function() {
                var query = $(this).val();

                $.ajax({
                    url: "{{ route('stock-in-url.search') }}",
                    type: "GET",
                    data: {
                        query: query
                    },
                    success: function(data) {
                        ...
                        if (data.length > 0) {
                            ...
                            <button class="nav-link stock-item"
