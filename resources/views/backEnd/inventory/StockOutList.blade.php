...
@section('mainContent')
    ...
    <aside class="left-nav col-3" id="leftSidebar">
        ...
        <div class="short-list" id="filters-short">
            <h4 class="mb-2">Shortage Stock (Stock Out)
            </h4>

         
            <div class="search-filter-container mb-4">
                <div class="input-group flex-nowrap">
                    <input type="text" name="documents_number" id="search_stockout" class="form-control" placeholder="Document No"
        ...
        <div class="long-list  d-none" id="filters-long">
            <div class="d-flex align-items-center justify-content-between">
                <h4 class="mb-2">Shortage Stock (Stock Out) List
                </h4>
 ...
 <input type="text" id="tableSearch" class="form-control d-inline-block"
                    ...
                    <tbody>
                        @foreach ($data as $value)
                            <tr @if ($value->status == 2) class="bg-dark" @endif>
                                ...
                                <td class="text-center"> <a href="{{url('stock-out/'.$value->id)}}">{{ @$value->doc_number }}</a> </td>
                                <td>{{ @$value->remarks }}</td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center align-items-center gap-1">
                                        ...
                                        <a class="btn btn-sm btn-light"
                                            href="{{ url('stock-out/' . $value->id . '?stockout_action=edit') }}" class="btn-small"><i
                                                class="ico icon-outline-pen-2" style="font-size: 16px;"></i></a>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            <script>
                $(document).ready(function() {
                    $(document).on('click', '.stock-item', function() {
                        ...
                        var newUrl = "{{ url('stock-out') }}/" + id;
                        window.history.pushState({
                            path: newUrl
                        }, '', newUrl);

                        var action = "{{ URL::to('stock-out') }}/" + id + "/view";
                        $('#loading_bg').show();
                ...
                <div class="" role="tabpanel" aria-labelledby="po-tab" id="stock-details">
                    @if ($action === 'add')
                        @include('backEnd.inventory.StockOutForm', $createData)
                    @elseif($action === 'edit')
                        @include('backEnd.inventory.StockOutFormEdit', $editData)
                    @elseif (!empty($selectedStock))
                        @include('backEnd.inventory.StockOutFormView', $selectedStock)
                    @else
                         <form id="supplierForm" method="GET" action="{{ url('stock-out') }}">

            <input type="hidden" name="stockout_action" value="add">
                      ...
                      <div onclick="document.getElementById('supplierForm').submit();" class="container-fluid d-flex flex-column justify-content-center align-items-center"
                        ...
                        <div class="text-center mb-4" >
                            ...
                            <h1 class="fw-bold mt-3" style="cursor:pointer" > Stock Out</h1>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#search_stockout').on('keyup', function() {
                var query = $(this).val();

                $.ajax({
                    url: "{{ route('stock-out-url.search') }}",
                    type: "GET",
                    data: {
                        query: query
                    },
