...
<div class="purchase-order-content-header d-flex align-items-center justify-content-between mb-1 gap-2">
    ...
    <div class="purchase-order-content-header-right d-flex align-items-center justify-content-end gap-2">
        ...
        <form method="GET" action="{{ url('stock-in', @$edit->id) }}">
            {{-- <input hidden type="text" value="{{@$po->id}}" name="id"> --}}
            <button type="submit" name="stockin_action" value="edit" class="btn btn-light">
                <i class="ico icon-outline-pen-2 text-success"></i> Edit
            </button>
        </form>

        <form method="GET" action="{{ url('stock-in') }}">
            <button type="submit" name="stockin_action" value="add" class="btn btn-light">
        ...
        <div class="" id="deal-info" role="tabpanel" aria-labelledby="deal-info-tab">
                ...
                <div class="col-8 mb-2 pdfarea">
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            ...
                            <td align="right"><b style="font-size: 30px; font-weight: 400;">Excess Stock</b></td>
                        </tr>
                    </table>
                    <br />
            ...
            @if ($m_trnno != '')TRN No: {{@$m_trnno}}<br>@endif --}}
                            ...
                            <td style="line-height: 18px;" valign="top">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tr>
                                        <td style="padding: 0px; margin: 0px; width: 150px;">Excess Stock No</td>
                                        <td style="padding: 0px; margin: 0px">: {{ @$edit->doc_number }}</td>
                                    </tr>
                                    <tr>
                                        <td style="padding: 0px; margin: 0px">Excess Stock Date</td>
                                        <td style="padding: 0px; margin: 0px">:
                                            {{ date('d/m/Y', strtotime(@$edit->date)) }}</td>
                                    </tr>
                    ...
                    @if (count($edit_items) > 0)
                        @foreach ($edit_items as $dt)
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td class="item-row" style="width: 20px;">{{ $i }} <?php $i++; ?>
                                        <span id="spn_"{{ $i }}></span></td>
                                    <td class="item-row">
                                        <span style="font-weight:bold;">{{ $dt->partno }}</span><br />
                                        <span style="font-size:11px;">{!! nl2br($dt->serialno) !!}</span> <br>
                                        <span style="font-size:11px;">{!! nl2br($dt->description) !!}</span>
                                    </td>
                                    <td class="item-row" style="width:20px; text-align: center;">{{ $dt->qty }}
                                    </td>
                                    <td class="item-row" style="width:70px; text-align: right;">
                                        {{ @App\SysHelper::com_curr_format($dt->unitprice, 2, '.', ',') }}</td>
                                    <td class="item-row" style="width:80px; text-align: right;">
                                        {{ @App\SysHelper::com_curr_format($dt->value, 2, '.', ',') }}</td>
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                            ...
                            <td
                                ...
                                Total Amount {{ $edit->currency_name->code }}</td>
                            <td
                                style="width: 80px; text-align: right; font-weight: bold; border-bottom: solid 1px #2c2b6d;">
                                {{ @App\SysHelper::com_curr_format($total_amount, 2, '.', ',') }}</td>
                        </tr>
                    </table>
                    ...
                    <footer>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td style="width: 30%; border: none; font-size: 10px;" align="left" valign="bottom">
                                    {{ @$po->createdby->full_name }}<br /><b class="bottom_b">Prepared By</b></td>
                                <td style="width: 40%; border: none; font-size: 10px;" align="center" valign="bottom">
                                    <br /><br />This document is computer generated Signature is not required</td>
