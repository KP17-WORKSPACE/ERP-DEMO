...
@section('mainContent')
        ...
        <div class="long-list" id="filters-long">
            ...
            <div class="search-filter-container mt-1 mb-4 filter-field d-none border">
                ...
                <div class="card" style="width: 100%">
                    <div class="card-body">
        {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'inventory-brand-report', 'method' => 'POST', 'id' => 'inventory-report']) }}
                    ...
                    <div class="no-gutters input-right-icon">
                        <div class="col">
                            <div class="input-effect">
                                ...
                                <input class="form-control date-picker" id="from_date" type="text" name="from_date" value="{{ @App\SysHelper::normalizeToDmy($from_date) }}" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="no-gutters input-right-icon">
                        <div class="col">
                            <div class="input-effect">
                                ...
                                <input class="form-control date-picker" id="to_date" type="text" name="to_date" value="{{ @App\SysHelper::normalizeToDmy($to_date) }}" autocomplete="off">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-3 mb-2">
                    ...
                    <input class="form-control" type="text" autocomplete="off" name="part_number" id="part_number" value="{{ $ctrl_part_number }}">
                  ...
                  <div class="col-1-5 mb-2">
                    ...
                    <select class="form-control js-example-basic-single" name="brand" id="brand">
                        ...
                        <option value="all" @if($ctrl_brand == "all") selected @endif>All</option>
                ...
                <div class="col-1-5 mb-2">
                    ...
                    <select class="form-control js-example-basic-single" name="category">
                        ...
                        <option value="{{ @$value->id }}" @if($ctrl_category == $value->id) selected @endif>{{ @$value->category_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-1-5 mb-2">
                    ...
                    <select class="form-control js-example-basic-single" name="sub_category">
                        ...
                        <option value="{{ @$value->id }}" @if($ctrl_sub_category == $value->id) selected @endif>{{ @$value->sub_category_name }}</option>
                        @endforeach
                    </select>
                </div>
                            ...
                            <div class="input-effect">
                                ...
                                <select class="form-control js-example-basic-single" name="supplier" id="supplier">
                                    ...
                                    @foreach ($supplier_list as $value)
                                        <option value="{{ @$value->id }}" @if(@$ctrl_supplier == $value->id) selected @endif>{{ @$value->account_name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="col-3 mb-2" @if(session('logged_session_data.company_id')!=1) style="display: none;" @endif>
                ...
                <table id="long-list" class="table table-hover data-table" style="table-layout: fixed;width:100%">
                        ...
                        @endphp
                            @foreach($stocklist2 as $ind => $value)
                                ...
                                <tr class="brand-report-summary">
                                    ...
                                    <td><a href="{{ $detailHref }}" target="_blank" rel="noopener noreferrer">{{@$value->part_number}}</a></td>
                                    <td class="text-start">
                                    ...
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['avg_rate'], 2, '.', ',') }}</td>
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['value'], 2, '.', ',') }}</td>
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['discount'], 2, '.', ',') }}</td>
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['taxableamount'], 2, '.', ',') }}</td>
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['vatamount'], 2, '.', ',') }}</td>
                                    <td class="text-end">{{ @App\SysHelper::com_curr_format($t['total_amount'], 2, '.', ',') }}</td>

                                    <td class="text-center">
                                        <div class="d-flex justify-content-center">   
                                            <a href="{{ $detailHref }}" target="_blank" rel="noopener noreferrer" class="btn btn-light" data-bs-popover="popover" data-bs-trigger="hover" data-bs-delay="500" data-bs-content="View Sub Report" data-bs-placement="bottom" data-bs-original-title="" title="">
                                                <i class="ico icon-outline-eye text-success" style="font-size:16px" ></i>
                                            </a>
                                        
                                            <a href="{{ url('stock-ledger/'.$value->part_number) }}" target="_blank" rel="noopener noreferrer" class="btn btn-light" data-bs-popover="popover" data-bs-trigger="hover" data-bs-delay="500" data-bs-content="View Stock Ledger" data-bs-placement="bottom" data-bs-original-title="" title="">
                            ...
                            @if (!isset($stocklist2) || count($stocklist2) === 0)
                                <tr>
                    ...
                    @if(isset($stocklist2) && count($stocklist2) > 0)
                        ...
                        <tr>
                            ...
                            <th class="text-center">{{ $grand_qty_si }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_avg_rate_si,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_value_si,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_discount_si,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_taxableamount_si,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_vatamount_si,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_total_amount_si,2,'.',',') }}</th>
                            <th class="text-center"></th>
                        </tr>
                        <tr>
                            <th colspan="6" class="text-end">Sales Return</th>
                            <th class="text-center">{{ $grand_qty_sr }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_avg_rate_sr,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_value_sr,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_discount_sr,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_taxableamount_sr,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_vatamount_sr,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_total_amount_sr,2,'.',',') }}</th>
                            <th class="text-center"></th>
                        </tr>
                        <tr id="ibrFooterNet">
                            <th colspan="6" class="text-end">Net Sales</th>
                            <th class="text-center">{{ $grand_qty }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_avg_rate,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_value,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_discount,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_taxableamount,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_vatamount,2,'.',',') }}</th>
                            <th class="text-end">{{ @App\SysHelper::com_curr_format($grand_total_amount,2,'.',',') }}</th>
                            <th class="text-center"></th>
                        </tr>
                    </tfoot>
                    @endif
    ...
    $(document).ready(function () {
        $('#exportExcelItems').on('click', function (e) {
            ...
            var companyName = @json(\App\SysCompany::find(session('logged_session_data.company_id') ?? 0)->trade_name ?? '');
