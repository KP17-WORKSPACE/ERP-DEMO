...
@section('mainContent')
    ...
    <?php try { ?>
        <div class="container-fluid">
            <div class="d-sm-flex justify-content-between">
                <div class="mb-3">
                    <h2 class="page-heading m-0">Postdated Payment</h2>
                    <span class="page-label">Home - Postdated Payment</span>
                </div>
                <div>
                    <a href="{{ url('postdatedpayment-add') }}" type="button" class="btn btn-info"><i class="fa fa-plus"></i> New</a>
                    <a href="{{ url('postdatedpayment') }}" type="button" class="btn btn-info"><i class="fa fa-list"></i> List</a>
                </div>
            </div>
            
            <div class="card p-4 mb-2">
                @if(isset($editData))
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'postdatedpayment-update/'. @$editData->id, 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'id' => 'postdatedpayment-create-form']) }}
                <input type="hidden" value="{{@$editData->id}}" name="cust_id">
                @else
                {{ Form::open(['class' => 'form-horizontal', 'files' => true, 'url' => 'postdatedpayment-store', 'method' => 'POST', 'enctype' => 'multipart/form-data', 'id' => 'postdatedpayment-create-form']) }}
                @endif
    
                <input type="hidden" id="postdatedpayment_process_id" name="process_id" value="{{Auth::user()->id . date("YmdHis")}}">
                    ...
                    <div class="col-lg-12">
                      <div class="white-box">
                                    ...
                                    <div class="no-gutters input-right-icon">
                                        <div class="col">
                                            <div class="input-effect">
                                                ...
                                                <input class="form-control" type="text" id="doc_number" name="doc_number" value="{{isset($editData)?@$editData->doc_number:'PP-'.sprintf('%03d', @App\SysPostdatedPayment::max('id') + 1) }}" readonly>
                                                <span class="focus-border"></span>
                            ...
                            <div class="equipment comon-status row mt-4 d-block">
                                <table class="table table-bordered table-striped" id="pdp-table" width="100%" cellspacing="0">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        <td>{{$roid}}</td>
                                            ...
                                            <td>
                                                <input class="form-control" type="number" id="amount_{{$roid}}" name="amount[]" autocomplete="off" min="0" value="{{@$editDataList[$roid-1]->debit_amount}}" onchange="calc_total()" onkeypress="pdp_popup_fun({{$roid}})">
                                            </td>
                                            <td>
                                        ...
                                        @endfor
                                        <?php $roid--;?>
                                        <input type="hidden" id="pdp-row-count" value="{{$roid}}">
                                        <a data-modal-size="modal-md" data-target="#pdp_popup_win" id="addCtrlPostdatedPaymentAdjest" data-toggle="modal"></a>
                                        <script>                                        
                                ...
                                <div style="display: none;">
                                    @if(!isset($view))
                                        <button type="button" class="primary-btn small fix-gr-bg" id="addRowPDP"><span class="ti-plus pr-2"></span>@lang('lang.item')</button>
                                    @endif
                                </div>
    ...
    function fn_addRow(id) {
        var rownum = document.getElementById('pdp-row-count').value;
        if(id==rownum) {
         document.getElementById('pdp-row-count').value = (Number(rownum) + Number(1));
            document.getElementById('addRowPDP').click();
        }
    }
    
    function calc_total(){
        var countrow = document.getElementById('pdp-row-count').value;
        var t1=0;
                    ...
                    <div class="row mt-40">
                        <div class="col-lg-12 text-center">
                            ...
                            <button class="btn btn-primary">
                                ...
                                @if(isset($editData)) @lang('lang.update') @else @lang('lang.add') @endif @lang('Postdated Payment')
                            </button>
                            @endif
                        </div>
                    </div>
    ...
    <form id="ta" >
    <div class="modal fade admin-query" id="pdp_popup_win">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="equipment comon-status row mt-40 d-block">
                                    <table class="sstable" cellspacing="0" width="100%" id="pdpListPostdatedPaymentAdjest">
                                        <thead>
                                            <tr>
...
<script>
    function PostdatedPaymentAdjestBalance(id) {
    ...
    function validatePostdatedPaymentAdjestForm(id) {
    ...
    var account_id = $('#pdp_account_id').val();
    var entry_date = $('#doc_date').val();
    var transaction_type ='postdatedpayment';
    var entry_type =1; //1 Debit, 2 Credit
    var process_id = $('#postdatedpayment_process_id').val();
    ...
    $(".btn_ajax_pdp").prop('disabled', true);
    ...
    $.ajax({
        ...
        success: function(response) {  
            ...
            {
                $("#btn_close2").click();
                $("#addCtrlPostdatedPaymentAdjest").click();
            }
        },
    ...
    function pdp_popup_fun(id)
    {
        $('#amount_'+id+'').keypress(function (e) {
        var key = e.which;
        if(key === 13)  // the enter key code
            {
                var pdp_account_id = $('#account_id_'+id+'').val();
                var pdp_account = $('#amount_'+id+'').val();
                if(pdp_account_id != "" && pdp_account != ""){
                    $('#pdp_account_id').val(pdp_account_id);
                    $('#pdp_account_id_amount').val(pdp_account);
                    $('#bi_adjusted_amount').val(pdp_account).focus();
