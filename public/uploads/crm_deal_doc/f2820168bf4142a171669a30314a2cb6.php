                    ...
                    <div class="row">
                        <div class="col-md-3 mb-2 filter-field d-none">
                            ...
                            <select class="form-control" name="role_id" id="role_id">
                                <option value=""></option>
                                @foreach ($roles as $r)
                                    <option value="{{ $r->id }}" @if(request('role_id')==$r->id) selected @endif>{{ $r->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-1-5 mb-2 filter-field d-none">
                            ...
                            <input class="form-control" type="text" name="staff_no" value="{{ request('staff_no') }}">
                        </div>

                        <div class="col-1-5 mb-2 filter-field d-none">
                            <label class="form-label">Name</label>
                            <input class="form-control" type="text" name="staff_name" value="{{ request('staff_name') }}">
                        </div>
...
@if ($staffs->count())
  @foreach ($staffs as $s)
    <li class="nav-item w-100" role="presentation">
      <button class="nav-link stf-item {{ (isset($active_id) && $active_id == $s->id) ? 'active' : '' }}"
              data-id="{{ $s->id }}" type="button" role="tab">
        <div class="row w-100 align-items-center">
          <div class="col-2 d-flex justify-content-center">
            <div class="rounded-circle bg-light border" style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;font-weight:600;color:#555;">
              {{ strtoupper(substr($s->first_name ?? $s->full_name ?? 'U',0,1)) }}
            </div>
          </div>
          <div class="col-10 ps-0">
            <div class="row">
              <div class="col-4"><span class="form-control-plaintext fw-semibold">{{ $s->staff_no ?? '—' }}</span></div>
              <div class="col-3"><span class="form-control-plaintext truncate-text">{{ optional($s->roles)->name ?? '—' }}</span></div>
              <div class="col-5"><span class="form-control-plaintext truncate-text text-end">@if($s->ext_no) Ext: {{ $s->ext_no }} @endif</span></div>
            </div>
            <div class="d-flex justify-content-between align-items-center text-muted xsmall mt-1">
              <span class="form-control-plaintext truncate-text">{{ $s->first_name }} {{ $s->last_name }}</span>
              @if(!empty($s->ext_no))
                <span class="form-control-plaintext truncate-text">{{ $s->email ?? '—' }}</span>
              @endif
            </div>
          </div>
        </div>
                    ...
                    @foreach ($staffs as $s)
                        <tr @if (@$s->delete_status == 0) style="background-color: rgba(0,0,0,0.05);" @endif>
                          ...
                          @php
                                ...
                                $co = $company->whereIn('id', $idArr)->sortBy(function ($c) use ($idArr) {
                                    return array_search($c->id, $idArr);
                                })->pluck('company_name');
                            @endphp
                            <td>
                                @foreach ($co as $cname)
                                    <span style="font-size:11px;padding:0.25em 0.4em;background-color:#cfe2ff" class="text-xs pr-1 pl-1">{{ $cname }}</span>
                                @endforeach
                            </td>
                            <td>
                                @if (optional($s->maincompany)->company_name)
                                    <span style="font-size:11px;padding:0.25em 0.4em;background-color:#d4edda" class="text-xs pr-1 pl-1">
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-start align-items-center gap-1">
                                    <a href="{{ url('hrms/staff/' . $s->id . '/edit') }}" class="btn btn-sm btn-light" title="Edit">
                                    ...
                                    @if ($s->role_id != 1)
                                        <a href="{{ route('deleteStaffView', $s->user_id) }}" class="btn btn-sm btn-light"
...
<div class="content-container col-9">
 ...
 <script>
  (function () {
    ...
    @php
        $firstStaff = isset($selectedStaff) && $selectedStaff 
                        ? $selectedStaff 
                        : ($staffs->first() ?? null);
    @endphp
    ...
    @if ($firstStaff)
        ...
        @include('backEnd.humanResource.staff_details', ['staff' => $firstStaff])
    @else
        <div class="container-fluid d-flex flex-column justify-content-center align-items-center"
            ...
            <div class="text-center mb-4">
                <div class="rounded-circle bg-success text-white d-flex justify-content-center align-items-center mx-auto"
                    ...
                    <i class="ico icon-outline-user"></i>
                </div>
                <h1 class="fw-bold mt-3">Staff</h1>
                <p class="text-muted">Select a staff member from the list to view details</p>
            </div>
        </div>
    @endif
</div>
