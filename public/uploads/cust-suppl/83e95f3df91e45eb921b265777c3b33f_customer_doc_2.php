...
$docs = $company->documents->first() ?? collect();

$setting = $company->settings->first() ?? collect();
        ...
        <div class="row g-2 align-items-start">
            ...
            <div class="col-md-10">
                ...
                <div class="row g-2">
                    ...
                    <div class="col-md-2">
                        <label class="small mb-0">Busienss Entity:</label>
                        <div class="small">{{ $company->businessEntity->name ?? '—' }}</div>
                    </div>

                    
                    <div class="col-md-2">
                        <label class="small mb-0">Industry</label>
                        <div class="small">{{ $company->businessIndustry->name ?? '—' }}</div>
                    </div>

                    <div class="col-md-2">
                        <label class="small mb-0">Sector</label>
                        <div class="small">{{ $company->businessSector->name ?? '—' }}
                        </div>
                    </div>
...
<div class="tab-wrap mb-3 mt-2">
    <ul class="nav nav-tabs">
        ...
        {{-- <li class="nav-item">
            <button class="nav-link" data-bs-toggle="tab" data-bs-target="#hrTab">
                HR & Payroll
            </button>
        </li> --}}
...
{{-- <div class="tab-pane fade show active" id="basicTab">
    ...
    <div class="row g-2">
        ...
        <div class="col-md-3">
            ...
            <div class="small">{{ $company->sales_code ?? '—' }}</div>
        </div>

        <div class="col-md-3">
            <label class="small mb-0">Other Code:</label>
            <div class="small">{{ $company->other_code ?? '—' }}</div>
        </div>
        ...
        <div class="col-md-3">
            <label class="small mb-0">Telephone:</label>
            <div class="small">{{ $company->telephone ?? '—' }}</div>
        </div>

        <div class="col-md-3">
            <label class="small mb-0">Fax:</label>
            <div class="small">{{ $company->fax ?? '—' }}</div>
        </div>
        ...
        <div class="col-md-3">
            ...
            <div class="small">{{ $company->currency_new->code ?? '—' }}</div>
        </div>
...
<div class="tab-pane fade show active" id="contactTab">
    ...
    <div class="col-md-3">
        <label class="small mb-0">Country:</label>
        <div class="small">
            {{ $company->country['name'] ?? $company->countryRelation->name ?? '—' }}
        </div>
    </div>
    ...
    <div class="col-md-3">
        <label class="small mb-0">State:</label>
        <div class="small">
            {{ $company->stateRelation->name ?? $company->state ?? '—' }}
        </div>
    </div>
...
<div class="tab-pane fade" id="complianceTab">
    ...
    <div class="row g-3 mt-2">
        ...
        <div class="col-md-2">
            ...
            <div class="small">{{ $setting->is_customer_code ? 'Yes' : 'No' }}</div>
        </div>

        <div class="col-md-2">
            <label class="small mb-0">Supplier Code:</label>
            <div class="small">{{ $setting->is_supplier_code ? 'Yes' : 'No' }}</div>
        </div>

        <div class="col-md-2">
            <label class="small mb-0">Account Code:</label>
            <div class="small">{{ $setting->is_account_code ? 'Yes' : 'No' }}</div>
        </div>

        <div class="col-md-2">
            <label class="small mb-0">Sub-Account Code:</label>
            <div class="small">{{ $setting->is_subaccount_code ? 'Yes' : 'No' }}</div>
        </div>
        ...
        <div class="col-md-2">
            ...
            <div class="small">{{ ucfirst($setting->hr_payroll_cycle ?? '—') }}</div>
        </div>
        ...
        <div class="col-md-2">
            ...
            <div class="small">{{ ucfirst($setting->hr_weekly_off ?? '—') }}</div>
        </div>

        <div class="col-md-2">
            <label class="small mb-0">Gratuity Method:</label>
            <div class="small">{{ ucfirst(str_replace('_', ' ', $setting->hr_gratuity_method ?? '—')) }}</div>
        </div>
        ...
        <div class="col-md-2">
            ...
            <div class="small">
                @if($setting->hr_insurance_policy_expiry)
...
<div class="tab-pane fade" id="bankingTab">
            ...
            <table class="table table-sm align-middle mb-0">
                ...
                @foreach($banks as $b)
                    <tr>
                        ...
                        <td>{{ $b->currency ?? '—' }}</td>
                        <td class="text-center">{!! $link($b->bank_letter) !!}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
    ...
    @else
        ...
        <div class="row g-2">
            ...
            <div class="col-md-3"><label class="small mb-0">Bank Letter:</label> <div class="small">{!! $link($company->company_logo) !!}</div></div>
        </div>
...
<div class="tab-pane fade" id="docsTab">
    ...
    <div class="table-responsive">
        <table class="table table-sm">
            ...
            <tbody>
            @foreach($rows as [$label, $num, $exp, $file])
                <tr>
                    <td>{{ $label }}</td>
                    <td>{{ $num ?: '—' }}</td>
                    <td>{{ $exp ?: '—' }}</td>
