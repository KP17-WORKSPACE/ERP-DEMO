...
// 1 SUPER ADMIN START
            ...
            $leads_type_project         = $lead_ret[3];
            $leads_type_channel         = $lead_ret[4];
            $leads_type_corporate       = $lead_ret[5];
            ...
            $users = $query->pluck('user_id');

            if(session('logged_session_data.company_id') == 0){ $sales_target = SysCrmSalesTarget::whereRaw("DATE_FORMAT(target_month_from, '%Y-%m') <= '" . date('Y-m') . "'")->wherein('user_id',$users)
                ->orderby('target_month_from','desc')->get(); }
            else{$sales_target = SysCrmSalesTarget::whereRaw("DATE_FORMAT(target_month_from, '%Y-%m') <= '" . date('Y-m') . "'")->wherein('user_id',$users)->orderby('target_month_from','desc')->get();}
            ...
            if(session('logged_session_data.company_id')==0){
                $sales = [0,0,0]; //SysHelper::get_total_sales_revenue_all($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $service = [0,0]; //SysHelper::get_total_service_revenue_all($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $amc = [0,0]; // SysHelper::get_total_amc_revenue_all($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $project = [0,0]; // SysHelper::get_total_project_revenue_all($_SESSION["page_date_id"],session('logged_session_data.company_id'));
            } else{
                $sales = [0,0,0]; //SysHelper::get_total_sales_revenue($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $service = [0,0]; // SysHelper::get_total_service_revenue($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $amc = [0,0]; // SysHelper::get_total_amc_revenue($_SESSION["page_date_id"],session('logged_session_data.company_id'));
                $project = [0,0]; // SysHelper::get_total_project_revenue($_SESSION["page_date_id"],session('logged_session_data.company_id'));
            }
           ...
           /* if(count($company_list)>0){
            ...
            $on_process =  0; //SysCrmReportController::get_total_on_process_by_company('m',session('logged_session_data.company_id'));
...
// 2 ADMIN START
            if(Auth::user()->role_id == 2){
                ...
                $sales_person = SmStaff::select('user_id','full_name')->whereRaw("find_in_set($com_id,company_access)")->get();
