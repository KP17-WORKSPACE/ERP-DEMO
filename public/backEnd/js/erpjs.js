...
function setBillWiseEnteredAmount(amount, adjustedAmount) {
    ...
    var displayAdjusted = formatAmount(adjustedAmount === undefined ? 0 : parseErpAmount(adjustedAmount));
...
function togglePositiveUnadjustedSection(showSection) {
    var $section = $('#billWisePositiveUnadjustedSection');
    if ($section.length) {
        $section.toggle(!!showSection);
    }
    if (!showSection) {
        $('#footer_unadjusted_amount').text(formatAmount(0));
        $('#footer_unadjusted_adjustment').text(formatAmount(0));
    }
}
function updatePositiveUnadjustedTotals() {
    var amountTotal = 0;
    var adjustmentTotal = 0;

    $('#crListBankBookAdjestUnadjusted tbody tr').each(function () {
        amountTotal += parseErpAmount($(this).find('input[name="bi_balance[]"]').val());
        adjustmentTotal += parseErpAmount($(this).find('.tot_amt').val());
    });

    $('#footer_unadjusted_amount').text(formatAmount(amountTotal));
    $('#footer_unadjusted_adjustment').text(formatAmount(adjustmentTotal));
}
function updateBillWiseAdjustmentTotals() {
    var enteredAmount = parseErpAmount($('#bi_cheque_amount').val());
    var adjustedTotal = 0;
    var invoiceAdjustedTotal = 0;

    $('#cr_popup_win .tot_amt').each(function () {
        adjustedTotal += parseErpAmount($(this).val());
    });
    $('#crListBankBookAdjest tbody .tot_amt, .crListBankBookAdjest tbody .tot_amt').each(function () {
        invoiceAdjustedTotal += parseErpAmount($(this).val());
    });
    ...
    $('#footer_adjustment').text(formatAmount(invoiceAdjustedTotal));
    updatePositiveUnadjustedTotals();
}
function isBillWiseAdjustmentWithinLimit() {
    var enteredAmount = parseErpAmount($('#bi_cheque_amount').val());
    var adjustedTotal = 0;
    var isWithinRowLimit = true;

    $('#cr_popup_win .tot_amt').each(function () {
        var lineAmount = parseErpAmount($(this).val());
        var rowMax = parseErpAmount($(this).data('max-adjust'));
        adjustedTotal += lineAmount;
        if (rowMax > 0 && lineAmount > rowMax) {
            isWithinRowLimit = false;
        }
    });

    return adjustedTotal <= enteredAmount && isWithinRowLimit;
}
function enforceBillWiseAdjustmentLimit(input) {
    ...
    var rowMax = parseErpAmount($input.data('max-adjust'));
    if (rowMax > 0) {
        maxAllowed = Math.min(maxAllowed, rowMax);
    }
    ...
    } else if (parseErpAmount($input.val()) < 0) {
        $input.val(formatAmount(0));
    }

    updateBillWiseAdjustmentTotals();
}
function autoFillBillWiseAdjustmentInput(id) {
    var $input = $('#bi_amount_' + id);
    if (!$input.length) {
        return;
    }
    ...
    var remaining = enteredAmount - otherTotal;
    if (remaining < 0) {
        remaining = 0;
    }
    ...
    var rowMax = parseErpAmount($input.data('max-adjust'));
    if (rowMax <= 0) {
        rowMax = parseErpAmount($('#bi_balance_' + id).val());
    }
    if (rowMax < 0) {
        rowMax = 0;
    }

    $input.val(formatAmount(Math.min(remaining, rowMax)));
    updateBillWiseAdjustmentTotals();
}
function normalizeBillWiseResponse(response) {
    ...
    if (response && typeof response === 'object') {
        var invoices = Array.isArray(response.invoices) ? response.invoices : (Array.isArray(response.data) ? response.data : []);
        var positiveUnadjusted = Array.isArray(response.positive_unadjusted) ? response.positive_unadjusted : [];
...
function getBillWiseDocKey(value) {
    var docNumber = (value && value.doc_number !== undefined ? value.doc_number : '').toString().trim().toUpperCase();
    if (docNumber) {
        return docNumber;
    }

    return [
        value && value.deal_id !== undefined ? value.deal_id : '',
        value && value.doc_date !== undefined ? value.doc_date : '',
        value && value.remarks !== undefined ? value.remarks : ''
    ].join('|').toString().trim().toUpperCase();
}
function filterBillWisePositiveUnadjustedRows(rows, invoiceRows) {
    var invoiceDocKeys = {};
    var filteredByDoc = {};
    var filteredRows = [];

    $.each(invoiceRows || [], function (key, value) {
        var invoiceKey = getBillWiseDocKey(value);
        if (invoiceKey) {
            invoiceDocKeys[invoiceKey] = true;
        }
    });

    $.each(rows || [], function (key, value) {
        var balance = parseErpAmount(value.balance);
        var currentAmount = parseErpAmount(value.bi_amount);
        var displayBalance = balance + currentAmount;
        var docKey = getBillWiseDocKey(value);

        if (balance < 0 || displayBalance <= 0 || (docKey && invoiceDocKeys[docKey])) {
            return;
        }
        ...
        var normalizedRow = $.extend({}, value, {
            _display_balance: displayBalance,
            _current_amount: currentAmount,
            _max_adjust: displayBalance
        });

        if (!filteredByDoc[docKey]) {
            filteredByDoc[docKey] = normalizedRow;
            filteredRows.push(normalizedRow);
            return;
        }

        var existingRow = filteredByDoc[docKey];
        var existingCurrentAmount = parseErpAmount(existingRow._current_amount);
        var existingDisplayBalance = parseErpAmount(existingRow._display_balance);
        var mergedDisplayBalance = Math.max(existingDisplayBalance, displayBalance);

        if (currentAmount > existingCurrentAmount) {
            normalizedRow._display_balance = mergedDisplayBalance;
            normalizedRow._max_adjust = mergedDisplayBalance;
            filteredByDoc[docKey] = normalizedRow;
            filteredRows[filteredRows.indexOf(existingRow)] = normalizedRow;
        } else {
            existingRow._display_balance = mergedDisplayBalance;
            existingRow._max_adjust = mergedDisplayBalance;
        }
    });

    return filteredRows;
}
function getBillWiseDocHref(baseUrl, docNumber, defaultInvoiceRoute) {
    var doc = (docNumber || '').toString();
    if (/^(BR|CR)/i.test(doc)) {
        return baseUrl + '/get-url-receipt/' + doc;
    }
    if (/^JV/i.test(doc)) {
        return baseUrl + '/get-url-journalvoucher/' + doc;
    }
    if (/^SR/i.test(doc)) {
        return baseUrl + '/get-url-sales-return/' + doc;
    }
    if (defaultInvoiceRoute) {
        return baseUrl + '/get-url-sales-invoice/' + doc;
    }
    return '';
}
function getPayableBillWiseDocHref(baseUrl, value) {
    var doc = (value && value.doc_number ? value.doc_number : '').toString();
    var type = (value && value.transaction_type ? value.transaction_type : '').toString().toLowerCase();

    if (!doc || /^OPB-\d+$/i.test(doc) || type === 'opbinvoice') {
        return '';
    }
    if (/^(BP|CP)/i.test(doc)) {
        return baseUrl + '/get-url-payment/' + doc;
    }
    if (/^JV/i.test(doc)) {
        return baseUrl + '/get-url-journalvoucher/' + doc;
    }
    if (/^PR/i.test(doc)) {
        return baseUrl + '/get-url-purchase-return/' + doc;
    }
    return baseUrl + '/get-url-purchase-invoice/' + doc;
}
function getPayableBillWiseDocLabel(baseUrl, value) {
    var docNumber = escapeErpHtml(value && value.doc_number ? value.doc_number : '');
    var href = getPayableBillWiseDocHref(baseUrl, value);
    if (!href || !docNumber) {
        return docNumber || '-';
    }
    return '<a target="_blank" href="' + href + '">' + docNumber + '</a>';
}
function getPayableBillWiseDealLabel(baseUrl, value) {
    var dealCode = escapeErpHtml((value && (value.deal_code || value.deal_id)) ? (value.deal_code || value.deal_id) : '');
    var type = (value && value.transaction_type ? value.transaction_type : '').toString().toLowerCase();

    if (!dealCode) {
        return '-';
    }
    if (dealCode === 'Without Deal' || type === 'opbinvoice') {
        return dealCode;
    }
    return '<a target="_blank" href="' + baseUrl + '/get-url-deal-track/' + dealCode + '">' + dealCode + '</a>';
}
...
$(document).on("click", "#addCtrlBankBookAdjest", function(event) {
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log(response);
            var parsedResponse = normalizeBillWiseResponse(response);
            var invoices = parsedResponse.invoices || [];
            var positiveUnadjusted = filterBillWisePositiveUnadjustedRows(parsedResponse.positive_unadjusted || [], invoices);
            var tr = "";
            var unadjTr = "";

            if (invoices.length === 0 && positiveUnadjusted.length === 0) {
                ...
                $('#crListBankBookAdjestUnadjusted tbody').empty();
                togglePositiveUnadjustedSection(false);
            ...
            $.each(invoices, function(key, value) {
                ...
                var balance = parseErpAmount(value.balance);
                var docHref = getBillWiseDocHref(url, value.doc_number, true);
                var dealHtml = dealId ? '&nbsp;&nbsp;<a target="_blank" href="' + url + '/get-url-deal/' + dealId + '">' + dealId + '</a>' : '-';

                tr += '<tr>\
                        <td class="text-center">' + dealHtml + '<input value="' + dealId + '" class="form-control text-center" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]" autocomplete="off" readonly></td>\
                        <td class="text-center"><a target="_blank" href="' + docHref + '">' + docNumber + '</a><input value="' + docNumber + '" class="form-control text-center row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]" autocomplete="off" readonly></td>\
                        ...
                        <td class="text-center">' + lpoNumber + '<input value="' + lpoNumber + '" class="form-control text-center" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]" autocomplete="off" readonly></td>\
                        <td class="text-end">' + formatAmount(total) + '<input value="' + formatAmount(total) + '" class="form-control text-end" type="hidden" id="bi_total_' + i + '" name="bi_total[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end">' + formatAmount(paid) + '<input value="' + formatAmount(paid) + '" class="form-control text-end" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]" autocomplete="off" min="0" onchange="BankBookAdjestBalance(' + i + ')" readonly></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(balance) + '" class="form-control text-end" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]" autocomplete="off" min="0" readonly></td>\
            ...
            $.each(positiveUnadjusted, function(key, value) {
                var dealId = escapeErpHtml(value.deal_id || '');
                var docNumber = escapeErpHtml(value.doc_number || '');
                var total = parseErpAmount(value.total);
                var paid = parseErpAmount(value.paid);
                ...
                var currentAmount = parseErpAmount(value._current_amount !== undefined ? value._current_amount : value.bi_amount);
                var narration = escapeErpHtml(value.remarks || '');
                var docHref = getBillWiseDocHref(url, value.doc_number, false);
                var dealCode = escapeErpHtml(value.deal_code || value.deal_id || '');
                var dealHtml = dealCode && dealCode !== 'Without Deal' ? '<a target="_blank" href="' + url + '/get-url-deal/' + dealCode + '">' + dealCode + '</a>' : (dealCode || '-');
                var docLabel = docHref ? '<a target="_blank" href="' + docHref + '">' + docNumber + '</a>' : docNumber;

                unadjTr += '<tr>\
                        <td class="text-center">' + dealHtml + '<input value="' + dealId + '" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]"></td>\
                        <td class="text-center">' + get_format_date(value.doc_date) + '<input value="' + (value.doc_date || '') + '" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]"></td>\
                        <td class="text-center">' + docLabel + '<input value="' + docNumber + '" class="row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]"></td>\
                        <td class="text-end">\
                            ' + formatAmount(balance) + '\
                            <input value="' + formatAmount(total) + '" type="hidden" id="bi_total_' + i + '" name="bi_total[]">\
                            <input value="' + formatAmount(paid) + '" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]">\
                            <input value="' + formatAmount(balance) + '" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]">\
                        </td>\
                        <td><input value="' + formatAmount(currentAmount) + '" data-current-amount="' + currentAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control border-0 form-control-sm tot_amt text-end" type="text" step="any" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"></td>\
                    </tr>';
                i++;
            });
            ...
            $('#crListBankBookAdjest tbody').empty().append(tr);
            if (positiveUnadjusted.length > 0) {
                $('#crListBankBookAdjestUnadjusted tbody').empty().append(unadjTr);
                togglePositiveUnadjustedSection(true);
            } else {
                $('#crListBankBookAdjestUnadjusted tbody').empty();
                togglePositiveUnadjustedSection(false);
            }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(invoices, function(key, value) {
                ...
                var paid = parseErpAmount(value.paid);
                var balance = parseErpAmount(value.balance);
                var existingAmount = parseErpAmount(value.bi_amount);
                var docHref = getBillWiseDocHref(url, value.doc_number, true);
                var dealHtml = dealId ? '&nbsp;&nbsp;<a target="_blank" href="' + url + '/get-url-deal/' + dealId + '">' + dealId + '</a>' : '-';
                ...
                tr += '<tr>\
                        ...
                        <td class="text-center"><a target="_blank" href="' + docHref + '">' + docNumber + '</a><input value="' + docNumber + '" class="form-control text-center row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + get_format_date(value.doc_date) + '<input value="' + (value.doc_date || '') + '" class="form-control text-center" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]" autocomplete="off" readonly><input value="' + get_format_date(value.doc_date) + '" class="form-control text-center" type="hidden" autocomplete="off" readonly></td>\
                        <td class="text-center">' + lpoNumber + '<input value="' + lpoNumber + '" class="form-control" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]" autocomplete="off" readonly></td>\
                        <td class="text-end">' + formatAmount(total) + '<input value="' + formatAmount(total) + '" class="form-control" type="hidden" id="bi_total_' + i + '" name="bi_total[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end">' + formatAmount(paid) + '<input value="' + formatAmount(paid) + '" class="form-control" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]" autocomplete="off" min="0" onchange="BankBookAdjestBalance(' + i + ')" readonly></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(balance) + '" class="form-control" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]" autocomplete="off" min="0" readonly></td>\
                        <td><input value="'+ formatAmount(existingAmount) +'" data-current-amount="' + existingAmount + '" class="form-control tot_amt text-end" step="any" type="decimal" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="" class="form-control" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"></td>\
                    </tr>';
                outstamount += total;
                footer_total += total;
            ...
            $.each(positiveUnadjusted, function(key, value) {
                ...
                var balance = parseErpAmount(value._display_balance !== undefined ? value._display_balance : value.balance);
                var maxAdjust = parseErpAmount(value._max_adjust !== undefined ? value._max_adjust : balance);
                var existingAmount = parseErpAmount(value._current_amount !== undefined ? value._current_amount : value.bi_amount);
                var narration = escapeErpHtml(value.remarks || '');
                var docHref = getBillWiseDocHref(url, value.doc_number, false);
                var dealCode = escapeErpHtml(value.deal_code || value.deal_id || '');
                var dealHtml = dealCode && dealCode !== 'Without Deal' ? '<a target="_blank" href="' + url + '/get-url-deal/' + dealCode + '">' + dealCode + '</a>' : (dealCode || '-');
                ...
                unadjTr += '<tr>\
                        ...
                        <td><input value="'+ formatAmount(existingAmount) +'" data-current-amount="' + existingAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control form-control-sm tot_amt text-end border-0" step="any" type="text" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"></td>\
                    </tr>';
                i++;
            });
        ...
        success: function(response) {
            ...
            $.each(invoices, function(key, value) {
                ...
                tr += '<tr>\
                        ...
                        <td class="text-end"><input value="" data-max-adjust="' + balance + '" class="form-control tot_amt text-end border-0" type="text" step="any" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
            ...
            $.each(positiveUnadjusted, function(key, value) {
                ...
                var docHref = getBillWiseDocHref(url, value.doc_number, false);
                var dealHtml = dealCode && dealCode !== 'Without Deal' ? '<a target="_blank" href="' + url + '/get-url-deal/' + dealCode + '">' + dealCode + '</a>' : (dealCode || '-');
                var docHtml = docHref ? '<a target="_blank" href="' + docHref + '">' + docNumber + '</a>' : docNumber;

                unadjTr += '<tr>\
                        <td class="text-center">' + dealHtml + '<input value="' + dealId + '" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]"></td>\
                        <td class="text-center">' + date + '<input value="' + (value.doc_date || '') + '" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]"></td>\
                        <td class="text-center">' + docHtml + '<input value="' + docNumber + '" class="row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]"></td>\
                        <td class="text-end">\
                            ' + formatAmount(balance) + '\
                            <input value="' + formatAmount(total) + '" type="hidden" id="bi_total_' + i + '" name="bi_total[]">\
                            <input value="' + formatAmount(paid) + '" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]">\
                        ...
                        <td>\
                            <input value="' + formatAmount(currentAmount) + '" data-current-amount="' + currentAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control border-0 form-control-sm tot_amt text-end" type="text" step="any" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')">\
                        </td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"></td>\
                    </tr>';
                i++;
        ...
        success: function(response) {
            ...
            $.each(invoices, function(key, value) {
                ...
                tr += '<tr>\
                        ...
                        <td class="text-end"><input value="' + formatAmount(paid) + '" class="form-control text-end border-0" type="text" id="bi_paid_' + i + '" name="bi_paid[]" autocomplete="off" min="0" onchange="BankBookAdjestBalance(' + i + ')" readonly></td>\
                        <td class="text-end"><input value="' + formatAmount(balance) + '" class="form-control text-end border-0" type="text" id="bi_balance_' + i + '" name="bi_balance[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end"><input value="' + formatAmount(storedAmount) + '" data-current-amount="' + storedAmount + '" data-max-adjust="' + balance + '" class="form-control tot_amt text-end border-0" step="any" type="text" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
            ...
            $.each(positiveUnadjusted, function(key, value) {
                var total = parseErpAmount(value.total);
                var storedAmount = parseErpAmount(value._current_amount !== undefined ? value._current_amount : value.bi_amount);
                var paid = parseErpAmount(value.paid);
                var balance = parseErpAmount(value._display_balance !== undefined ? value._display_balance : value.balance);
                var maxAdjust = parseErpAmount(value._max_adjust !== undefined ? value._max_adjust : balance);
                var date = value.doc_date ? get_format_date(value.doc_date) : '';
                ...
                unadjTr += '<tr>\
                        ...
                        <td>\
                            <input value="' + formatAmount(storedAmount) + '" data-current-amount="' + storedAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control form-control-sm tot_amt text-end border-0" step="any" type="text" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')">\
                        </td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"></td>\
                    </tr>';
...
$(document).on("click", "#addCtrlPaymentAdjest", function(event) {
    ...
    if (invoices.length === 0 && positiveUnadjusted.length === 0) {
        ...
        return;
            ...
            $.each(invoices, function(key, value) {
                var dealLabel = getPayableBillWiseDealLabel(url, value);
                var dealCode = escapeErpHtml(value.deal_code || '');
                var dealId = escapeErpHtml(value.deal_id || '');
                var docNumber = escapeErpHtml(value.doc_number || '');
                var lpoNumber = escapeErpHtml(value.lpo_number || '');
                var billNumber = escapeErpHtml(value.bill_number || '');
                var total = parseErpAmount(value.total);
                var paid = parseErpAmount(value.paid);
                var balance = parseErpAmount(value.balance);

                tr += '<tr>\
                        <td class="text-center">' + dealLabel + '<input value="' + dealCode + '" class="form-control row_ctrl" type="hidden" id="bi_deal_code_' + i + '" name="bi_deal_code[]" autocomplete="off" readonly><input value="' + dealId + '" class="form-control row_ctrl" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + getPayableBillWiseDocLabel(url, value) + '<input value="' + docNumber + '" class="form-control row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + get_format_date(value.doc_date) + ' <input value="' + (value.doc_date || '') + '" class="form-control text-center" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]" autocomplete="off" readonly><input value="' + get_format_date(value.doc_date) + '" class="form-control text-center" type="hidden" autocomplete="off" readonly></td>\
                        <td class="text-center">' + lpoNumber + '<input value="' + lpoNumber + '" class="form-control text-center" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + billNumber + '<input value="' + billNumber + '" class="form-control text-center" type="hidden" id="bi_bill_number_' + i + '" name="bi_bill_number[]" autocomplete="off" readonly></td>\
                        <td class="text-end">' + formatAmount(total) + '<input value="' + formatAmount(total) + '" class="form-control text-end" type="hidden" id="bi_total_' + i + '" name="bi_total[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end">' + formatAmount(paid) + '<input value="' + formatAmount(paid) + '" class="form-control text-end" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]" autocomplete="off" min="0" onchange="BankBookAdjestBalance(' + i + ')" readonly></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(balance) + '" class="form-control text-end" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]" autocomplete="off" min="0" readonly></td>\
                        <td><input value="" class="form-control tot_amt text-end" type="decimal" step="any" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
            ...
            $.each(positiveUnadjusted, function(key, value) {
                ...
                unadjTr += '<tr>\
                        <td class="text-center">' + dealLabel + '<input value="' + dealCode + '" type="hidden" id="bi_deal_code_' + i + '" name="bi_deal_code[]"><input value="' + dealId + '" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]"></td>\
                        <td class="text-center">' + get_format_date(value.doc_date) + '<input value="' + (value.doc_date || '') + '" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]"></td>\
                        <td class="text-center">' + getPayableBillWiseDocLabel(url, value) + '<input value="' + docNumber + '" class="row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]"></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(total) + '" type="hidden" id="bi_total_' + i + '" name="bi_total[]"><input value="' + formatAmount(paid) + '" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]"><input value="' + formatAmount(balance) + '" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]"></td>\
                        <td><input value="' + formatAmount(currentAmount) + '" data-current-amount="' + currentAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control form-control-sm border-0 tot_amt text-end" type="text" step="any" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"><input value="" type="hidden" id="bi_bill_number_' + i + '" name="bi_bill_number[]"></td>\
                    </tr>';
                i++;
            });
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(invoices, function(key, value) {
                ...
                tr += '<tr>\
                        <td class="text-center">' + dealLabel + '<input value="' + dealCode + '" class="form-control row_ctrl" type="hidden" id="bi_deal_code_' + i + '" name="bi_deal_code[]" autocomplete="off" readonly><input value="' + dealId + '" class="form-control row_ctrl" type="hidden" id="bi_deal_id_' + i + '" name="bi_deal_id[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + getPayableBillWiseDocLabel(url, value) + '<input value="' + docNumber + '" class="form-control row_ctrl text-center" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + get_format_date(value.doc_date) + '<input value="' + (value.doc_date || '') + '" class="form-control text-center" type="hidden" id="bi_doc_date_' + i + '" name="bi_doc_date[]" autocomplete="off" readonly><input value="' + get_format_date(value.doc_date) + '" class="form-control text-center" type="hidden" autocomplete="off" readonly></td>\
                        <td class="text-center">' + lpoNumber + '<input value="' + lpoNumber + '" class="form-control text-center" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]" autocomplete="off" readonly></td>\
                        <td class="text-center">' + billNumber + '<input value="' + billNumber + '" class="form-control text-center" type="hidden" id="bi_bill_number_' + i + '" name="bi_bill_number[]" autocomplete="off" readonly></td>\
                        <td class="text-end">' + formatAmount(total) + '<input value="' + formatAmount(total) + '" class="form-control text-end" type="hidden" id="bi_total_' + i + '" name="bi_total[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end">' + formatAmount(paid) + '<input value="' + formatAmount(paid) + '" class="form-control text-end" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]" autocomplete="off" min="0" onchange="BankBookAdjestBalance(' + i + ')" readonly></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(balance) + '" class="form-control text-end" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]" autocomplete="off" min="0" readonly></td>\
                        <td class="text-end"><input value="'+ formatAmount(existingAmount) +'" data-current-amount="' + existingAmount + '" class="form-control tot_amt text-end" step="any" type="text" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="" class="form-control" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"></td>\
                    </tr>';
                outstamount += total;
                footer_total += total;
            ...
            $.each(positiveUnadjusted, function(key, value) {
                ...
                unadjTr += '<tr>\
                        ...
                        <td class="text-center">' + getPayableBillWiseDocLabel(url, value) + '<input value="' + docNumber + '" class="row_ctrl" type="hidden" id="bi_doc_no_' + i + '" name="bi_doc_no[]"></td>\
                        <td class="text-end">' + formatAmount(balance) + '<input value="' + formatAmount(total) + '" type="hidden" id="bi_total_' + i + '" name="bi_total[]"><input value="' + formatAmount(paid) + '" type="hidden" id="bi_paid_' + i + '" name="bi_paid[]"><input value="' + formatAmount(balance) + '" type="hidden" id="bi_balance_' + i + '" name="bi_balance[]"></td>\
                        <td><input value="'+ formatAmount(existingAmount) +'" data-current-amount="' + existingAmount + '" data-max-adjust="' + maxAdjust + '" class="form-control form-control-sm border-0 tot_amt text-end" step="any" type="text" id="bi_amount_' + i + '" name="bi_amount[]" autocomplete="off" min="0" onclick="get_set_amount(' + i + ')"></td>\
                        <td><input value="' + narration + '" class="form-control form-control-sm border-0" type="text" id="bi_narration_' + i + '" name="bi_narration[]" autocomplete="off"><input value="" type="hidden" id="bi_lpo_no_' + i + '" name="bi_lpo_no[]"><input value="" type="hidden" id="bi_bill_number_' + i + '" name="bi_bill_number[]"></td>\
                    </tr>';
                i++;
            });
